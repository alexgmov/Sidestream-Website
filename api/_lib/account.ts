...
export async function createOrResumeActivationCheckout(options: {
  activationKey: string;
  baseUrl: string;
}): Promise<ActivationCheckoutResult> {
  const activation = await getActivationCheckoutContext(options.activationKey);
  if (!activation) {
    return {
      ok: false,
      statusCode: 409,
      error: "Activation expired or unavailable",
    };
  }
  ...
  if (activation.checkoutSessionId) {
    ...
    if (attachedSession.status === "complete") {
      ...
      const completedUrl = new URL("/thank-you.html", options.baseUrl);
      completedUrl.searchParams.set("checkout", "success");
      completedUrl.searchParams.set("activation", options.activationKey);
      return { ok: true, url: completedUrl.toString() };
    }
    if (attachedSession.status === "open" && attachedSession.url) {
      return { ok: true, url: attachedSession.url };
    }
    return {
      ok: false,
      statusCode: 409,
      error: "Attached Checkout Session is unavailable",
    };
  }

  const priceId = await getSidestreamProPriceId();
  const productId = getSidestreamProProductId();
  const cancelUrl = new URL("/upgrade.html", options.baseUrl);
  cancelUrl.searchParams.set("checkout", "cancelled");
  cancelUrl.searchParams.set("activation", options.activationKey);
  const metadata = {
    sidestream_plan: SIDESTREAM_PRO_PLAN_KEY,
    sidestream_price_id: priceId,
    sidestream_activation_key: options.activationKey,
  };
  const checkoutSession = await stripe.checkout.sessions.create(
    {
      ...
      allow_promotion_codes: true,
      billing_address_collection: "auto",
      success_url: buildCheckoutCompletionUrl(
        options.baseUrl,
        options.activationKey,
      ),
      cancel_url: cancelUrl.toString(),
      expires_at: activation.checkoutExpiresAt,
      client_reference_id: options.activationKey,
      custom_text: {
        submit: {
    ...
    {
      ...getStripeRequestOptions(),
      idempotencyKey: getActivationCheckoutIdempotencyKey(
        options.activationKey,
      ),
    },
  );
  ...
  if (!attached) {
    const winner = await getActivationCheckoutContext(options.activationKey);
    if (winner?.checkoutSessionId) {
      ...
      if (winnerSession.status === "open" && winnerSession.url) {
        return { ok: true, url: winnerSession.url };
      }
    }
    ...
    return {
      ok: false,
      statusCode: 409,
      error: "Could not attach Checkout to activation",
    };
  }
...
export async function getActivationStatus(
  ...
  const schema = await getAccountRuntimeSchemaCapabilities();
  if (schema.accountDevices) {
  ...
  const issued = schema.accountDevices
    ? await issueLicenseTokenPair({
      activationId: row.activation_id,
      activationKey,
      accountId: row.account_id,
    ...
    : await issuePreMigrationLicenseTokenPair({
      activationId: row.activation_id,
      activationKey,
      accountId: row.account_id,
      licenseId: row.license_id,
...
export async function verifyLicenseToken(
  ...
  const schema = await getAccountRuntimeSchemaCapabilities();
  if (!schema.accountDevices) {
    return verifyPreMigrationLicenseToken(tokenHash, deviceIdHash);
  }
...
async function verifyPreMigrationLicenseToken(
  tokenHash: string,
  deviceIdHash: string,
) {
  const result = await query<{
  ...
  const legacyTokenExpiresAt = needsLegacyLicenseCompatibility(
      row.activation_app_version,
    )
    ? addDays(new Date(), LEGACY_LICENSE_TOKEN_TTL_DAYS).toISOString()
    : "";
  const { legacyTokenRenewalThresholdDays, licenseWriteThrottleSeconds } =
    loadLicenseWriteConfiguration();
  ...
  let effectiveExpiresAt = updated.rows[0]?.expires_at;
  if (!effectiveExpiresAt) {
    const stillCurrent = await query<{ expires_at: Date | string }>(
      `
        select expires_at
    ...
    effectiveExpiresAt = stillCurrent.rows[0]?.expires_at;
    if (!effectiveExpiresAt) {
      return { active: false as const, status: "invalid", code: "revoked" as const };
    }
  }
  return {
    active: true as const,
    status: license.status,
    tokenExpiresAt: toIsoString(effectiveExpiresAt),
    license,
  };
}
...
export async function fulfillCheckoutSession(
  ...
  const schema = await getAccountRuntimeSchemaCapabilities();
  if (activationKey && !schema.entitlementLifecycle) {
    return fulfillPreMigrationActivationCheckout({
      checkoutSession,
      checkoutSessionId,
      activationKey,
      activationId,
...
async function fulfillPreMigrationActivationCheckout(options: {
  ...
  const customerId = normalizeStripeId(options.checkoutSession.customer);
  if (!customerId) {
    return { fulfilled: false as const, reason: "missing_customer" };
  }
  const accountId = await findOrCreateAccountForStripeCustomer(customerId, {
    email: options.checkoutSession.customer_details?.email ||
      options.checkoutSession.customer_email,
    name: options.checkoutSession.customer_details?.name,
  });
  if (!accountId) {
    return { fulfilled: false as const, reason: "missing_account" };
  }
  ...
  return withPgClient(async (client) => {
    ...
    try {
      await client.query("select pg_advisory_xact_lock(hashtext($1))", [
        `checkout_session:${options.checkoutSessionId}`,
      ]);
      const lockedActivation = await client.query<{
        account_id: string | null;
        stripe_checkout_price_id: string;
      ...
      if (
        !locked ||
        locked.stripe_checkout_price_id !== options.expectedPriceId ||
        locked.stripe_checkout_product_id !== options.expectedProductId ||
        !canBindActivationAccount(locked.account_id, accountId)
      ) {
        await client.query("rollback");
        return { fulfilled: false as const, reason: "activation_binding_mismatch" };
      }
      ...
      const licenseId = await upsertPreMigrationOneTimeCheckoutLicense({
        accountId,
        customerId,
        checkoutSessionId: options.checkoutSessionId,
        paymentIntentId: normalizeStripeId(options.checkoutSession.payment_intent),
      }, client);
      if (!licenseId) {
        await client.query("rollback");
      ...
      await client.query("commit");
      return { fulfilled: true as const, activationBound: true };
    } catch (error) {
      await client.query("rollback");
      throw error;
    }
...
async function upsertPreMigrationOneTimeCheckoutLicense(options: {
  accountId: string;
  customerId: string;
  checkoutSessionId: string;
...
type AccountRuntimeSchemaCapabilities = {
  checkoutIntents: boolean;
  entitlementLifecycle: boolean;
  accountDevices: boolean;
};

async function getAccountRuntimeSchemaCapabilities(): Promise<
  AccountRuntimeSchemaCapabilities
> {
  const result = await query<{
    checkout_intents: boolean;
    entitlement_lifecycle: boolean;
    account_devices: boolean;
  }>(
    `
      select
        to_regclass('public.sidestream_checkout_intents') is not null as checkout_intents,
        exists (
          select 1
          from information_schema.columns
          where table_schema = 'public'
            and table_name = 'sidestream_licenses'
            and column_name = 'entitlement_status'
        ) as entitlement_lifecycle,
        to_regclass('public.sidestream_account_devices') is not null as account_devices
    `,
  );
  const row = result.rows[0];
  return {
    checkoutIntents: Boolean(row?.checkout_intents),
    entitlementLifecycle: Boolean(row?.entitlement_lifecycle),
    accountDevices: Boolean(row?.account_devices),
  };
}
...
export async function authorizeLicenseDownload(options: {
  ...
  const schema = await getAccountRuntimeSchemaCapabilities();
  if (!schema.accountDevices) {
    return authorizePreMigrationLicenseDownload(tokenHash, deviceIdHash);
  }
  const accountLookup = await query<{ account_id: string }>(
    `
      select account_id
...
async function authorizePreMigrationLicenseDownload(
  tokenHash: string,
  deviceIdHash: string,
) {
  return withPgClient(async (client) => {
...
async function issuePreMigrationLicenseTokenPair(options: {
  activationId: string;
  activationKey: string;
  accountId: string;
  licenseId: string;
...
export async function refreshLicenseToken(
  ...
  const schema = await getAccountRuntimeSchemaCapabilities();
  if (!schema.accountDevices) {
    return refreshPreMigrationLicenseToken(
      refreshToken,
      refreshTokenHash,
      deviceIdHash,
    );
...
async function refreshPreMigrationLicenseToken(
  refreshToken: string,
  ...
  return withPgClient(async (client) => {
    await client.query("begin");
    try {
      const selectCredential = (whereClause: string) =>
        client.query<RefreshCredentialRow>(
          `
            select
              t.id as token_id,
              t.account_id,
      ...
      const current = (await selectCredential("t.refresh_token_hash = $1")).rows[0];
      if (current && !matchesDeviceHash(current.device_id_hash, deviceIdHash)) {
        await client.query("rollback");
        return { active: false as const, status: "invalid", code: "device_mismatch" as const };
      }

      const replayRow = !current || current.revoked_at
        ? (await selectCredential(`
            t.previous_refresh_token_hash = $1
            and t.previous_refresh_valid_until > now()
            and t.revoked_at is null
            and t.refresh_expires_at > now()
          `)).rows[0]
      ...
      if (replayRow) {
        ...
        if (
          !safeEqual(replayRow.token_hash, hashToken(replayedTokens.licenseToken)) ||
          !safeEqual(
            replayRow.refresh_token_hash,
            hashToken(replayedTokens.refreshToken),
          )
        ) {
          await client.query("rollback");
          return { active: false as const, status: "invalid", code: "invalid_token" as const };
