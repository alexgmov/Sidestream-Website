...
test("the repaired live shape receives only the exact current Stripe Customer link", {
  timeout: 120_000,
}, async () => {
  const summary = await runPaidTelemetryHandoffFixture({
  ...
  assert.equal(
    summary.observedContract,
    "exact-current-stripe-customer-link-repaired",
  );
  assert.ok(Object.values(summary.recoverablePreState).every(Boolean));
  assert.ok(Object.values(summary.customerLinkOnlyRepair).every(Boolean));
