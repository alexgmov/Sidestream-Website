...
test("fixture purchase connects sticky paid entry to idempotent verified $9.99 fulfillment", async () => {
  ...
  assert.deepEqual(
    verifyPaidCheckoutSession(serverRetrievedCheckout, {
      ...
      priceId: "price_test_sidestream_pro_999",
      productId: "prod_test_sidestream_pro",
      paidPlanKeys: ["sidestream_pro"],
    }),
    { ok: true },
  );
  assert.equal(serverRetrievedCheckout.amount_total, 999);
  ...
  const fulfilled = await databasePaidAcquisition
    .completePaidAcquisitionCheckout({
      ...
      verifiedPriceRef: "price_test_sidestream_pro_999",
      verifiedQuantity: 1,
      verifiedAmountMinor: 999,
...
function checkoutSessionFixture() {
  return {
    ...
    metadata: {
      ...
      sidestream_price_id: "price_test_sidestream_pro_999",
      ...
      data: [
        {
          quantity: 1,
          price: {
            id: "price_test_sidestream_pro_999",
