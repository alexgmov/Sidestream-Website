...
`deploy:production` fails unless the worktree is clean, `HEAD` exactly matches the live remote `origin/codex/release-1.0.14` head, and `.vercel/project.json` identifies the pinned Sidestream project/team. It then creates a fresh Production build, rejects a bundle without the Resolve CTA, modal, browser route, or server function, and only then runs the prebuilt Production deployment. Afterward, still inspect `sidestream.tv`, confirm the active alias/deployment ID, open the rendered Resolve modal, verify `GET /api/resolve-waitlist` returns `405 Allow: POST`, and use a clearly synthetic POST plus masked private-Blob readback when the capture path changed.
...
- Run `node scripts/assert-no-runtime-ddl.mjs` and `node scripts/validate-vercel-contract.mjs` after API/migration/routing work. For a human Vercel build, follow `npx vercel@latest build` with `npm run verify:vercel-build`; the verifier requires the Resolve function and its three landing-page integration markers. For Production, use only `npm run deploy:production`, which adds clean canonical-source and pinned-project checks before building.
