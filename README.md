...
- Customer 360 Preview/Test acceptance - `docs/customer-360-preview-test-plan.md`, `scripts/verify-customer-360-preview-environment.mjs`, and `scripts/verify-customer-360-preview-deployment.mjs`; the matrix owns ordered evidence, stop conditions, one-time host-bound usage-sync verification, rollback/recreate, and the hard exit. The shared Preview project is rejected; a dedicated isolated non-Production Vercel project/scheduler with exact provider configuration is a human gate.
