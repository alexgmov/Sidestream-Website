...
- `api/_lib/checkout-offers.ts` - Server-owned regional Checkout catalog and trusted-country selector. It allowlists the global USD offer plus the India INR offer, reads only Vercel's server-side `x-vercel-ip-country` signal, and ignores browser query/body price, currency, country, and offer values. India activates only when `SIDESTREAM_PRO_INDIA_PRICE_ID` names an approved immutable INR Price; otherwise the approved global offer remains the fallback.
...
The global catalog entry uses the approved USD Sidestream Unlimited Price resolver. The India entry uses the same Sidestream Product, requires the server-only `SIDESTREAM_PRO_INDIA_PRICE_ID`, and accepts it only when Stripe reports an active, non-recurring INR Price for that Product with a positive immutable amount. If the India Price ID is not configured, India safely receives the approved global offer; this repository does not create the India Price or change visible landing-page pricing. Future countries use another catalog entry with explicit country codes, currency, and immutable server-configured Price ID.
...
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card links to `/api/checkout/start`; visible `$24.99 once` copy remains the unchanged global presentation, while Checkout truth comes from `api/_lib/checkout-offers.ts` plus the stored intent snapshot. The compatibility-named async `getSidestreamProPriceId()` still resolves the global entry by validating configured/default Price IDs as non-authoritative hints, then checking the expanded Product `default_price`, exact `sidestream_pro_once_2499` lookup key, and any other active matching Product Price before its idempotent create fallback. India uses only the configured immutable `SIDESTREAM_PRO_INDIA_PRICE_ID` after exact Product/INR/one-time validation and never creates a Price. A stale or deleted global configured Price may fall through; an explicitly configured invalid regional Price fails closed. The public tier name is Sidestream Unlimited, while existing `SIDESTREAM_PRO_*`, `sidestream_pro`, and lookup-key identifiers remain stable for historical purchase compatibility. If the visible global amount changes, update the global resolver constants, visible copy, JSON-LD, `llms.txt`, and README together. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
