...
- The standard release manifests are static imports in the shared resolver and runtime inputs to both `api/download.ts` and `api/releases/latest.ts`. Keep those imports plus the `data/release-manifest*.json` `includeFiles` declarations in `vercel.json`; without both traced source dependencies and explicit file maps, Vercel can reuse a prior function bundle after a manifest-only commit even while `/version.json` reports the new SHA.
...
- 2026-08-29: Published the signed, notarized, and stapled standard Mac `1.0.20` installer to `sidestream/1.0.20/Sidestream-1.0.20-Mac-Installer.dmg` and advanced the shared public download/update manifest at a noncritical 25% rollout; the shared resolver now statically imports both manifests, both standard release functions declare their explicit Vercel file inputs, and the Vercel-build verifier checks each resulting file map, so a manifest-only deploy cannot reuse stale release metadata while the independently qualified Windows manifest remains unchanged.
