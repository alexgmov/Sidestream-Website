...
- `account.html` - Minimal noindex account bridge. Uses the server auth session to show plan status, latest installer, sign out, and a Manage Billing button that creates a Stripe Customer Portal session.
- `upgrade.html` - Minimal noindex post-checkout/cancel fallback page. Main upgrade entry points redirect directly to Stripe Checkout through `/api/checkout/start`, while this page preserves optional plugin activation keys for retry links.
...
- `api/download-lead.ts` - Vercel Node Function that accepts email-gate submissions from the download modal and stores each lead in server-side Postgres through the Sidestream Supabase/Vercel connector's `SIDESTREAM_POSTGRES_URL`, with generic `POSTGRES_URL` fallback if the connector env is unavailable.
...
- `api/checkout/start.ts` - Public Stripe Checkout redirect for the Sidestream subscription. It creates a hosted Checkout Session with Managed Payments, the cancellation/refund promise custom text, and optional plugin activation metadata, then redirects directly to Stripe without a Google sign-in interstitial.
- `api/checkout/create.ts` - Backward-compatible authenticated JSON Checkout Session creator for account-page callers. It uses the persisted Basic subscription Stripe Product/Price, enables Managed Payments, and includes the same Stripe-page cancellation/refund custom text. The subscription is not fulfilled until Stripe webhooks update the backend license record.
- `api/billing/portal.ts` - Authenticated Stripe Customer Portal redirect creator for cancellation, invoices, payment methods, and billing history.
- `api/stripe/webhook.ts` - Stripe webhook endpoint. Verifies signatures, stores idempotency rows, and updates Sidestream license state from checkout/subscription events.
...
- `db/migrations/20260704120000_add_sidestream_billing_resources.sql` - Postgres schema for persisted Stripe billing resources, currently the Basic subscription Product and default monthly Price used by Checkout.
...
- Download and upgrade actions - `[data-download]`, `[data-purchase]`, and `#toast`; download CTAs point directly at the public `https://sidestream-xi.vercel.app/api/download` URL with the CSS Apple platform mark from `.btn[data-download]::before`, while Sidestream Unlimited entry points open `/api/checkout/start` so the browser lands directly on Stripe Checkout.
...
- Download lead capture - `api/download-lead.ts` accepts `POST /api/download-lead` JSON with an email, optional page, and optional source, captures the request IP from proxy headers, then writes to Postgres table `public.sidestream_download_leads` through a server-only Postgres connection string. Accepted env names are `SIDESTREAM_POSTGRES_URL`, `SIDESTREAM_POSTGRES_PRISMA_URL`, `SIDESTREAM_POSTGRES_URL_NON_POOLING`, `POSTGRES_URL`, `POSTGRES_PRISMA_URL`, and `POSTGRES_URL_NON_POOLING`, in that order; if Postgres is not configured or the write fails, it falls back to the private Vercel Blob prefix `SIDESTREAM_DOWNLOAD_LEADS_BLOB_PREFIX` or `sidestream/download-leads`
- Account/auth/billing - `account.html`, `upgrade.html`, `api/_lib/account.ts`, `api/auth/*`, `api/checkout/start.ts`, `api/checkout/create.ts`, `api/billing/portal.ts`, `api/stripe/webhook.ts`, `api/activation/*`, and `api/license/verify.ts` own optional Google account management, Stripe Product/Price bootstrap, direct Managed Payments Checkout redirects, Customer Portal redirects, webhook-owned entitlements, plugin activation, and license-token verification
...
```text
sidestream/1.0.9/Sidestream-1.0.9-Mac-Installer.dmg
```
...
The current public installer artifact, `Sidestream-1.0.9-Mac-Installer.dmg`, is 207,509,668 bytes locally, about 198 MiB / 0.208 GB. At that size, 10 GB of Blob Data Transfer is only about 48 full downloads; use 80% of the allowance as the stop-and-ask threshold, roughly 38 full downloads/month unless the Vercel plan changes. Keeping more than four current-size DMGs in the active Blob store can approach the 1 GB storage allowance.
...
Download CTAs are intentionally unblocked in the canonical HTML. Their anchors point at the canonical public Vercel download URL, `https://sidestream-xi.vercel.app/api/download`, so local static previews and adjacent static hosts do not 404 on a relative `/api/download` path. The old `#download-email-gate` markup and `POST /api/download-lead` route remain for historical lead-capture compatibility, but visible download clicks no longer require an email before starting the installer.

The SaaS/account flow is server-owned. Google OAuth uses `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, optional `GOOGLE_REDIRECT_URI`, and `SIDESTREAM_BASE_URL`; successful sign-in stores an HTTP-only `sidestream_session` cookie backed by `public.sidestream_account_sessions`, but Google sign-in is no longer required before checkout. Stripe Checkout uses `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`; the first checkout creates the Basic subscription Product with tax code `txcd_10103100`, a `$4.99/month` USD default Price, and the `2026-02-25.preview` request version, then persists those Stripe resource IDs in `public.sidestream_billing_resources` for reuse. Billing resources are keyed separately for live and sandbox Stripe keys so Preview/test checkout cannot overwrite the live Product/Price pointer. Direct upgrade links call `GET /api/checkout/start`, which creates a hosted Checkout Session, sets `managed_payments[enabled]=true`, adds the Stripe-page custom text `Cancel anytime. Refund your last month at any time.`, and carries optional plugin activation metadata. Stripe-created customers are linked to existing Sidestream accounts by customer ID or normalized checkout email; otherwise the webhook creates a Stripe-first account row with no Google subject. Customer Portal sessions are still created from authenticated web sessions, and subscription access is granted only from verified webhook events. Plugin activation sessions are short-lived rows in `public.sidestream_activation_sessions`; the CEP panel receives an opaque 7-day license token from `/api/activation/status` and revalidates it through `/api/license/verify`. Do not store Stripe secrets, Google client secrets, raw payment data, or permanent paid-state in browser code or the CEP plugin.
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Beta" and says "Unlimited downloads for beta testers." The Unlimited plan is intentionally presented as coming soon: `.beta-coming` softly blurs and dims the paid `$19` plan details, while `.beta-overlay` keeps the visible "Coming soon" and "Sidestream Unlimited" message crisp and click/tap accessible.
...
- Scroll down to pricing and confirm both pricing cards begin animating before the section feels empty, with the Unlimited card following the Beta card by a slight stagger, both cards using visibly rounder 28px corners, and the Unlimited card using a white outline with no drop shadow.
- Confirm the Beta card says "Unlimited downloads for beta testers."
- Confirm the Unlimited card links directly to `/api/checkout/start`, includes the `Cancel anytime. Refund your last month at any time.` promise, and does not route users through the old Google-first upgrade interstitial.
...
- With Vercel dev and account env configured, confirm `/api/checkout/start` redirects directly to Stripe Checkout, `/upgrade.html?checkout=cancelled` renders the retry fallback, `/account.html` can still sign in with Google for Customer Portal access, `/api/stripe/webhook` accepts signed Stripe CLI events, and `/api/billing/portal` opens Customer Portal for a signed-in customer.
- Confirm `/api/activation/start` returns an activation URL, `/api/activation/status` stays pending before checkout, and returns a short-lived license token only after Stripe webhooks create an active license.
...
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card links to `/api/checkout/start`; price truth comes from the persisted Stripe Product/Price created by the server Checkout path, not hardcoded browser JavaScript. The Product ID identifies the subscription product, while the Price object owns the amount and recurrence. If the amount changes, create/switch to a new Price and update the persisted `stripe_price_id`; do not expect a Product ID to encode the amount. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
...
- Local account/billing testing also requires Vercel dev plus env vars: a server-only Postgres connection string (`SIDESTREAM_POSTGRES_URL` preferred, `POSTGRES_URL` fallback), `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `SIDESTREAM_BASE_URL`. `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` are only needed when testing the optional account/Customer Portal sign-in path. Use environment placeholders for Stripe keys and obtain real values from the Stripe Dashboard; rotate any secret key that has been pasted into chat before using live mode.
...
- Production, Preview, and Development `SIDESTREAM_INSTALLER_BLOB_PATHNAME` should resolve to the uploaded native/base `sidestream/1.0.9/Sidestream-1.0.9-Mac-Installer.dmg` artifact. The `Mac-ZXP-Installer.dmg` path is the retired ZXP-helper handoff and should not be used for the public website download.
...
- Added Managed Payments bootstrap for the Stripe subscription flow: Checkout now creates/reuses a persisted Basic subscription Product/default monthly Price through `public.sidestream_billing_resources`, uses the preview request version where required, and sets `managed_payments[enabled]=true`.
