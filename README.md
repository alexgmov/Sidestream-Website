...
- `api/_lib/paid-telemetry-handoff-repair.ts`, `scripts/reconcile-paid-telemetry-handoff.mjs`, and `scripts/replay-paid-telemetry-handoff.mjs` - One-acquisition, fail-closed Meta-paid telemetry repair. Starting only from the canonical acquisition UUID and trusted namespace, it supports the original direct/simple split, the single pending verified-account-review path, the narrower dual-active-path case where exactly one disjoint reviewed bridge selects the current activation while a direct-account path remains historical, and one exact legacy entitlement snapshot on that fully verified path. The legacy alternative requires null entitlement Product and Price, exact zero entitlement amount, a strictly positive verified paid amount, and the unchanged canonical payment/account/install/lifecycle/commerce/binding gates; a null claim email is accepted only behind exact account, entitlement, activation, and Checkout-email ownership. Duplicate or partial evidence still refuses before mutation. Apply is atomic/additive, never backfills those legacy fields, and exact-binding replay is a no-op.
...
- After changing the guarded paid-telemetry selector, run the default, pending-review, reviewed-path, and `npm run replay:paid-telemetry-handoff -- --expect-legacy-entitlement-repaired` disposable modes. The legacy replay must prove the exact null Product/Price plus zero entitlement amount against a strictly positive verified payment, exact reviewed ownership with a null claim email, `repair_ready`, atomic `already_repaired`, no-op replay, and unchanged entitlement/email placeholders. Partial snapshots, nonpositive payment, mismatched provider references or email, refund, and account/claim conflicts must still stop before identity reads.
