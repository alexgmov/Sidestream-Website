...
- `index.html` - Canonical page implementation served at `/`. Contains the shader mount root, header, hero, Windows waitlist capture, feature sections, pricing, final CTA, footer, styles, rotating-word script, toast behavior, crawler metadata, and structured data.
...
- Windows waitlist - `.windows-waitlist` and `[data-windows-waitlist-form]` live in the hero below the Mac download CTA, post valid emails to `POST /api/download-lead` with `source: "windows-waitlist"`, and store a local `sidestream.windows.email` value only to prefill the form on repeat visits.
...
Download CTAs are intentionally unblocked in the canonical HTML. Their anchors point at the canonical public Vercel download URL, `https://sidestream-xi.vercel.app/api/download`, so local static previews and adjacent static hosts do not 404 on a relative `/api/download` path. The old `#download-email-gate` markup remains for historical compatibility, but visible download clicks no longer require an email before starting the installer. The active visible lead capture is the hero Windows waitlist, which posts to `/api/download-lead` with `source: "windows-waitlist"` and should be counted through the `cta_source` column.
...
- Confirm the hero Windows waitlist sits below the Free Download CTA on desktop and mobile, validates invalid emails inline, submits valid emails to `/api/download-lead` with `source: "windows-waitlist"` under Vercel dev or deployment, and does not block the Mac download CTA.
...
- The Windows waitlist reuses `/api/download-lead` and stores its segment in `cta_source`; do not create a separate client-only endpoint or table unless the lead product needs a different data model.
