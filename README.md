...
- Confirm the decorative `.feature-corner-demo-video` keeps the visible Premiere panel's top-left corner within `1px` of `50vw 35vh` at multiple desktop window sizes, revealing more of the recording's lower edge while playing with `screen` blend and `0.9` opacity, keeping `.feature-glass` unmoved, and remaining hidden and paused at `900px` and below.
