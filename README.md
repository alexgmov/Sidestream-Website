...
- `components/ui/demo.tsx` - Adapted Paper demo component mounted as the page background. The active default effect keeps the original simple `MeshGradient` look with non-black stops darkened 20% to `#151515`, `#292929`, and `#a3a3a3`, with demo install/clipboard overlay text removed and movement-only fluid wake samples implemented through the active shader uniforms.
...
- `demos/sidestream-panel-corner.webm` - Cropped VP9-alpha WebM generated from the ProRes source `sidestream demo Linked Comp 01_2.mov`. Mounted as an opaque decorative top-left Premiere/Sidestream corner high on the right side of the hero/main page, scaled smaller than the feature cards.
...
- Shader background - `#shader-background-root`, `src/main.tsx`, `components/ui/demo.tsx`, the active `InteractiveMeshGradient`/`useFluidWake` implementation, `components/ui/background-paper-shaders.tsx`, and `src/paper-shaders-compat.d.ts`
...
```text
sidestream/1.0.5/Sidestream-1.0.5-Mac-Installer.dmg
```
...
The feature cards are chrome-free video frames that use native muted, looping MP4s from `demos/`. The active demos are `search demo.mp4` and `preview demo.mp4`, both recorded around the Tudor Place workflow. The Search and Preview feature sections sit inside `.feature-glass`, a full-bleed dark translucent band with heavy `backdrop-filter` blur that separates the demo proof area from the continuous shader without changing the individual `.shot` card treatment. `.feature-corner-demo` mounts `demos/sidestream-panel-corner.webm` as an opaque decorative VP9-alpha video high on the right side of the hero/main page, starts its left edge at `50vw`, stays roughly 20% smaller than the previous corner-demo pass, and keeps the plugin panel in full view. Its `<source>` is desktop-gated with `media="(min-width: 901px)"`, and the bottom inline playback helper pauses it whenever `.feature-corner-demo` is hidden. The `.feature-glass` band is pulled up behind the bottom of this demo with a matching `.feature-start` padding offset, so the frosted glass covers the lower edge without leaving a gap or dragging the Search content upward. The band spans the full feature wrapper vertically so the top and bottom separator lines have clear breathing room around the first and last demo videos. The Search and Preview feature copy blocks intentionally do not include inline download CTAs; each keeps the heading plus `.feature-subtext` as the centered copy block beside its demo video. They intentionally do not use the `autoplay` attribute; the bottom inline script uses `IntersectionObserver` to play each `.demo-video` only while it is visible and pause it when it leaves the viewport. On fine-pointer hover, the same script tilts the parent `.shot` from its midpoint with CSS variables capped at 15 degrees on X/Y and a tiny Z-axis twist, so the video frame reads as one subtle 3D plane. The hover math tracks against the card's untransformed layout box and resets with an S-curve transition to prevent corner-entry jitter. Raw Screen Studio project folders, ProRes `.mov` renders, and Premiere/After Effects project files should stay out of git; export compact MP4s or alpha WebMs for the site instead.

The page background should preserve the provided Paper demo's shader direction without keeping its demo-site UI. The canonical HTML keeps a black CSS fallback on `body`; `#shader-background-root` is a fixed full-viewport mount, and `src/main.tsx` renders the adapted `DemoOne` component from `components/ui/demo.tsx`. The demo's default `activeEffect` is `"mesh"`, so the visible background keeps the original simple black/charcoal/gray `MeshGradient` branch with non-black stops darkened 20% to `#151515`, `#292929`, and `#a3a3a3`. The active mesh branch uses `ShaderMount` with the Paper mesh fragment shader structure plus ten decaying `u_wake*` samples and paired `u_wakeMotion*` direction uniforms. The resting cursor state must not pull, swirl, or jiggle the mesh; only pointer movement should add a soft, direction-aware fluid wake with localized trail eddies, low-amplitude UV displacement, subtle same-shader monochrome shading, and no hard ring/line edges. Keep this inside the single shader canvas; do not add drawn ripple outlines, overlays, extra canvases, new colors, or CSS filters. Page text tokens use the off-white `#E2E8F0` and translucent off-white variants for contrast, while cards and pricing surfaces are dark translucent glass.
...
- Confirm the desktop background idles as the original simple Paper-style mesh with no cursor-following distortion when the pointer rests. Confirm dragging the mouse across the hero leaves a smooth, localized turbulent wake through the existing shader bands, like a finger moving through a stream. Confirm the background does not jiggle as one large sheet, the wake dissipates without harsh edges or line artifacts, this remains one visible canvas, and it does not block CTA hit targets.
...
- Confirm the `.feature-glass` backdrop spans the full x-axis behind the Search and Preview demo sections, blurs/darkens the shader behind it, covers the bottom of the decorative corner demo without a visible gap, and does not overlap the hero, pricing headline, or mobile content.
- Confirm the decorative `.feature-corner-demo-video` starts at the viewport midpoint on desktop, plays opaque as a roughly 20% smaller window high on the right side of the hero/main page with the plugin panel in view, and stays hidden and paused on mobile.
...
- The desktop movement wake lives inside `components/ui/demo.tsx` as `InteractiveMeshGradient` plus `useFluidWake`. Keep it as localized shader UV displacement plus subtle monochrome same-shader wake shading, not CSS blur/filter, particle decoration, drawn outlines, whole-background translation, resting-cursor pull, or a second visible overlay. It is intentionally gated to fine pointers, avoids a first-hover impulse, and is disabled under reduced motion.
...
- Desktop hero-to-feature spacing is tuned with `.hero-pad` filling `100svh`, bottom-aligning content, and using `padding: 112px 0 clamp(72px, 9vh, 104px)`. `.feature-glass` is pulled upward with `margin-top: -210px`, and `.feature-start` adds that same `210px` back into its top padding with `calc(clamp(96px, 12vh, 136px) + 210px)` so the frosted band can cover the bottom of the decorative corner demo without making a dead gap before the Search section. The mobile override uses `.hero-pad { padding: 128px 0 72px; }` and `.feature-start { padding-top: 84px; }`, with a narrower override of `64px`/`72px` at `520px`. Adjust those first-feature entries before changing shared `.sec-pad` rhythm, but keep the Search copy grid-centered beside its demo video.
- `.feature-corner-demo` is a non-interactive decorative layer after `#hero` and before `.feature-glass`. Keep its left edge at `50vw` if the alpha video's visible left edge should begin halfway across the page; keep `.feature-glass` at a higher z-index so the frosted band covers the bottom of the decorative demo. The `.feature-corner-demo-video` stays fully opaque, remains about 20% smaller than the previous corner-demo pass, and uses only brightness/contrast tuning to keep the dark Premiere UI readable on a dark shader background.
...
- Production, Preview, and Development `SIDESTREAM_INSTALLER_BLOB_PATHNAME` should resolve to the uploaded native/base `sidestream/1.0.5/Sidestream-1.0.5-Mac-Installer.dmg` artifact. The `Mac-ZXP-Installer.dmg` path is the retired ZXP-helper handoff and should not be used for the public website download.
...
- Made the cropped VP9-alpha Sidestream/Premiere corner demo about 20% smaller, kept it high/opaque on the right side of the hero/main page, and pulled the frosted feature band up so it covers the demo bottom without a visible gap.
...
- Reworked the shader interaction so the idle background returns to the original simple Paper-style mesh, while mouse movement lays down soft direction-aware fluid wake samples with no broad hover jiggle or hard ripple edges.
