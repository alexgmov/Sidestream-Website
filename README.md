...
- Production currently lacks `sidestream_checkout_intents`, `sidestream_licenses.entitlement_status`, and `sidestream_account_devices`. Activation checkout, fulfillment, status, verify, refresh, and download authorization must retain the explicit catalog-selected compatibility path until the hardened schema is actually qualified and deployed. Customer-facing reads must use `LICENSE_ENTITLEMENT_STATUS_SQL`; a direct `l.entitlement_status` reference fails at PostgreSQL parse time before fallback logic can run. Do not replace this with runtime DDL, a manual table or column addition, or an unreviewed migration. The compatibility expression recognizes only exact attached one-time paid rows and the legacy credential path remains device-hash bound.
...
- The shipped v1.0.14 panel validates every `restoreUrl` as `/api/activation/claim`, even after an Upgrade click. Keep the query-conditioned `upgrade=1` rewrite to `/api/checkout/start` so new Upgrade clicks bypass the claim function and redundant Postgres classification without breaking that client validator. The marker is routing only, not authorization; Checkout still validates the activation capability and payment state. Claim URLs without the marker must retain the database fallback for already-saved activations.
...
- 2026-07-20: Restored and streamlined the shipped v1.0.14 Upgrade path in Production without deploying Customer 360. Activation-bearing Checkout GETs resume or create one idempotent attached Stripe Session; v1.0.14 keeps its required activation-claim `restoreUrl`, while new Upgrade URLs use an internal Vercel rewrite to Checkout that avoids the claim function and redundant Neon lookup. Older saved Upgrade claims retain the database-classified fallback. The existing Neon database received only the recognized legacy account/billing migrations through activation Checkout attachment and refresh rotation, while checkout-intent, entitlement-lifecycle, device, and Customer 360 migrations remain unapplied. Production has the required `SIDESTREAM_POSTGRES_URL` binding to that same pooled Neon target. Vercel per-function TypeScript compatibility fixes are runtime-neutral and keep the provider build deployable.
