...
The project is linked to Vercel project `alex-3685s-projects/sidestream`. `.vercel/`, `.env.local`, and other `.env*` files are ignored. The installer download pointer should be updated by changing the `SIDESTREAM_INSTALLER_BLOB_PATHNAME` Vercel env var after publishing a new private Blob artifact.
...
- Confirm moving or hovering the mouse across the desktop hero smoothly bends/pushes the existing shader bands like a broad current while the idle frame still reads like the same Paper mesh background. Confirm there are no harsh ring/line artifacts, this remains one visible canvas, and it does not block CTA hit targets.
...
- The desktop hover current lives inside `components/ui/demo.tsx` as `InteractiveMeshGradient` plus `usePointerCurrents`. Keep it as shader UV displacement, not CSS blur/filter, particle decoration, or a second visible overlay. It is intentionally gated to fine pointers, avoids a first-hover impulse, and is disabled under reduced motion.
...
- Replaced the initial fine-pointer water-wake with a smoother hover-controlled curl-current field in the active Paper-style mesh shader, preserving the same colors, timing, and single-canvas mount while avoiding harsh visible ripple lines.
