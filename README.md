...
- 2026-07-22: Applied and read-only verified `20260713203000_add_checkout_intents.sql` in Production while preserving the direct shipped-panel Stripe Upgrade candidate. The visible claim/confirmation regression came from a later referral-tracking worktree that omitted the direct-handoff lineage, so future deploys must retain both that chain and the current ManyChat/Instagram/Meta routes. The earlier device schema remains verified; entitlement-lifecycle and Customer 360 remain unapplied, and no repaired application artifact or Production alias was deployed.
