...
- `thank-you.html` - Minimal noindex Checkout success page. Stripe success URLs land here after purchase and tell buyers how to reconnect Sidestream to the paid account.
...
- `api/referral-visit.ts` and `api/_lib/referral-visits.ts` - First-party landing-referral attribution for the unchanged `https://sidestream.tv/m` and `/m/` ManyChat aliases. A real canonical-page load posts the allowlisted source, receives `204` without waiting for storage, and writes at most one private Blob record per anonymous request fingerprint/day/classification. Daily HMACs reuse the installer-analytics secret; raw IP and user-agent values are never stored.
- `middleware.ts`, `mobile-paid-prototype.html`, and `api/paid-acquisition/landing.ts` - Exact top-level `GET /mc` paid-acquisition router and deterministic noindex paid landing. Only conservatively eligible phones may receive a signed sticky 50/50 assignment. Missing, short, or invalid `SIDESTREAM_PAID_ACQUISITION_ASSIGNMENT_SECRET` redirects to the unchanged ManyChat root fallback without setting a cohort cookie or creating paid state. The root HTML path is a symlink to the traced handler asset under `api/paid-acquisition/`, so the Production guard's deployable-root allowlist remains unchanged.
- `api/_lib/paid-acquisition.ts`, `api/paid-acquisition/{checkout,artifact,claim}.ts`, and `db/migrations/20260727010000_add_paid_acquisition_experiment.sql` - Namespaced paid-entry, Checkout, receipt, claim, activation, lifecycle, and privacy-limited event state. Browser input never selects commerce truth; paid fulfillment requires the existing Stripe verifier plus exact paid-experiment USD 999 facts.
- `api/_lib/paid-installer-email.ts`, `api/_lib/paid-release-manifest.ts`, `api/paid-download.ts`, `api/releases/paid-latest.ts`, and `data/release-manifest.paid*.json` - Paid-only exact-one email outbox/message contract and receipt-gated, platform-exact artifact surfaces. Provider sending is opt-in only when `SIDESTREAM_PAID_ACQUISITION_EMAIL_ENABLED=1`; committed paid manifests are fixtures, not publication or Blob-existence proof.
- `docs/paid-acquisition-contract.md` and `docs/paid-acquisition-runbook.md` - Durable paid experiment invariants, endpoint/data/privacy contracts, deterministic verification, and human-gated Test-only procedure. They authorize no Production deployment, migration, provider contact, environment change, or artifact publication.
...
- `api/checkout/start.ts` and `api/checkout/complete.ts` - Authenticated one-time Sidestream Pro Checkout flow. Start sends signed-out users through Google authentication, creates a locked database intent for the signed-in Free account, creates or reuses the Stripe Session, and redirects to Stripe. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. A complete zero-total Stripe order may omit its PaymentIntent while reporting either `paid` or `no_payment_required`; fulfillment accepts that shape only when the exact Session/Price/Product/activation checks pass, the amount is exactly zero, and the currency is valid.
...
- `api/activation/start.ts`, `api/activation/status.ts`, and `api/activation/claim.ts` - CEP-facing activation plus authenticated account routing. A Free account continues to Checkout; restore or transfer requires an active-license session, same-origin CSRF-protected POST, and an explicit prior-device deactivation confirmation for a move.
...
- Landing referral attribution and paid `/mc` isolation - `/m` and `/m/` retain their temporary canonical-root redirect with `utm_source=manychat`; the loaded root page POSTs that allowlisted source to `/api/referral-visit`. Exact top-level mobile `GET /mc` is evaluated by `middleware.ts`; control, ineligible, uncertain, `HEAD`, and missing/invalid-secret requests use the same root fallback, while only a valid signed paid cohort reaches the internal paid landing. `/mc/` is not normalized into the experiment. Referral visitor-days, cohort assignment, paid Checkout, email acceptance, artifact requests, installs, activations, and entitlements remain separate stages.
...
- Account/auth/billing/device entitlement - `account.html`, `thank-you.html`, `api/_lib/account.ts`, `api/_lib/entitlement.ts`, `api/_lib/device-policy.ts`, `api/_lib/license-environment.ts`, `api/auth/*`, `api/checkout/*`, `api/billing/*`, `api/stripe/webhook.ts`, `api/activation/*`, `api/account/device.ts`, and `api/license/*` own Google account management, the server-owned $9.99 one-time Sidestream Pro Product/Price, Checkout intents, namespace-separated active-device rows, restricted Test isolation, refund/dispute lifecycle, confirmed transfers, download authorization, deactivation, and device-bound access/refresh credentials. Device mismatch policy defaults to `observe`; only explicit `enforce` blocks, and Customer 360 does not change that mode. The API/operator contract is `docs/api-hardening-runbook.md`; device/support details are in `docs/single-device-entitlements.md`.
...
The unchanged first-party ManyChat referral aliases are:
...
They return a temporary redirect to `https://sidestream.tv/?utm_source=manychat`, then the loaded root page records a privacy-limited daily visitor in the `manychat` bucket.

Exact top-level `GET /mc` is the isolated paid-acquisition experiment entry. `middleware.ts` conservatively rechecks phone eligibility and a signed first-party assignment. Control, ineligible, uncertain, `HEAD`, and missing/invalid-assignment-secret requests redirect to the same unchanged ManyChat root destination without a cohort cookie, database write, Checkout, email, artifact, activation, or entitlement. Only a valid paid cohort is internally rewritten to `/api/paid-acquisition/landing`; `/mc/`, case variants, and encoded lookalikes are not experiment entries.
...
| `/api/paid-acquisition/landing` | `GET` | Internal middleware rewrite only; validates the signed assignment/proof, persists the paid entry, and renders the deterministic noindex landing |
| `/api/paid-acquisition/checkout` | `POST` | Same-origin exact JSON entry/idempotency proof creates or reuses the anonymous locked intent and returns one verified HTTPS Stripe URL |
| `/api/paid-acquisition/artifact` | `GET` | Valid environment-bound paid receipt selects one exact platform and returns a short-lived paid download handoff |
| `/api/paid-acquisition/claim` | `GET` | Receipt-bound Google authentication and exact verified-email claim/recovery outcome; it never trusts a browser-supplied email |
| `/api/paid-download` | `GET`, `HEAD` | Platform-exact paid artifact metadata or signed private-Blob redirect; no omitted-platform default or public pathname |
| `/api/releases/paid-latest` | `GET`, `HEAD` | Platform-exact no-store public paid manifest without receipt, email, or private Blob fields |
...
`GET /api/checkout/start` owns that sequence. It sends signed-out users to Google with an allowlisted return path. For a signed-in Free account, it applies the account/IP rate limit, creates a 24-hour database intent, invokes the locked/idempotent Stripe worker, persists the exact Session/Price/Product, and returns `303` to Stripe. Checkout uses one `mode=payment` card Session for one quantity of the exact Sidestream Pro Product/Price. The success callback re-fetches the literal `{CHECKOUT_SESSION_ID}` and verifies payment, line item, quantity, Price, Product, activation metadata, and attachment before fulfillment. Stripe may represent a complete zero-total order as `payment_status=paid` or `payment_status=no_payment_required` with no PaymentIntent; both are valid only when the canonical total is exactly zero and every other checkout verification succeeds.

The `/mc` experiment does not alter that authenticated Upgrade contract: the root/account Upgrade remains Upgrade → Google authentication → direct `303` to Stripe from `GET /api/checkout/start`, with no Sidestream confirmation, landing, upgrade, or other intermediate page. The paid experiment's anonymous Checkout endpoint is namespaced under `/api/paid-acquisition/checkout` and is reachable only from a server-validated paid `/mc` entry.
...
`/api/license/verify` and `/api/license/refresh` return 401 codes `invalid_token`, `revoked`, `device_mismatch`, `device_replaced`, or `device_deactivated`, and 403 `license_inactive`; callers retain credentials on transient 5xx failures. `/api/activation/claim` authenticates first. A signed-in Free account continues to `/api/checkout/start`; an active owner may use the no-store restore or transfer decision and its same-origin, CSRF-valid POST to CAS-bind a fresh activation whose account is still null or identical. An active Pro owner is routed to claim/account instead of starting another purchase. Do not store Stripe secrets, Google client secrets, raw payment data, activation keys, license tokens, refresh tokens, or permanent paid-state in browser code or logs.
...
| Checkout intent | Database intent TTL is 24 hours; Product/Price selection uses `SIDESTREAM_PRO_PRODUCT_ID`, `SIDESTREAM_PRO_PRICE_ID`, and compatible `SIDESTREAM_UNLIMITED_PRICE_ID` |
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Free" and says "5 free downloads every day." The Pro plan is a visible `$9.99 once` one-time upgrade that opens `/api/checkout/start`, authenticates the user, and opens Stripe Checkout.
...
To report ManyChat landing visitor-days from the private Blob store, use an ignored environment file with Blob credentials:
...
The report separates likely-human and likely-scanner daily visitor counts. It does not identify people or prove downloads, installs, activations, or purchases.
...
Run the deterministic paid-acquisition proof without contacting Stripe, Resend,
Blob, Google, Vercel, or a live database:

```bash
npm run test:migrations
npm run test:paid-acquisition-e2e
node scripts/verify-paid-acquisition-e2e-fixtures.mjs
node --test tests/paid-experiment-routing.test.mjs tests/paid-landing.test.mjs tests/referral-visits.test.mjs
node --experimental-strip-types --test tests/paid-acquisition-integration.test.mjs tests/paid-acquisition-checkout.test.mjs tests/paid-installer-email.test.mjs tests/paid-release-manifest.test.mjs
```

These are fixture and source-contract checks only. They do not apply the paid
migration, contact a provider, publish or validate a real artifact, change an
environment variable, deploy, prove an inbox delivery, or prove a loaded panel.
...
- Run `npm run test:referral-visits` after changing `/m`, `/mc`, `/api/referral-visit`, the ManyChat browser hook, private-Blob referral storage, or its report. It verifies that `/m` and `/m/` remain hard redirects, `/mc` and `/mc/` are absent from `vercel.json`, plus the allowlisted source, bounded request body, response-before-storage behavior, daily anonymous dedupe inputs, scanner separation, and hash-free aggregate output.
- Run the paid-acquisition command block above after changing the exact `/mc` matcher, assignment cookie/proof, landing, paid Checkout, completion, email, receipt/artifact, claim, activation outcome, lifecycle, paid manifest, or namespaced migration. Confirm a missing/invalid assignment secret produces only the unchanged ManyChat root fallback with no cohort cookie or paid side effect, email provider delivery remains exact-`1` opt-in, and ordinary Upgrade still passes `npm run verify:checkout-contract`.
...
- Confirm the Pro card says `$9.99 once` and links to `/api/checkout/start`. Verify the browser sequence is Upgrade button, Google authentication, Stripe payment.
...
- `/mc` is middleware-owned only for exact top-level `GET`; do not restore `/mc` or `/mc/` redirects in `vercel.json`. `/m` and `/m/` remain hard redirects. The paid path is default-off: an absent, short, or invalid assignment secret must redirect to the unchanged ManyChat root destination without setting a cohort cookie or touching paid database, Checkout, email, artifact, activation, or entitlement state.
- `mobile-paid-prototype.html` is intentionally a root symlink to `api/paid-acquisition/mobile-paid-prototype.html`. The handler reads the traced colocated asset, while the existing Production source guard continues to reject any new regular root HTML file. Regenerate through `node scripts/build-paid-landing.mjs`; do not replace the symlink with a regular file.
...
- Mac and Windows download CTAs use `[data-download]` and `[data-windows-download]` button overrides that win over primary/secondary button classes: white capsule backgrounds, black platform marks and text, and red hover fills with white text while preserving existing sizing. The desktop hero and Free pricing card use matching visible `Download` labels with platform-specific accessible labels and URLs; the Free card’s platform pair is hidden at `900px` and below because the hero email handoff already sends both installers. The final CTA remains a Mac-only `Free Download`, and the header intentionally has no download CTA. All visible Mac download CTAs should point at `https://sidestream.tv/api/download`, while Windows CTAs use `?platform=win32-x64`, unless the fulfillment host intentionally changes. Do not reintroduce the email modal as a blocking step without explicitly revisiting the unblocked installer strategy.
...
- 2026-07-27: Reconciled the independently audited dormant paid `/mc` experiment onto current Production main without changing the ordinary Upgrade → Google authentication → direct Stripe contract. `/m` and `/m/` remain unchanged; exact eligible mobile `GET /mc` is middleware-owned and default-off on missing/invalid assignment secret; paid Checkout, email, receipt/artifact, claim/activation, lifecycle, manifests, migration, fixtures, tests, and Test-only documentation remain namespaced. The disposable activation-security harness now loads the integrated paid-acquisition helper explicitly. No migration, provider call, environment change, artifact publication, or deployment was performed.
- 2026-07-27: Restored `/m` and `/m/` as temporary ManyChat aliases and briefly restored equivalent `/mc` redirects. The paid-experiment reconciliation above supersedes only the `/mc` redirect portion; `/m` and `/m/` remain unchanged.
