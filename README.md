...
- `thank-you.html` - Minimal noindex Checkout success page. Stripe success URLs land here after purchase; the page omits the activation-delay and return-to-Premiere messages while retaining account reconnect, device-move, receipt, and installer guidance.
...
- Run `npm run test:referral-visits` after changing `/mc`, `/api/referral-visit`, the ManyChat browser hook, private-Blob referral storage, or its report. It verifies both route forms, the allowlisted source, bounded request body, response-before-storage behavior, daily anonymous dedupe inputs, scanner separation, and hash-free aggregate output.
...
- 2026-07-26: Removed the thank-you page's activation-delay and return-to-Premiere messages while preserving the remaining account reconnect, device-move, receipt, installer, and download actions.
