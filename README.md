...
At `520px` and below, the final phone override supersedes the earlier crop values with a compact `clamp(260px, 32vh, 300px)` wrapper, a `min(590px, 124vw)` video, and a `60px` wrapper lift. Its WebKit-compatible mask now fades in across the top `8%` before retaining the existing lower fade, blending the alpha WebM's hard Premiere-window boundary into the shader instead of moving that line to a new clipping row. The headline and inline mobile handoff rise `88px` over the recording's dark lower fade, while `48px` bottom padding keeps the full email card visible at `390x844` with no horizontal overflow.
