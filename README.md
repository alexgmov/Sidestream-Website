...
- `docs/api-hardening-runbook.md` - Sole production migration/cutover procedure and exact hardened API/release contract, shared Postgres and migration model, Stripe/lead/maintenance operations, bounded configuration, metrics and alerts, required webhook events, ordered human gates, and forward-compatible rollback. It does not claim production was changed.
...
`scripts/apply-postgres-migrations.mjs` owns an advisory-locked SHA-256 ledger in `public.sidestream_schema_migrations`. `--status` and `--validate` prove the chain; a non-empty legacy schema requires a verified explicit `--baseline`; applying commits each pending SQL file and ledger row together. Runtime handlers never create or alter schema. The final migration removes the redundant unique `lead_key` constraint while preserving canonical `(email, cta_source)` uniqueness and a non-unique lookup index.
