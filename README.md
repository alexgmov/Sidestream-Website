...
- `upgrade.html` - Minimal noindex upgrade page opened from the CEP panel or pricing card. Requires Google sign-in, starts Stripe Checkout for the configured subscription price, and preserves an optional plugin activation key through checkout.
...
- `api/checkout/create.ts` - Authenticated Stripe Checkout Session creator for the Sidestream subscription. It uses the persisted Basic subscription Stripe Product/Price and enables Managed Payments on the Checkout Session. The subscription is not fulfilled until Stripe webhooks update the backend license record.
...
- Download and upgrade actions - `[data-download]`, `[data-purchase]`, and `#toast`; download CTAs point directly at the public `https://sidestream-xi.vercel.app/api/download` URL with the CSS Apple platform mark from `.btn[data-download]::before`, while Sidestream Unlimited entry points open `/upgrade.html`
...
- Account/auth/billing - `account.html`, `upgrade.html`, `api/_lib/account.ts`, `api/auth/*`, `api/checkout/create.ts`, `api/billing/portal.ts`, `api/stripe/webhook.ts`, `api/activation/*`, and `api/license/verify.ts` own Google sign-in, Stripe Product/Price bootstrap, Managed Payments Checkout, Customer Portal redirects, webhook-owned entitlements, plugin activation, and license-token verification
...
- Confirm the Unlimited card reads as a coming-soon placeholder: the `$19` paid details are commented out in the HTML, the overlay text says "Coming soon" and "Sidestream Unlimited", the overlay remains readable on mobile, and clicking/tapping it shows the Sidestream Unlimited coming-soon toast instead of checkout language.
...
- With Vercel dev and account env configured, confirm `/account.html` signs in with Google, `/upgrade.html` creates a Stripe Checkout Session, `/api/stripe/webhook` accepts signed Stripe CLI events, and `/api/billing/portal` opens Customer Portal for a signed-in customer.
...
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card links to `/upgrade.html`; price truth comes from the persisted Stripe Product/Price created by the server Checkout path, not hardcoded browser JavaScript. The Product ID identifies the subscription product, while the Price object owns the `$4.99/month` amount and recurrence. If the amount changes, create/switch to a new Price and update the persisted `stripe_price_id`; do not expect a Product ID to encode the amount. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
