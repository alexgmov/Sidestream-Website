...
- `config/upgrade-pricing-experiment.mjs`, `api/_lib/upgrade-pricing-experiment.ts`, `db/migrations/20260812120000_add_upgrade_pricing_experiment.sql`, `db/migrations/20260825140000_add_annual_upgrade_pricing_experiment.sql`, and `docs/upgrade-pricing-experiment.md` - Authenticated Upgrade pricing contracts. Historical `upgrade-pricing-v1` remains closed and immutable. Dormant-by-default `upgrade-pricing-v2` assigns eligible new global-USD accounts to `$19.99` one-time or `$19.99/year`, while every v1 assignment keeps its original one-time/monthly terms and regional traffic stays on the unchanged one-time fallback outside the v2 cohort.
...
- Authenticated Upgrade assignment is account-permanent, not browser-sticky. Historical `upgrade-pricing-v1` ended on 2026-08-21; its assignments, `$4.99/month` `monthly_half` terms, open Sessions, subscriptions, entitlements, and audit history remain immutable and supported. New `upgrade-pricing-v2` uses a separate assignment version and compares the same initial global USD price: `$19.99 once` versus `$19.99/year`. It is source-defaulted to disabled with rollout `0`, uses separate `SIDESTREAM_UPGRADE_PRICING_V2_*` settings so stale v1 environment values cannot start it, excludes every account already assigned to v1, and keeps all regional offers on their existing one-time fallback. The annual Checkout says it renews automatically, promises an email 30 days before renewal, links cancellation to the Sidestream account, and confirms access through the paid year. Its six-hour reminder job stages only active v2 annual subscriptions 7-30 days before renewal, skips scheduled cancellations, and uses a durable idempotent retry/dead-letter ledger. Do not activate v2 until the migration, exact live annual Stripe Price, recurring webhook events, Test lifecycle, actual reminder delivery, and one-time control are separately verified; then move from rollout `0` to a canary before `5000`. See `docs/upgrade-pricing-experiment.md` before changing assignment, Price, disclosure, reminder, webhook, or lifecycle behavior.
