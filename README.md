...
| Credit wallet | `SIDESTREAM_DOWNLOAD_CREDITS_ENABLED=1` activates the routes only after the migration gate. The starter grant and video/audio costs are fixed at 1,000 and 100/100. `SIDESTREAM_CREDIT_PACK_PRICE_ID` must identify an active one-time USD $4.99 Price and `SIDESTREAM_CREDIT_PACK_CREDITS` must equal `1000`; the browser label is fixed to `1,000 more credits` and `$4.99 one-time`, so environment text cannot change the approved offer. Missing/malformed settings keep purchases unavailable. The live catalog entry is Product `prod_V8htjvzfhobt5j` with Price `price_1U8QTYDFKjeGlioXdrQNTHNf`; the Price and credit count are staged in the protected Hetzner runtime, but `SIDESTREAM_DOWNLOAD_CREDITS_ENABLED` remains absent. Production activation still requires the separately reviewed refund/dispute adjustment policy and qualification. |
