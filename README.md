...
- `api/activation/start.ts`, `api/activation/status.ts`, and `api/activation/claim.ts` - CEP-facing activation plus the authenticated restore/transfer/purchase decision surface. Shipped v1.0.14 panels require `restoreUrl` to remain the activation-claim route for every handoff. Claim GET sends known, still-pending `download_history` and `results_quota` Upgrade activations to the activation-bearing Checkout route before account authentication, while `settings_account` remains on the authenticated claim route. Non-Upgrade claim GET is read-only; restore or transfer requires an active-license session, same-origin CSRF-protected POST, and an explicit prior-device deactivation confirmation for a move.
...
- 2026-07-20: Restored the shipped v1.0.14 Upgrade path in Production without deploying Customer 360. Activation-bearing Checkout GETs now resume or create their one idempotent attached Stripe Session; v1.0.14 keeps its required activation-claim `restoreUrl`, and pending Upgrade claims recover to Checkout before account authentication. The existing Neon database received only the recognized legacy account/billing migrations through activation Checkout attachment and refresh rotation, while checkout-intent, entitlement-lifecycle, device, and Customer 360 migrations remain unapplied. Production now has the required `SIDESTREAM_POSTGRES_URL` binding to that same pooled Neon target. The deployed public path returned `200` for a v1.0.14 activation and `303` from its Upgrade URL to live Stripe Checkout. Vercel per-function TypeScript compatibility fixes are runtime-neutral and keep the provider build deployable.
