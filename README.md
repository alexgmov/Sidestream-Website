...
- Historical lifecycle reconciliation is a separate pre-cutover blocker. The repository has no tested idempotent tool that can find and repair refund/dispute events terminalized by the known-bad mapper. A provisional full scan must retain an inclusive high-watermark and exact-ID manifest, but it cannot close the gate while old code still accepts events. After the maintenance deny is active and old in-flight writes drain, the tool must run a final canonical full/delta reconciliation covering every lifecycle ID through the deny boundary, including pre-drain and transition IDs, then prove canonical entitlement state, resulting watermarks, and an idempotent no-op rerun before reopening. Production remains blocked until that two-phase proof exists, or the same final post-deny evidence proves no row was affected. Manual ledger or entitlement edits are forbidden.
