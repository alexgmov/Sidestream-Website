...
`deploy:production` fails unless the worktree is clean, `HEAD` exactly matches the live remote `origin/codex/release-1.0.14` head, and `.vercel/project.json` identifies the pinned Sidestream project/team. It then creates a fresh Production build, rejects a bundle without both product waitlist CTAs, modals, browser routes, or server functions, and only then runs the prebuilt Production deployment. Afterward, still inspect `sidestream.tv`, confirm the active alias/deployment ID, open each rendered waitlist modal, verify both waitlist `GET` routes return `405 Allow: POST`, and use a clearly synthetic POST plus masked private-Blob readback for any capture path that changed.
...
- Run `node scripts/assert-no-runtime-ddl.mjs` and `node scripts/validate-vercel-contract.mjs` after API/migration/routing work. For a human Vercel build, follow `npx vercel@latest build` with `npm run verify:vercel-build`; the verifier requires both product waitlist functions and each CTA/modal/browser-route marker set. For Production, use only `npm run deploy:production`, which adds clean canonical-source and pinned-project checks before building.
