...
- `upgrade.html` - Minimal noindex checkout/cancel fallback page. Main upgrade entry points open the server-owned `/api/checkout/start` transition boundary; it displays one sentence with no action controls and automatically submits the signed purchase intent. Activation-bearing recovery enters the authenticated claim page so the account can safely choose same-device reconnect, confirmed transfer, or purchase without a duplicate charge.
...
- `api/auth/google/start.ts` and `api/auth/google/callback.ts` - Google OAuth redirect/callback handlers. They require the configured callback to share the browser-facing start origin before setting short-lived HTTP-only state and return-intent cookies, upsert `sidestream_accounts`, issue a server-side session cookie, and render a retryable noindex HTML error instead of raw JSON when sign-in state is stale. A current plugin activation or text-only Checkout transition may carry one server-owned purchase capability through Google; the callback account-binds that capability and enters the same locked and rate-limited Checkout worker without rendering a confirmation page. An existing valid session skips Google and continues through the signed auto-submit transition.
...
- `api/checkout/start.ts`, `public/checkout-transition.js`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - Intentional one-time Sidestream Pro Checkout flow. GET start creates/resumes only a signed database intent and renders the exact text-only transition with no visible button; a same-origin external script submits the hidden signed POST without relying on an inline CSP nonce. Create sends signed-out buyers through Google with the opaque Checkout capability, then the authenticated callback or existing session enters one locked/idempotent worker and persists the exact Price/Product. Generic GETs, link previews, and arbitrary OAuth `next` values still cannot create Stripe resources. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. The old Vercel host still fails closed when legacy activation context is missing.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers, while viewports at or below `900px` replace the hero platform choice with the email form and route lower-page download taps back to that form. Website Sidestream Pro entry points open the no-store `/api/checkout/start` text transition, which has no action controls and automatically submits the signed same-origin POST. Google authentication runs only when the buyer has no valid account session, then the server redirects directly to Stripe.
...
| `/api/checkout/start` | `GET` | `200` no-store text-only transition plus a hidden signed auto-submit form; no visible button and no Stripe write |
...
Website purchase requires a Sidestream account, but never asks for a second purchase confirmation. `GET /api/checkout/start` creates or resumes only a 24-hour database intent and renders the single sentence `Continue with authentication with Google if you haven't already.` with no visible action controls. Its hidden form automatically submits a signed same-origin token valid for 10 minutes. If the buyer has no session, `POST /api/checkout/create` carries only the opaque intent capability through Google OAuth; the callback enforces its account binding before Stripe. An existing session skips Google. The authenticated path is bounded to 8 requests per intent and 20 per IP per 15 minutes, locks and reuses the intent, then creates one `mode=payment` card Session for one quantity of the exact Sidestream Pro Product/Price.

A current plugin activation with explicit request source `plugin` and a parseable version newer than `1.0.13` receives a different `upgradeUrl`: `/api/auth/google/start?plugin_upgrade=<signed-capability>`. The HMAC capability binds the exact activation key and expires with the 10-minute OAuth window. OAuth start validates it first. A signed-out buyer carries it in an HTTP-only return-intent cookie through Google, and only the callback with matching OAuth state may consume it. A buyer with an existing valid session skips Google and enters the same text-only signed auto-submit transition. An active Pro owner returns to the activation claim branch; a Free account enters the same 8-per-intent, 20-per-IP limiter plus locked/idempotent Session worker used by `POST /api/checkout/create`. Missing, malformed, tampered, expired, legacy-client, or non-plugin intent cannot reach Stripe. Arbitrary OAuth `next` values still collapse to the account or restore allowlist, and `GET /api/checkout/start` remains non-creating. The success callback re-fetches the literal `{CHECKOUT_SESSION_ID}` and verifies payment, line item, quantity, Price, Product, activation metadata, and attachment before fulfillment.
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Free" and says "5 free downloads every day." The Pro plan is a visible `$9.99 once` one-time upgrade that opens `/api/checkout/start`; the transition has no confirmation control, uses Google only when authentication is missing, and then redirects to Stripe.
...
- Confirm the Pro card says `$9.99 once` and links to `/api/checkout/start`. The no-store transition must show only `Continue with authentication with Google if you haven't already.`, contain no visible button, make no Stripe write on GET, auto-submit its hidden signed form, use Google only when authentication is missing, and then redirect directly to Stripe.
...
- With Vercel dev and account env configured, confirm `GET /api/checkout/start` shows the exact one-sentence transition with no visible button and makes no Stripe write. Confirm the hidden signed same-origin form auto-submits, signed-out buyers complete Google OAuth before the state-verified callback enters Stripe, and an existing valid session skips Google and reaches Stripe directly. Separately start a current `source=plugin` activation and confirm the same signed-out and signed-in branches reach the one attached Stripe Test Checkout Session without a purchase-confirmation control. Tamper with, omit, or expire either capability and confirm no Checkout worker call occurs; repeat with a non-plugin source and a 1.0.13 client and confirm they retain the text-only transition. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, cancelled Checkout resumes/rotates intentionally, `/account.html` can still sign in with Google, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
...
- 2026-07-20: Removed the internal purchase-confirmation control. `/api/checkout/start` now renders only `Continue with authentication with Google if you haven't already.` plus a hidden signed auto-submit form; its same-origin external script avoids the inline CSP nonce failure that could strand browsers on the transition. Signed-out buyers carry the opaque intent through state-verified Google OAuth, existing sessions skip Google, and both enter the same locked/rate-limited Stripe worker. Restore/transfer controls remain separate. Added focused no-button, OAuth-resume, account-binding, browser-transition, and abuse regressions. Only the isolated Preview/Test surface may be deployed; Production remains untouched.
