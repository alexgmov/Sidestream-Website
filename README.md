...
- `api/download.ts` - Vercel Node Function for installer fulfillment. `HEAD` returns attachment metadata for the manifest-configured private Vercel Blob installer, and `GET` validates the Blob then redirects to a short-lived signed private Blob URL. Bare requests remain Mac; `?platform=win32-x64` selects the Windows beta artifact. Supports `GET` and `HEAD` only.
...
- Header/nav - `header`, `.nav`, `.brand`, `.nav-links`; the desktop header exposes Features, Pricing, and Account without a download CTA
...
- Windows download - `[data-windows-download]` lives beside the hero `[data-download]` `Download` CTA as a matching white platform pill with a Windows mark and links directly to `https://sidestream.tv/api/download?platform=win32-x64`. The Mac control keeps `aria-label="Download Sidestream for Mac"` because its shorter visible label relies on the Apple mark for platform context. The old `#windows-waitlist-gate` markup and capture code remain dormant for historical compatibility, with no active page trigger.
...
The header is a fixed transparent overlay with no scroll divider so the shader remains uninterrupted behind the nav. The `.hero-pad` section fills the first viewport and aligns the hero headline, description, and primary Mac `Download` CTA to the lower-left first-fold gutter. The Sidestream wordmark and hero copy share the viewport-left `24px` first-fold gutter, and the Features/Pricing/Account navigation cluster is absolutely anchored to the viewport's top-right corner with a `15px` top offset and matching `24px` right gutter. The header intentionally has no download CTA; downloads remain available in the hero, pricing, and final CTA sections.
...
- Confirm the Sidestream wordmark and desktop hero copy share the viewport-left `24px` first-fold gutter, and the Features/Pricing/Account header cluster sits at the viewport's top-right with a `15px` top offset and `24px` right gutter, with no header download CTA.
