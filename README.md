...
- `docs/customer-360.md` - Durable cross-repo Customer 360 contract: exact private list/detail fields and nullability, trusted runtime database selection versus authorized admin row selection, identity/merge and observe-by-default single-device separation, currency-partitioned money and purchase-history semantics in minor units, exhaustive stored/derivable usage versus compact API exposure, privacy/retention, observability, disposable tests, dry-run backfill, rollback, and the only human-gated Preview/Test-first rollout. No accepted Preview or Production release exists, and Production is not migrated.
...
- `upgrade.html` - Minimal noindex checkout/cancel fallback page. Its visible purchase control submits a same-origin POST to `/api/checkout/start`; the server then redirects through Google only when authentication is missing and otherwise goes straight to Stripe. No intermediate Checkout page is rendered. Activation-bearing recovery enters the authenticated claim page so the account can safely choose same-device reconnect, confirmed transfer, or purchase without a duplicate charge.
...
- `api/auth/google/start.ts` and `api/auth/google/callback.ts` - Google OAuth redirect/callback handlers. They require the configured callback to share the browser-facing start origin before setting short-lived HTTP-only state and return-intent cookies, upsert `sidestream_accounts`, issue a server-side session cookie, and render a retryable noindex HTML error instead of raw JSON when sign-in state is stale. A current plugin activation or Checkout entry may carry one server-owned purchase capability through Google; the callback account-binds that capability and enters the same locked and rate-limited Checkout worker. An existing valid session skips Google and enters that worker immediately. Neither happy path renders an intermediate Checkout page.
...
- `api/checkout/start.ts`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - Intentional one-time Sidestream Pro Checkout flow. The website uses a same-origin start POST, while activation and cancellation GETs must carry an opaque server-created capability. Start creates or resumes only the database intent and responds with a server redirect to Google auth; a bare GET returns to `upgrade.html`. Google callback or an existing session enters one locked/idempotent worker and persists the exact Price/Product before redirecting to Stripe. Every authenticated intent, including plugin activations, passes the account's validated Stripe Customer so hosted Checkout prefills the account email and displays `Signed in to Sidestream as <email>. One-time payment. No subscription.` beside the payment action. No happy-path transition document or client-side auto-submit script exists. Generic GETs, link previews, and arbitrary OAuth `next` values cannot create Stripe resources. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. The old Vercel host still fails closed when legacy activation context is missing.
...
- `api/stripe/webhook.ts`, `api/_lib/stripe-events.ts`, and `api/internal/stripe-events/process.ts` - Signature verification, durable event recording, absolutely capped leased `SKIP LOCKED` claims, retry/backoff/dead-letter isolation, and watermark-protected entitlement reconciliation. Expired final-attempt leases terminalize without a ninth normal processing attempt; Customer/account reads do not process this queue.
...
- `api/activation/start.ts`, `api/activation/status.ts`, and `api/activation/claim.ts` - CEP-facing activation plus authenticated purchase routing and the restore/transfer decision surface. A Free account with a purchasable activation redirects through Google only when needed and then directly to Stripe; restore or transfer requires an active-license session, same-origin CSRF-protected POST, and an explicit prior-device deactivation confirmation for a move.
...
- `tests/entitlement.test.mjs` - Focused Node test harness for exact paid-Session verification, attacker-link/pre-bind regressions, signed plugin Upgrade handoff and fail-closed OAuth cases, device/account binding, restore CSRF/origin checks, safe OAuth return paths, and deterministic lost-response credential replay.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers, while viewports at or below `900px` replace the hero platform choice with the email form and route lower-page download taps back to that form. Website Sidestream Pro entry points open `upgrade.html`; its purchase form POSTs to `/api/checkout/start`, which redirects through Google only when the buyer has no valid account session and then redirects directly to Stripe. No intermediate Checkout page is rendered.
...
- Account/auth/billing/device entitlement - `account.html`, `thank-you.html`, `upgrade.html`, `api/_lib/account.ts`, `api/_lib/entitlement.ts`, `api/_lib/device-policy.ts`, `api/_lib/license-environment.ts`, `api/auth/*`, `api/checkout/*`, `api/billing/*`, `api/stripe/webhook.ts`, `api/activation/*`, `api/account/device.ts`, and `api/license/*` own optional Google account management, the server-owned $9.99 one-time Sidestream Pro Product/Price, signed plugin Upgrade handoffs, confirmed Checkout intents, namespace-separated active-device rows, restricted Test isolation, refund/dispute lifecycle, confirmed transfers, download authorization, deactivation, and device-bound access/refresh credentials. Device mismatch policy defaults to `observe`; only explicit `enforce` blocks, and Customer 360 does not change that mode. The API/operator contract is `docs/api-hardening-runbook.md`; device/support details are in `docs/single-device-entitlements.md`.
...
- Customer 360 usage and private reads - `api/_lib/customer-usage.ts`, `api/_lib/customer-query.ts`, `api/internal/customer-usage/sync.ts`, `api/internal/customers/index.ts`, and `api/internal/customers/[customerId].ts`; schema-versioned telemetry becomes replaceable UTC daily aggregates with exhaustive stored/derivable first/last use and attempt timestamps, outcome counts, lifetime and rolling activity, attempts-per-active-day frequency, coarse client summaries, and source/materialization freshness. Before either compact list/detail query opens a connection, its default read-only transaction resolves an explicit pooled database from trusted server deployment state: Test uses `SIDESTREAM_TEST_POSTGRES_URL` and Production uses `SIDESTREAM_POSTGRES_URL`. The authenticated POST-body `licenseNamespace` remains only an authorized row filter, signed cursors bind it, and invalid or conflicting deployment state fails before pool access. The projection exposes neither total accepted attempts nor current subscription status. The full cross-repo field/privacy/rollout contract is `docs/customer-360.md`.
...
- Customer 360 Preview/Test acceptance - `docs/customer-360-preview-test-plan.md`, `scripts/verify-customer-360-preview-environment.mjs`, and `scripts/verify-customer-360-preview-deployment.mjs`; the matrix owns ordered evidence, stop conditions, read-only host-bound deployment verification, rollback/recreate, and the hard exit. The shared Preview project is rejected; a dedicated isolated non-Production Vercel project/scheduler with exact provider configuration is a human gate, and the repository currently exposes no supported live usage-sync verification command.
...
| `/api/auth/google/start`, `/api/auth/google/callback` | `GET` | Existing-session account/Checkout continuation or Google OAuth and server session creation; an exact state-verified plugin or Checkout capability redirects directly to the locked/reused Stripe Checkout Session |
...
| `/api/checkout/start` | `GET`, `POST` | Website POST or capability-bearing activation/cancellation GET creates or resumes an intent and `303` redirects into auth; bare GET `302` returns to Upgrade; no Stripe write or transition document |
| `/api/checkout/create` | `POST` | Signed browser form authenticates through Google when needed, then `303` to Stripe; authenticated JSON receives `200 {"url":"...","reused":boolean}` |
...
No accepted Customer 360 Preview or Production release exists, and Production
is not migrated. A read-only adversarial audit observed that an earlier candidate
auto-deployed into the shared Preview project; that deployment is evidence of an
unsafe exposure, not Preview acceptance, rollout proof, or a rollback artifact.
Money remains Stripe-verified, currency-separated minor units rather than
entitlement truth. `installIdHash` is a Customer 360 association key, not the
single-device binding, and the Gmail installer-referral HMAC is attribution only,
never identity.

The audit and remediation cycle closed its code-owned findings locally. The
final real-Postgres pass uses this checkout's own lock-matching install and a
private SCRAM-authenticated loopback cluster:
...
The gate supplies the only database URL, blocks provider/non-loopback access,
and runs `test:customer-360-postgres`, `test:postgres-integration`, and
`test:single-device`. It exposed and closed two final defects: stale backfill
fixtures still carried prohibited PII/behavioral fields, and the Stripe ingress
immutability trigger also blocked the maintenance job's required payload
...
configured base/OAuth callback origin, and all four registered cron routes
returned their sanitized `unavailable` responses. Do not infer a backlog drain,
provider fix, or live recovery from local tests.

The canonical execution checklist is
`docs/customer-360-preview-test-plan.md`. It requires a dedicated isolated
non-Production Vercel project and scheduler boundary, complete provider and
connected-target proof, a clean immutable website artifact paired with a clean
FlowState SHA, separately evidenced nonzero paid and FREEDEV sandbox journeys,
named tested alerts, and a successful non-Production rollback/recreate drill.
Only after every timestamped Preview/Test row passes may a fresh Production plan
be commissioned. No deployment, migration application, provider change,
backfill, live rail exercise, Preview readiness, or Production readiness is
claimed here.
...
The private Customer 360 list/detail transaction also selects its physical
database from that trusted state before opening a connection. An isolated Test
runtime needs at least one trusted server selector that resolves to `test`
(`VERCEL_ENV=preview|development|test` or
`SIDESTREAM_LICENSE_NAMESPACE=test`), agreement when both are present, a valid
nonempty `SIDESTREAM_TEST_API_HOSTS`, and a valid dedicated
`SIDESTREAM_TEST_POSTGRES_URL` distinct from a configured
`SIDESTREAM_POSTGRES_URL` target.
The request-body `licenseNamespace` and generic Postgres aliases cannot change
that target. This is repository behavior only; it does not establish that a
Preview database is connected, migrated, or accepted.
...
The SaaS/account flow is server-owned. The landing-page Account link enters `/api/auth/google/start`: a valid server session goes directly to `account.html`, while a signed-out visitor enters Google OAuth and the callback creates or reconnects the account. Direct signed-out visits to `account.html` use the same entry instead of rendering an empty account panel. The Google callback origin must exactly match the browser-facing OAuth start origin; the required Production contract is `SIDESTREAM_BASE_URL=https://sidestream.tv` plus `https://sidestream.tv/api/auth/google/callback`, but the 2026-07-19 read-only audit observed a canonical/configured origin mismatch and did not repair or reverify it. The start route fails closed before setting state cookies when origins disagree. The session is remembered for up to 30 days by the existing HTTP-only, `SameSite=Lax` `sidestream_session` cookie; do not add a browser-readable identity cookie. While Production intentionally remains on the pre-entitlement-lifecycle schema, every customer-facing license read uses the shared `LICENSE_ENTITLEMENT_STATUS_SQL` expression to detect the missing column without runtime DDL and treats only exact one-time paid `active`/`trialing` rows as compatible Pro access; after the column exists, its canonical stored value always wins. Sign out clears the server session and returns to the landing page so the automatic account entry cannot immediately sign the user back in.

Website purchase requires a Sidestream account, but never asks for a second purchase confirmation. The `upgrade.html` purchase form sends a same-origin `POST /api/checkout/start`; activation and cancellation retries use capability-bearing GETs, while a bare GET returns to Upgrade. Start creates or resumes only a 24-hour database intent and immediately redirects to `/api/auth/google/start` with its opaque browser capability. If the buyer has no session, Google OAuth carries that capability in a short-lived HTTP-only cookie and the state-verified callback enforces its account binding before Stripe. An existing session skips Google and the auth start route enters the same worker immediately. There is no happy-path transition document, hidden auto-submit form, or checkout-transition script. The authenticated worker validates or creates the account's Stripe Customer, passes that Customer to Checkout so its email is prefilled, and adds the explicit Sidestream signed-in email beside the payment action. The path is bounded to 8 requests per intent and 20 per IP per 15 minutes, locks and reuses the intent, then creates one `mode=payment` card Session for one quantity of the exact Sidestream Pro Product/Price.

A current plugin activation with explicit request source `plugin` and a parseable version newer than `1.0.13` receives a different `upgradeUrl`: `/api/auth/google/start?plugin_upgrade=<signed-capability>`. The HMAC capability binds the exact activation key and expires with the 10-minute OAuth window. OAuth start validates it first. A signed-out buyer carries it in an HTTP-only return-intent cookie through Google, and only the callback with matching OAuth state may consume it. A buyer with an existing valid session skips Google and enters the locked/idempotent Stripe Session worker directly. In both authenticated branches the worker passes the validated account Customer to Stripe and identifies the signed-in Sidestream email on hosted Checkout. An active Pro owner returns to the activation claim branch; a Free account enters the same 8-per-intent, 20-per-IP limiter plus worker used by `POST /api/checkout/create`. Missing, malformed, tampered, expired, legacy-client, or non-plugin intent cannot reach Stripe. Arbitrary OAuth `next` values still collapse to the account or restore allowlist, and `/api/checkout/start` remains non-creating. The success callback re-fetches the literal `{CHECKOUT_SESSION_ID}` and verifies payment, line item, quantity, Price, Product, activation metadata, and attachment before fulfillment.
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Free" and says "5 free downloads every day." The Pro plan is a visible `$9.99 once` one-time upgrade that opens `upgrade.html`; its purchase POST redirects through Google only when authentication is missing and then directly to Stripe without an intermediate page.
...
the absence of a supported live usage-sync command, project-wide cron
...
- Run `node --experimental-strip-types --test --test-concurrency=1 tests/customer-360/query-api.test.mjs` after changing the Customer 360 list/detail runtime boundary. Its default-dependency regression proves trusted Test and Production target selection, keeps the request namespace as a row filter, and proves invalid or conflicting deployment state opens no transaction. This local command authorizes no deployment or Production activity.
- For the reproducible real-Postgres closure, first run `npm ci --ignore-scripts --no-audit --no-fund`, leave every database/runtime/deployed-target selector unset, and run `node scripts/run-customer-360-isolated-postgres-gate.mjs`. It requires local `initdb`, `postgres`, and `pg_ctl`, creates and removes its own private loopback cluster, and runs the Customer 360 Postgres, broader Postgres integration, and single-device aggregates without accepting an external target.
...
- Confirm the Pro card says `$9.99 once` and opens `upgrade.html`. Submitting `Continue to Stripe` must use the same-origin `/api/checkout/start` POST, render no intermediate Checkout page, use Google only when authentication is missing, and then redirect directly to Stripe.
...
- With Vercel dev and account env configured, confirm the `upgrade.html` purchase POST receives only server redirects: signed-out buyers land on Google OAuth before the state-verified callback enters Stripe, and an existing valid session skips Google and lands on Stripe directly. Confirm no request in either happy path renders a Sidestream transition page. Separately start a current `source=plugin` activation and confirm its signed-out and signed-in branches reach the one attached Stripe Test Checkout Session without a purchase-confirmation control. Hosted Checkout must prefill the authenticated account email and show `Signed in to Sidestream as <email>. One-time payment. No subscription.` beside the payment action; an actually anonymous compatibility call must retain only `One-time payment. No subscription.` and create no account Customer. Tamper with, omit, or expire either capability and confirm no Checkout worker call occurs; repeat with a non-plugin source and a 1.0.13 client and confirm its capability-bearing compatibility path also redirects through auth without rendering a transition. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, cancelled Checkout resumes/rotates intentionally, `/account.html` can still sign in with Google, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
...
- No accepted Customer 360 Preview or Production release exists, and Production is not migrated. The earlier shared-project auto-deploy is unsafe-exposure evidence only. The repository contract allows only the human-gated Preview/Test-first sequence in `docs/customer-360.md`; no dry-run, local test, build, or documentation result is Production approval.
- Customer 360 local or fixture-backed FlowState consumers may implement the audited contract before website deployment, but live upstream Preview/Test integration and QA remain gated on separately approved migration, configuration, deployment, protected API, freshness, and scheduling evidence. Vercel cannot enable only the usage job; the Preview/Test matrix keeps project-wide cron disabled, and the current deployment verifier is read-only. U01 remains Blocked until a separate non-Production usage-sync mechanism is reviewed and approved.
- The shared Vercel Preview project fails the Customer 360 isolation gate. Do not work around this with client namespace fields, a mutable alias, shared credentials, or project-wide cron changes; a human must provision and approve a dedicated isolated project/scheduler boundary before the Preview/Test matrix can advance.
...
- License environment resolution fails closed unless the supplied trusted deployment state, trusted host when applicable, and selected database agree. Production uses `SIDESTREAM_POSTGRES_URL`; preview/development/test require exact `SIDESTREAM_TEST_API_HOSTS` plus a distinct `SIDESTREAM_TEST_POSTGRES_URL`. Customer 360 list/detail resolves this same boundary before pool access even though the request-body namespace may select authorized rows. Client `buildChannel` and request JSON cannot select the database.
...
- Hosted Checkout only accepts promotion codes that already exist in the same Stripe account and mode as `STRIPE_SECRET_KEY`. The repo utility `npm run billing:ensure-freedev` is sandbox-only: it requires the reviewed Test account plus exact active one-time $9.99 Product/Price and creates or verifies a 100% once coupon restricted to that Product. It has no live-mode escape. Stripe can report a completed zero-total promotion Checkout as `payment_status=paid` with `amount_total=0` and no PaymentIntent; fulfillment accepts that exact shape only after the normal Session, Customer, line-item, Price, Product, metadata, and currency checks pass. FREEDEV proves the zero-dollar rail only; use a separate nonzero sandbox Checkout for PaymentIntent, Charge, receipt, refund, and portal evidence. Vercel protected env pulls can return `STRIPE_SECRET_KEY=""`; in that case, use an ignored local env file through `SIDESTREAM_STRIPE_ENV_FILE`.
...
- 2026-07-20: Fixed FREEDEV completion after live Stripe Sandbox evidence showed a fully discounted Checkout Session can be `payment_status=paid`, `amount_total=0`, and omit both PaymentIntent and Charge. Fulfillment now accepts that exact zero-total shape while retaining the existing completed-Session, Customer, Product, Price, quantity, metadata, and currency guards. Added disposable-Postgres proof for initial fulfillment and idempotent replay with zero stored payment identifiers, including a temporary-module resolver for the runtime's intentional `.js` import specifiers. Production remains untouched.
...
- 2026-07-20: Deleted the Checkout transition page and its auto-submit script after real-browser verification showed that the rendered handoff could strand buyers. Website purchase now uses an explicit same-origin POST that responds only with server redirects; current plugin Upgrade enters Google directly when signed out, while an existing session enters the locked/rate-limited Stripe worker immediately. Capability-bearing legacy and cancellation paths also redirect without rendering an intermediate page. Restore/transfer controls remain separate. Added redirect, signed-in direct-Stripe, OAuth-resume, account-binding, and abuse regressions. Only the isolated Preview/Test surface may be deployed; Production remains untouched.

- 2026-07-19: Carried an exact current-plugin Upgrade capability through Google OAuth and routed the state-verified callback directly into the existing rate-limited, locked, idempotent Stripe Checkout worker. Generic GET, link-preview, arbitrary `next`, missing, stale, tampered, legacy-client, and non-plugin paths remain non-creating; restore, transfer, active-license, Test isolation, and website confirmation behavior remain intact. Added focused entitlement regressions and updated the API runbook. This is source and local verification only; no deployment, provider, database, Stripe, or Production action occurred.

- 2026-07-19: Repaired and documented the Customer 360 list/detail runtime boundary so trusted server deployment state selects the explicit pooled Test or Production database before connection, while the authenticated body namespace remains only a row filter. Added default-dependency regression coverage, removed the temporary diagnosis note after promoting its durable conclusions, and retained every Preview/Production human gate; no provider, database, deployment, migration, or Production action occurred.

- 2026-07-19: Closed the adversarial Customer 360 local database gate with a self-provisioned isolated Postgres command, removing forbidden stale backfill fixture fields and permitting only the maintenance job's exact irreversible Stripe payload redaction while keeping ingress identity immutable. The two observed Production incidents and every provider, Preview, live rail, migration, backfill, rollback, and Production-plan gate remain external and blocked.
