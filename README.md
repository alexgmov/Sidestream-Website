...
- `api/checkout/start.ts` - Public Stripe Checkout redirect for the Sidestream subscription. It creates a hosted Checkout Session with the cancellation/refund promise custom text and optional plugin activation metadata, then redirects directly to Stripe without a Google sign-in interstitial.
- `api/checkout/create.ts` - Backward-compatible authenticated JSON Checkout Session creator for account-page callers. It uses the persisted Basic subscription Stripe Product/Price and includes the same Stripe-page cancellation/refund custom text. The subscription is not fulfilled until Stripe webhooks update the backend license record.
...
- Account/auth/billing - `account.html`, `upgrade.html`, `api/_lib/account.ts`, `api/auth/*`, `api/checkout/start.ts`, `api/checkout/create.ts`, `api/billing/portal.ts`, `api/stripe/webhook.ts`, `api/activation/*`, and `api/license/verify.ts` own optional Google account management, Stripe Product/Price bootstrap, direct Checkout redirects, Customer Portal redirects, webhook-owned entitlements, plugin activation, and license-token verification
