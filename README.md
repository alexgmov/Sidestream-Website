...
The header is a fixed transparent overlay with no scroll divider so the shader remains uninterrupted behind the nav. The `.hero-pad` section fills the first viewport and aligns the hero headline, description, and primary `Download for Mac` CTA to the lower-left first-fold gutter. The Sidestream wordmark and hero copy share the viewport-left `24px` first-fold gutter, and the Features/Pricing/Account navigation cluster is absolutely anchored to the viewport's top-right corner with a `15px` top offset and matching `24px` right gutter. The header intentionally has no download CTA; downloads remain available in the hero, pricing, and final CTA sections.
