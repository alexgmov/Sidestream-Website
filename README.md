...
- `config/upgrade-pricing-experiment.mjs`, `api/_lib/upgrade-pricing-experiment.ts`, `db/migrations/20260812120000_add_upgrade_pricing_experiment.sql`, `db/migrations/20260825140000_add_annual_upgrade_pricing_experiment.sql`, and `docs/upgrade-pricing-experiment.md` - Authenticated Upgrade pricing contracts. Historical `upgrade-pricing-v1` remains closed and immutable. Production `upgrade-pricing-v2` concluded on 2026-09-04 with `$19.99/year` selected for every future eligible global-USD account; source forces rollout `10000` despite stale environment values, while existing one-time/monthly assignments and regional one-time fallbacks remain unchanged. Missing assignment/provider configuration still fails closed without creating an annual assignment.
