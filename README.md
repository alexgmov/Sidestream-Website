...
- 2026-07-20: Restored the shipped CEP Upgrade contract without deploying Customer 360 or mutating Production schema. Activation-bearing Checkout GETs now resume or create their one idempotent attached Stripe Session; v1.0.14 keeps its required activation-claim `restoreUrl`, and pending Upgrade claims recover to Checkout before account authentication. Account/restore remains on the claim path. Runtime Postgres accepts Neon’s required channel-binding parameter instead of rejecting Production before connection. Catalog-selected compatibility covers paid fulfillment, activation status, token verification/refresh, and download authorization on the known pre-hardening Production schema. Disposable Postgres tests prove the complete v1.0.14 purchase path on that schema and preserve the hardened Checkout-intent path on the full migration chain.
