...
- `api/referral-visit.ts` and `api/_lib/referral-visits.ts` - First-party landing-referral attribution for the short `https://sidestream.tv/m` ManyChat link. A real page load posts the allowlisted source, receives `204` without waiting for storage, and writes at most one private Blob record per anonymous request fingerprint/day/source/classification. Daily HMACs reuse the installer-analytics secret; raw IP and user-agent values are never stored.
...
- Landing referral attribution - `https://sidestream.tv/ig` temporarily redirects to the canonical root with `utm_source=instagram&utm_medium=social&utm_campaign=bio`; `https://sidestream.tv/m` does the same with `utm_source=manychat`. The loaded page POSTs only the allowlisted ManyChat source to `/api/referral-visit`; private Blob pathnames dedupe repeated visits from the same anonymous request fingerprint on the same UTC day and separate likely-human from likely-scanner traffic. The Instagram short link retains standard UTM attribution without writing that private referral event. These links measure landings, not downloads, installs, activations, purchases, or durable identities.
...
The Instagram bio link is `https://sidestream.tv/ig`. It returns a temporary redirect to `https://sidestream.tv/?utm_source=instagram&utm_medium=social&utm_campaign=bio` so standard landing and lead attribution retain the source.
...
| `/api/referral-visit` | `POST` | `204` after accepting the allowlisted `manychat` source and scheduling a private, daily-deduped anonymous Blob write |
...
- ManyChat referral attribution deliberately measures real landing-page API calls, not ManyChat sends, link clicks that never load the page, downloads, installs, activations, or purchases. The `/m` redirect preserves `utm_source=manychat`, but the authoritative owned count is the private-Blob report, not Vercel's plan-gated UTM panel. Same-day reloads from the same anonymous request fingerprint are deduped; cross-day totals are not durable person counts.
