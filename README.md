...
- `middleware.ts`, `server/hetzner-api.ts`, `tsconfig.server.json`, and `api/internal/hetzner-secret-export.ts` - Three-state `source`/`fenced`/`target` API routing, loopback-only Hetzner execution for the existing Website handlers, and an expiring bearer-protected RSA/AES encrypted one-time export for server-only runtime configuration and exact license-hash continuity. Unknown/invalid target configuration fails closed to a retryable no-store 503; the export never returns plaintext secrets and is removed after cutover.
