...
- `api/referral-visit.ts` and `api/_lib/referral-visits.ts` - First-party landing-page referral attribution. The POST-only route accepts the fixed `manychat`, `instagram-bio`, `instagram-alex`, `reddit-1`, and `reddit-2` sources plus numbered `meta-ads-1` through `meta-ads-999`, returns `204` before its bounded background write, hashes request identity per source/day without storing raw IP or user agent, flags likely scanners, and writes one deterministic private Vercel Blob record per daily visitor.
...
- `demos/sidestream-panel-corner.webm` - Square VP9-alpha WebM generated from the ProRes source `sidestream demo Linked Comp 01_2.mov` using the full-plugin/timeline top-left crop. Its partially transparent top-edge RGB is stored as straight alpha so Safari does not double-premultiply that row into a black bar. Mounted as an opaque decorative Premiere/Sidestream corner inside the hero, visually scaled to 70% from the Premiere panel's top-left corner and anchored at `45vw 25vh` on desktop. At `900px` and below, the same loop reflows into a compact, lifted, right-biased product demo with a low soft fade above the main hero copy.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers, while viewports at or below `900px` replace the hero platform choice with the email form and its "Email me the download link" submit action, then route lower-page download taps back to that form. Anonymous website entry points retain the no-store `/api/checkout/start` confirmation page, while activation-bearing panel handoffs and authenticated Free activation claims remain direct. The authenticated Free-account Upgrade form posts directly to `/api/checkout/create` and receives the Stripe redirect without visiting `upgrade.html` or the start confirmation.
...
Numbered Meta Ads referral URLs are available from campaign `1` through `999`:

```text
https://sidestream.tv/meta/1
https://sidestream.tv/meta/2
...
https://sidestream.tv/meta/999
```

Each uses standard paid-social tags with its matching `utm_campaign` number and records separately as `meta-ads-<n>`. For example, `/meta/2` records as `meta-ads-2`. Trailing-slash forms are supported identically. Zero, leading-zero, negative, decimal, and numbers above `999` are intentionally rejected.
...
| `/api/checkout/start` | `GET` | Anonymous website entry returns `200` no-store signed intent confirmation HTML with no Stripe write; a valid activation-bearing app entry authenticates as needed and redirects through the locked worker to Stripe |
...
| `/api/activation/claim` | `GET`, `POST` | Read-only GET decision; an eligible authenticated Free claim redirects into direct app Checkout, while active-owner restore or transfer requires a CSRF/same-origin POST |
...
At `520px` and below, the final phone override supersedes the earlier crop values with a compact `clamp(260px, 32vh, 300px)` wrapper, a `min(590px, 124vw)` video, a `60px` wrapper lift, and the full `-8%` internal vertical translation. The phone crop has no top fade; it inherits only the existing lower mask transition. The corrected straight-alpha video edge removes Safari's black fringe without hiding the Premiere window top. The headline and inline mobile handoff rise `88px` over the recording's dark lower fade, while `48px` bottom padding keeps the full email card visible at `390x844` with no horizontal overflow.
...
SIDESTREAM_ENV_FILE='<ignored-env-file-with-blob-credentials>' \
  npm run analytics:referrals -- --source meta-ads-2 --days 7
SIDESTREAM_ENV_FILE='<ignored-env-file-with-blob-credentials>' \
...
- Run `npm run test:referral-visits` after changing `/m`, `/ig`, numbered `/meta/<n>` routes, `/reddit/1`, `/reddit/2`, `/api/referral-visit`, or the private-Blob attribution helper. It verifies the source allowlist, short routes, daily deduplication, privacy boundary, storage path, and scanner heuristic.
...
- 2026-07-24: Generalized Meta Ads referral links to `/meta/1` through `/meta/999`; every valid number redirects with its matching paid-social campaign tag and records in its own `meta-ads-<n>` first-party bucket.
...
- 2026-07-24: Restored the full `-8%` phone crop and corrected the WebM's premultiplied top-edge RGB so Safari no longer renders that alpha row as a black bar.
- 2026-07-24: Removed the remaining app-flow Checkout interstitial: an authenticated Free activation claim now redirects to the activation-bearing Checkout route, which enters the existing locked worker and sends the browser directly to Stripe. Public website purchase confirmation and explicit restore/transfer confirmation remain unchanged.
- 2026-07-24: Made repeated same-device reconnects credential-idempotent: issuing a new activation credential family now revokes the predecessor for the exact account/license/device inside the existing transaction, leaving one live family without consuming a device move.
- 2026-07-24: Repaired the disposable activation-security and single-device Postgres harnesses so their temporary runtime copies resolve the source `postgres-target.ts` module while production imports keep their compiled `.js` contract.
