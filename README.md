...
- The Postgres migration runner has a checksummed ledger and advisory lock. `npm run db:migrate -- --status` is the authoritative complete applied/pending chain; use `--validate`, and use `--baseline` only after backup plus catalog verification of an existing non-empty pre-ledger schema. `scripts/verify-migration-baseline.mjs` recognizes only its narrower baseline/conditional-RLS profile and does not list all later migrations. Production applies through the direct URL, while runtime must use the shared pooled URL. Fallback keeps the migrated schema and promotes only a separately staged, Production-environment, runtime-distinct artifact qualified against the full chain; it never deletes the ledger or reverses destructive migrations.
...
- Vercel Preview/Test remains the only Stripe test-mode lifecycle proof. Finalize Production secrets and configuration first, then build the exact Production-environment release and fallback with `vercel --prod --skip-domain`, protect and qualify their unassigned deployment URLs, and attach the unchanged release with `vercel promote`. Preview promotion rebuilds, and any source/config/environment change invalidates a staged artifact and requires full requalification.
- The current repository has no Production maintenance rule or operator bypass. Before any production mutation, the runbook requires first-priority Vercel WAF rules on every Production hostname: publicly allow only `GET`/`HEAD`/`OPTIONS /api/releases/latest` and `HEAD /api/download`, deny every other `/api/**` request, and permit only five exact method/path operator canaries from an approved IP plus secret header. Tagged and untagged download GETs stay blocked because tagged GET can schedule a referral write. Production is blocked until missing/wrong bypass and every host/method/path case are separately proved.
- Keep the live Stripe event destination enabled during maintenance and fallback; deny its webhook at the edge so delivery receives non-`2xx` and remains retryable. After the compatible artifact is live, allow only exact `POST /api/stripe/webhook`, retry every Failed/Pending Workbench delivery in the recorded UTC/event-ID window, and require `2xx`, a local ledger row per ID, zero due backlog, and zero dead letters before reopening other writes. Events created while a destination is disabled do not auto-resend.
