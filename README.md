...
```text
sidestream/1.0.16/73fb0c6fc5461c82/Sidestream-Unlimited.dmg
```
...
- 2026-08-09: Rolled the receipt-gated paid Mac pointer back from the isolated `1.0.17` Unlimited artifact to the last stable paid-onboarding `1.0.16` artifact. The `1.0.17` artifact had been rebuilt from FlowState `main` after preview clipping commits landed, while retaining the older Production application version; this exposed Test-intended clipping behavior only to the Meta/Unlimited fulfillment path. The ordinary public Mac and both Windows pointers remain unchanged. Paid releases now require an explicit FlowState source-commit/release-boundary check before their pointer advances.
