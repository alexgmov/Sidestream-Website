...
- `pryt.png` - Lightweight 500×500 transparent Premiere Pro/YouTube mark artwork shown only in the mobile hero. The visible marks occupy the center of the padded square canvas so the responsive wrapper can center them without a background matte.
...
- `.mobile-hero-logos` is the mobile-only replacement for that recording. Keep its wrapper flexible and its square `pryt.png` child absolutely centered; putting the padded image in normal flow would reserve its full 500×500 canvas height and shove the conversion form below the first fold.
