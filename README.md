...
- `api/_lib/paid-installer-email.ts`, `api/_lib/paid-release-manifest.ts`, `api/paid-download.ts`, `api/releases/paid-latest.ts`, and `data/release-manifest.paid*.json` - Isolated paid-onboarding email and installer surfaces. Provider delivery requires `SIDESTREAM_PAID_ACQUISITION_EMAIL_ENABLED=1`; Production enables it so an exactly verified paid-acquisition Checkout creates one idempotent Resend delivery to Stripe's verified Checkout email. The paid artifact does not grant entitlement by itself.
