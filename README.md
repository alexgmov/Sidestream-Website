...
- `api/_lib/paid-telemetry-handoff-repair.ts`, `scripts/reconcile-paid-telemetry-handoff.mjs`, and `scripts/replay-paid-telemetry-handoff.mjs` - One-acquisition, fail-closed Meta-paid telemetry repair. Starting only from the canonical acquisition UUID and trusted namespace, it supports the original direct/simple split, the single pending verified-account-review path, and the narrower dual-active-path case where exactly one disjoint reviewed bridge selects the current activation while a direct-account path remains historical. Duplicate reviewed paths/owners, direct+review overlap before binding, root mismatch, or any downstream payment, lifecycle, identity, commerce, stage, or binding conflict refuses before mutation. Apply is atomic/additive and exact-binding replay is a no-op.
