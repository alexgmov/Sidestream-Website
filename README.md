...
- Customer 360 commerce ledger - `api/_lib/customer-commerce.ts`, `20260715122000_add_customer_commerce_ledger.sql`, and `tests/customer-360/commerce*.test.mjs`; captured money comes from one canonical PaymentIntent or standalone Charge per payment group, paid InvoicePayment edges carry invoice classification without collapsing many-to-many allocations, and contradictory live identity evidence triggers sticky whole-group quarantine.
...
- Customer 360 captured money authority is the PaymentIntent `amount_received`, or `amount_captured` on a standalone Charge when no PaymentIntent exists. Checkout `amount_total` and Invoice `amount_paid` are advertised/attribution parents and must never re-inflate a partial capture or win through a heterogeneous `MAX`. Paid InvoicePayment rows are allocation edges keyed by `invoice_payment.id`; open/canceled rows do not attribute money, and an invoice/instrument many-to-many graph must not become alias equivalence.
