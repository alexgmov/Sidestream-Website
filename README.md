...
- `docs/single-device-entitlements.md` - Focused single-active-device domain reference for privacy, API/page states, and operator/support commands only. Its older migration/cutover section is dangerously stale (it denies the checksummed ledger and gives a one-file `psql` procedure). This step cannot edit that file, so production is blocked until a separately owned change places the supersession warning inside it; do not use it for production mutation or rollback.
- `docs/api-hardening-runbook.md` - Sole intended production migration/cutover procedure and exact hardened API/release contract, shared Postgres and migration model, Stripe/lead/maintenance operations, bounded configuration, metrics and alerts, staged-Production artifact rules, executable WAF matrix, ordered human gates, and fix-forward/fallback procedure. It explicitly identifies missing controls and unresolved lifecycle/stale-document gaps as pre-production blockers and does not claim production was changed.
...
| Limiter/lead | `SIDESTREAM_RATE_LIMIT_HASH_SECRET` and `SIDESTREAM_LEAD_HASH_SECRET` are stable server-only HMAC values of at least 32 characters; `SIDESTREAM_DOWNLOAD_LEADS_BLOB_PREFIX` selects the private fallback prefix. Vercel WAF counters are per region, so its IP rule is not a global 20-per-10-minute bound; use an approved regional exposure or a durable shared fallback limiter. |
...
Only server-secret HMAC-SHA-256 device digests plus coarse platform/version/timestamps may be persisted. Raw hardware fingerprints, raw device IDs, serial numbers, and device names are prohibited from storage and logs. OS-backed non-exportable device keys are future hardening, not protection delivered by this implementation. See `docs/single-device-entitlements.md` only for the device schema, API/page states, environment matrix, privacy rules, and operator/support actions. Its migration/cutover section is historical and superseded, but the warning is not yet present inside that file; external routing is insufficient for someone opening it directly. All production status, baseline, migration, backfill, cutover, and application rollback work must follow `docs/api-hardening-runbook.md` exclusively, and production is blocked until the stale section is neutralized in a separately owned change.
...
- Keep the live Stripe event destination enabled during maintenance and fallback; deny its webhook at the edge so delivery receives non-`2xx`. Live automatic retries last at most three days, Dashboard/Workbench resend at most 15 days, and Stripe CLI resend at most 30 days. The runbook caps planned maintenance at two hours, escalates unresolved reconciliation at 24 hours, and hard-aborts by 48 hours. Endpoint `enabled_events` and Workbench deliveries are endpoint-scoped, so reconciliation paginates the account-level List Events API for every required type across a conservative inclusive created-time interval, unions those IDs with every Workbench delivery page, and requires a `2xx` attempt plus matching local ledger type for every unioned ID. Zero due backlog and zero dead letters are required before reopening. Events created while a destination is disabled do not auto-resend.
...
- Checkout and lead rate limits are atomic Postgres controls, not a substitute for edge protection. The Blob fallback cannot consume the database lead limiter. Vercel WAF rate-limit counters are per region, so the same source can exceed a configured per-region threshold in aggregate. Before relying on fallback, production needs either a security-approved regional threshold/reachable-region exposure with multi-region proof or a durable shared fallback limiter; if the intended global bound cannot be proved, cutover is blocked.
