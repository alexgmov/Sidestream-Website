...
- `api/send-download-links.ts` and `api/_lib/download-link-email.ts` - Mobile-only computer handoff. The public POST route requires an idempotency key, fails closed without Postgres-backed abuse controls, stores the canonical `mobile-download-handoff` lead plus bounded UTM context, enforces 3/email and 10/IP per hour, and sends one transactional Resend message from `downloads@alexg.mov` with direct Mac and Windows installer links. Provider errors and logs never return or print the recipient address.
...
| `/api/send-download-links` | `POST` | `200 {"ok":true}` after a rate-limited canonical lead write and Resend acceptance; fails closed when Postgres or email delivery is unavailable |
...
- 2026-07-17: Added the mobile computer handoff: phone-width visitors see one inline email form instead of choosing Mac or Windows, lower mobile download CTAs return to that form, and the new fail-closed `/api/send-download-links` route stores bounded campaign context before sending one idempotent Resend email with both installers from the existing `alexg.mov` domain. Desktop downloads remain direct.
