...
- `docs/single-device-entitlements.md` - Focused single-active-device domain reference for privacy, API/page states, and operator/support commands. Its older migration and production-cutover sections predate the checksummed ledger and are superseded; do not use them for production migration, backfill, cutover, or application rollback.
- `docs/api-hardening-runbook.md` - Sole production migration/cutover procedure and exact hardened API/release contract, shared Postgres and migration model, Stripe/lead/maintenance operations, bounded configuration, metrics and alerts, required webhook events, staged-Production artifact rules, executable WAF matrix, ordered human gates, and fix-forward/fallback procedure. It explicitly identifies missing controls as pre-production blockers and does not claim production was changed.
...
Canonical paid state is `entitlement_status=active` on `sidestream_pro` or compatible `sidestream_unlimited`. Partial refund remains `active/partial_refund`; full refund becomes irreversible `revoked/full_refund`; an open dispute is `suspended/dispute_open`; a won dispute may return to `active/dispute_won`; a lost dispute is permanent `revoked/dispute_lost`. A Stripe-created-at plus event-ID watermark prevents stale Checkout, refund, dispute, or subscription events from resurrecting a later state. Legacy recurring access is default-deny and requires an exact Product in `SIDESTREAM_LEGACY_SUBSCRIPTION_PRODUCT_IDS` and exact Price in `SIDESTREAM_LEGACY_SUBSCRIPTION_PRICE_IDS` before reviewed backfill.
...
Apply the Postgres download-lead schema and migrate existing Blob leads only with a database URL whose live waitlist count has been verified:
...
```bash
SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --status
SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --validate
SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --dry-run
SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate
```
...
- Vercel Preview/Test remains the only Stripe test-mode lifecycle proof. Finalize Production secrets and configuration first; in both clean checkouts, non-interactively link and prove the exact `alex-3685s-projects/sidestream` team/project IDs before running `vercel --prod --skip-domain`. Protect and qualify the unassigned Production-environment deployment URLs, then attach the unchanged release with `vercel promote`. Preview promotion rebuilds, and any source/config/environment change invalidates a staged artifact and requires full requalification.
- The current repository has no Production maintenance rule or operator bypass. Before any production mutation, the runbook requires a complete inventory and ordering review of existing Vercel firewall controls, followed by first-priority custom rules on every Production hostname: publicly allow only `GET`/`HEAD`/`OPTIONS /api/releases/latest` and `HEAD /api/download`, deny every other `/api/**` request, and permit only five exact method/path operator canaries from an approved IP plus secret header. A Vercel custom-rule bypass skips all later custom and managed WAF rules, not merely the maintenance deny, so each tuple's actual bypass effect must be explicitly scoped, accepted, and separately tested. Tagged and untagged download GETs stay blocked because tagged GET can schedule a referral write.
- Keep the live Stripe event destination enabled during maintenance and fallback; deny its webhook at the edge so delivery receives non-`2xx`. Live automatic retries last at most three days, Dashboard/Workbench resend at most 15 days, and Stripe CLI resend at most 30 days. The runbook caps planned maintenance at two hours, escalates unresolved reconciliation at 24 hours, and hard-aborts the cutover by 48 hours from the earliest event so the operator does not consume the retry window. After the compatible artifact is live, allow only exact `POST /api/stripe/webhook`, retry every Failed/Pending delivery in the recorded UTC/event-ID window, and require `2xx`, a local ledger row per ID, zero due backlog, and zero dead letters before reopening other writes. Events created while a destination is disabled do not auto-resend.
...
- 2026-07-14: Closed the follow-up API-hardening documentation audit: documented exact release-channel normalization; added explicit env-file injection for non-runner database tools and separate local/ledger checksum evidence; made Vercel linking deterministic by team/project ID and the protected read matrix executable; accounted for custom bypasses skipping later custom/managed WAF rules; and bounded Stripe automatic/Dashboard/CLI resend recovery with pre-expiry abort gates. Missing WAF, dead-letter replay, per-job kill switches, and a runtime-distinct fallback remain explicit production blockers. This docs-only step performed no production deploy, query, migration, secret/WAF/cron/traffic change, or cutover.
- 2026-07-14: Remediated all three API-hardening documentation audits. The sole runbook now keeps Stripe delivery enabled and reconciles an exact Workbench event-ID window; stages protected Production-environment artifacts before no-rebuild promotion; defines an exact host/method/path/IP/header WAF maintenance matrix with HEAD-only download access; uses Vercel's global cron control plus sequential manual jobs; makes migration status authoritative; and gates database URL changes on exact license-hash continuity. Missing WAF, dead-letter replay, per-job kill switches, and a runtime-distinct fallback remain explicit production blockers. This docs-only step performed no production deploy, query, migration, secret/WAF/cron/traffic change, or cutover.
- 2026-07-14: Consolidated the hardened API contract and sole production runbook: confirmed-POST Checkout, normalized release aliases, shared pooled Postgres ownership, checksummed migrations, activation compatibility, refund/dispute entitlements, durable Stripe retries/dead letters, lead fallback/replay, protected crons, bounded retention, alerts, and disposable-database proof. Cutover is explicitly blocked until a runtime-distinct fallback is proved against the full migration chain; Preview/Test, secrets, WAF, and a production write-maintenance gate precede the contiguous migration/promotion/smoke window. Terminal Stripe dead letters still have no recovery tool. No production deployment or configuration change is claimed.
