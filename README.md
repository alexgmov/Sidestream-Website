...
- `paid-thank-you.html` - Phone-first noindex success page used only by verified paid-acquisition Checkout. It leads directly into the numbered computer setup steps: find the separate Sidestream setup email, install from its receipt-gated platform link, and authenticate with the same Google email used at Checkout, then ends with the support link without a redundant introductory email reminder or phone-completion card. The original `thank-you.html` remains the ordinary Upgrade/Restore destination.
...
- `api/_lib/paid-telemetry-handoff-repair.ts`, `scripts/reconcile-paid-telemetry-handoff.mjs`, and `scripts/replay-paid-telemetry-handoff.mjs` - One-acquisition, fail-closed Meta-paid telemetry repair. Starting only from the canonical acquisition UUID and trusted namespace, it supports the original direct/simple split, the single pending verified-account-review path, the narrower dual-active-path case where exactly one disjoint reviewed bridge selects the current activation while a direct-account path remains historical, one exact legacy entitlement snapshot, and one exact recoverable commerce fact on that fully verified path. The fifth boundary requires one payment key and one verified exact-Checkout fact with matching currency and Checkout/payment identity evidence, null owner, zero gross/net, and no competing fact, key, owner, conflict, refund, dispute, inquiry, or lifecycle stop. Apply merges first, conditionally raises only that locked fact to the already verified positive paid amount and completion time, confirms its survivor owner, refreshes totals, and requires a positive attached owner before `already_repaired`. Provider identifiers, verified provenance, entitlement placeholders, and unrelated commerce remain unchanged; any mismatch rolls back and replay is a no-op.
...
- `scripts/reset-alex-upgrade-state.mjs` - Dry-run-first Alex-only authentication, entitlement, device, Stripe-customer, and paid-acquisition reset across fixed Production and Test targets. Apply still requires the exact destructive confirmation. Stripe credentials come from explicit environment variables or an ignored reset env file first; on macOS, missing values fall back to Keychain items named `SIDESTREAM_RESET_PRODUCTION_STRIPE_SECRET_KEY` and `SIDESTREAM_RESET_TEST_STRIPE_SECRET_KEY` under account `alex@alexg.mov`, so repeat tests do not require plaintext secret files.
...
predate/equal that landmark. Within each class, earliest exact evidence wins
with stable database tie-breakers. Repeated leads may fill only a previously
...
acquisition through the receipt-gated paid-onboarding artifact, a loaded
...
- For repeat Alex reset tests on macOS, store the Live and Test Stripe secrets as generic-password Keychain items with service names matching the two reset environment variables and account `alex@alexg.mov`. `npm run account:reset:alex` reads them only when the corresponding environment value is absent; environment and ignored reset-file values remain authoritative. The command never passes either secret to the Neon CLI.
...
- Run `npm run test:customer-360`, `npm run test:acquisition-journey-matrix`, `npm run verify:checkout-contract`, and `npm run test:entitlement` after acquisition/retention documentation or contract changes. For schema/query/concurrency behavior, also run `SIDESTREAM_TEST_POSTGRES_URL='<disposable-postgres-url>' npm run test:customer-360-postgres` and `SIDESTREAM_TEST_POSTGRES_URL='<disposable-postgres-url>' npm run test:acquisition-journey-matrix-postgres` against the approved disposable database. Confirm first-install and first-purchase selectors remain independent, signed cursor filters cannot be changed between pages, all ten stage grains deduplicate, exact Stripe lookup stays privacy-safe, historical nulls are never inferred, and deterministic first-write contention converges without quarantine.
...
- After changing the guarded paid-telemetry selector, run the default, pending-review, reviewed-path, legacy-entitlement, and `npm run replay:paid-telemetry-handoff -- --expect-unowned-commerce-repaired` disposable modes. The fifth replay must prove exactly one verified zero/unowned Checkout fact becomes the one positive attached survivor fact with paid/upgrade timing from paid completion and refreshed totals, while provider provenance and legacy entitlement/email placeholders remain unchanged. It must also prove second keys/facts, wrong source/currency/evidence, nonzero or owner mismatch, conflict/refund/dispute/inquiry, missing positive paid truth, and failed totals refresh refuse atomically. All repaired forms must reach `already_repaired` and replay as no-ops.
