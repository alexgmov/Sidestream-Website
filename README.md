...
The pricing headline intentionally sits halfway between the Preview demo video and the pricing cards: `#pricing` overrides the shared section top padding to `32px`, while `.pricing-head` uses a larger bottom margin to keep the cards in place. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored.
...
- Confirm the top `.feature-glass` separator line leaves enough vertical breathing room before the first Search demo video.
