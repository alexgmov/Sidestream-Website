...
At `520px` and below, the final phone override supersedes the earlier crop values with a compact `clamp(260px, 32vh, 300px)` wrapper, a `min(590px, 124vw)` video, and a `60px` wrapper lift. The headline and inline mobile handoff rise `88px` over the recording's dark lower fade, while `48px` bottom padding keeps the full email card visible at `390x844` with no horizontal overflow.
...
- At `520px` and below, confirm the compact recording ends in its lower fade above the headline, the full mobile handoff remains visible at `390x844`, the shader has no horizontal viewport seam, and desktop geometry remains unchanged at `1440x900`.
