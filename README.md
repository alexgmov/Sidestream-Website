...
- `index.html` - Canonical page implementation served at `/`. Contains the shader mount root, header, hero, direct Windows download CTA, dormant historical waitlist modal, feature sections, pricing, final CTA, footer, styles, rotating-word script, toast behavior, crawler metadata, and structured data.
...
- `account.html` - Minimal noindex account bridge with a headline-only intro. Uses the server auth session to show plan status, coarse active-production-device status, explicit device deactivation, latest installer, sign out, and a Manage Billing button that creates a Stripe Customer Portal session.
- `docs/single-device-entitlements.md` - Device-domain and support reference for the single-active-device contract, privacy boundary, API/page states, and conceptual support decisions. Its obsolete production command surface has been removed in place; it links to the canonical runbook and authorizes no production action.
- `docs/api-hardening-runbook.md` - Sole production migration/cutover procedure and exact hardened API/release contract, shared Postgres and migration model, Stripe/lead/maintenance operations, bounded configuration, metrics and alerts, protected immutable Production-artifact rules, complete WAF order, ordered human gates, and fix-forward/fallback procedure. It explicitly identifies every missing implementation control as a pre-production blocker and does not claim production was changed.
...
- `scripts/apply-postgres-migrations.mjs` - Checksummed, advisory-locked migration runner for all SQL files under `db/migrations/`, with `--status`, `--validate`, `--dry-run`, explicit verified `--baseline`, and atomic migration-plus-ledger transactions.
- `scripts/verify-migration-baseline.mjs` - Read-only exact catalog/RLS verifier for recognized pre-20260713 production profiles; use it before the migration runner's human-gated `--baseline`, never to bless unexplained drift.
...
- `scripts/audit-legacy-subscriptions.mjs` - Read-only-by-default Stripe/Product/Price inventory plus explicitly confirmed direct-database backfill/quarantine for exact allowlisted legacy subscriptions.
- `scripts/audit-license-devices.mjs` - Read-only-by-default pseudonymous fleet audit plus an explicitly confirmed, direct-connection production backfill mode.
- `scripts/manage-license-device.mjs` - Account/namespace-scoped support view, binding clear, and bounded expiring move-limit override. Mutations require `--apply`, a reason/operator ID, a non-pooling database URL variable, and an additional production confirmation.
...
- `scripts/report-installer-referrals.mjs` - Maintainer-only aggregate report for a Gmail installer campaign. It reports request, likely-scanner, likely-human, and unique daily likely-human request counts by batch without returning raw request hashes.
...
- Hero - `#hero`, `.hero-split`, `.hero-copy`, `.hero-title-line`, `.rotating-copy`, `.rotating-word`, and `.hero-subline`
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-purchase]`, and `#toast`; Mac download CTAs point at `https://sidestream.tv/api/download` with the Apple platform mark, the hero Windows CTA points at `https://sidestream.tv/api/download?platform=win32-x64` with the Windows mark, and Sidestream Pro entry points open the no-store `/api/checkout/start` confirmation page. Stripe is reached only after the user submits the signed same-origin POST.
...
- Account/auth/billing/device entitlement - `account.html`, `thank-you.html`, `upgrade.html`, `api/_lib/account.ts`, `api/_lib/entitlement.ts`, `api/_lib/device-policy.ts`, `api/_lib/license-environment.ts`, `api/auth/*`, `api/checkout/*`, `api/billing/*`, `api/stripe/webhook.ts`, `api/activation/*`, `api/account/device.ts`, and `api/license/*` own optional Google account management, the server-owned $9.99 one-time Sidestream Pro Product/Price, confirmed Checkout intents, one active production device per account, restricted Test isolation, refund/dispute lifecycle, confirmed transfers, download authorization, deactivation, and device-bound access/refresh credentials. The API/operator contract is `docs/api-hardening-runbook.md`; device/support details are in `docs/single-device-entitlements.md`.
- API operations - `api/_lib/postgres.ts` owns the shared bounded runtime pool; checksummed migrations own schema changes; `api/_lib/stripe-events.ts` owns durable claimed Stripe work; `api/_lib/maintenance.ts` owns bounded cleanup/redaction; `vercel.json` schedules all three `CRON_SECRET`-protected internal routes. Customer reads never run migrations or drain event backlog.
...
| `/api/auth/google/start`, `/api/auth/google/callback` | `GET` | OAuth redirects and server session creation |
...
The current public Mac artifact, `Sidestream-1.0.12-Mac-Installer.dmg`, is 226,402,945 bytes locally, about 216 MiB. Windows beta EXEs are about 59 MiB. Use the live Vercel Usage view rather than stale Hobby math before adding artifacts or estimating a launch's transfer cost.
...
The SaaS/account flow is server-owned. Google OAuth may create a server session, but sign-in is not required before purchase. `GET /api/checkout/start` creates or resumes only a 24-hour database intent and renders a signed confirmation whose POST token lasts 10 minutes. It never creates a Stripe Customer, Product, Price, or Checkout Session. Confirmed same-origin `POST /api/checkout/create` is bounded to 8 requests per intent and 20 per IP per 15 minutes, locks and reuses the intent, then creates one `mode=payment` card Session for one quantity of the exact Sidestream Pro Product/Price. The success callback re-fetches the literal `{CHECKOUT_SESSION_ID}` and verifies payment, line item, quantity, Price, Product, activation metadata, and attachment before fulfillment.
...
`scripts/apply-postgres-migrations.mjs` owns an advisory-locked SHA-256 ledger in `public.sidestream_schema_migrations`. `npm run db:migrate -- --status` is authoritative for every applied/pending filename in the complete chain and fails on a tracked ledger/local checksum mismatch, but its output does not print checksum values. `--validate` checks local files only. The production runbook therefore pairs status with a separate secret-safe read-only export of local and ledger checksums. A non-empty legacy schema requires a verified explicit `--baseline`; `scripts/verify-migration-baseline.mjs` is only the narrower known-catalog/conditional-RLS guard and does not enumerate every later hardening migration. Applying commits each pending SQL file and ledger row together. Runtime handlers never create or alter schema. The final migration removes the redundant unique `lead_key` constraint while preserving canonical `(email, cta_source)` uniqueness and a non-unique lookup index.
...
| Limiter/lead | `SIDESTREAM_RATE_LIMIT_HASH_SECRET` and `SIDESTREAM_LEAD_HASH_SECRET` are stable server-only HMAC values of at least 32 characters; `SIDESTREAM_DOWNLOAD_LEADS_BLOB_PREFIX` selects the private fallback prefix. Pro WAF is a per-region fixed-window counter: the trailing boundary burst is approximately `2 * L * R` for regional limit `L` across reachable regions `R`, plus reconciliation risk. Require a rejecting rule and an approved measured exposure, or a durable shared fallback limiter. |
...
| Stripe lifecycle | Retry/backoff is fixed and bounded in code; legacy recurring access requires exact comma-separated `SIDESTREAM_LEGACY_SUBSCRIPTION_PRODUCT_IDS` and `SIDESTREAM_LEGACY_SUBSCRIPTION_PRICE_IDS`. `refund.failed` handling/recovery and complete current dispute-status mapping remain production blockers. |
| License continuity | When `SIDESTREAM_LICENSE_HASH_SECRET` is absent, device hashing falls back to the first configured value from `SIDESTREAM_POSTGRES_URL`, `SIDESTREAM_POSTGRES_PRISMA_URL`, `POSTGRES_URL`, or `POSTGRES_PRISMA_URL`; the runtime trims/selects and URL-normalizes that connection value first. The runbook requires secure capture of the exact resulting bytes into the dedicated variable before any URL/pool change, with no further normalization, and the same real device/token canary before and after promotion. |
...
Apply the Postgres download-lead schema and migrate existing Blob leads only with
a command-specific env file whose database target and live waitlist count have
been verified:

```bash
env -i PATH="$PATH" HOME="$HOME" \
  SIDESTREAM_DB_ENV_FILE=/absolute/path/to/verified-lead-migration.env \
  npm run leads:migrate
```

Do not use the adjacent `website/alexg/.env.local` as a production Sidestream database source. It has previously resolved to a stale store with no current Windows waitlist rows. Vercel connector secrets may also be unavailable to local env pulls, so production migration work must fingerprint the runtime database before mutation.

Inspect and apply the checksummed migration chain through a reviewed direct URL:

```bash
env -i PATH="$PATH" HOME="$HOME" SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --status
env -i PATH="$PATH" HOME="$HOME" SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --validate
env -i PATH="$PATH" HOME="$HOME" SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate -- --dry-run
env -i PATH="$PATH" HOME="$HOME" SIDESTREAM_DB_ENV_FILE=/absolute/path/to/direct-db.env npm run db:migrate
```

The env file should expose the verified `SIDESTREAM_POSTGRES_URL_NON_POOLING` and must remain ignored. The runner takes a global advisory lock, verifies SHA-256 checksums against `public.sidestream_schema_migrations`, and commits each migration plus its ledger record atomically. An existing non-empty schema without the ledger requires independent backup/catalog review and explicit `--baseline`; never baseline merely to silence drift. Follow the ordered human gates in `docs/api-hardening-runbook.md` before any production status, baseline, apply, or legacy subscription backfill.

Report the current Windows Gmail campaign after the tracking migration is applied:

```bash
env -i PATH="$PATH" HOME="$HOME" \
  SIDESTREAM_DB_ENV_FILE=/absolute/path/to/verified-sidestream-db.env \
...
`devices:audit` is read-only unless `--apply` is explicit. `devices:manage view` is always read-only. Backfill, clear, and override mutations require an explicit target, non-pooling database URL variable, confirmation flags, the device-specific safety rules in `docs/single-device-entitlements.md`, and the production gates in `docs/api-hardening-runbook.md`; do not treat a successful help/self-test invocation as production authorization. The API hardening runbook is the sole authority for production migration and cutover ordering.
...
- Confirm the decorative `.feature-corner-demo-video` keeps the visible Premiere panel's top-left corner within `1px` of `50vw 50vh` at multiple desktop window sizes, plays with `screen` blend and `0.9` opacity, keeps `.feature-glass` unmoved, and is hidden and paused at `900px` and below.
...
- When auditing old lead data, confirm `npm run leads:dump` still includes prior Windows waitlist submissions with `cta_source` equal to `windows-waitlist`.
- With Vercel dev and account env configured, confirm `GET /api/checkout/start` renders confirmation and makes no Stripe write; only its signed same-origin `POST /api/checkout/create` creates/reuses one-time Checkout with invoice creation and persists the activation's exact Session before redirect. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, cancelled Checkout resumes/rotates intentionally, `/account.html` can still sign in with Google, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
...
- Mac download CTAs use a `[data-download]` button override that wins over primary/secondary button classes: white capsule background, a black Apple platform mark, black text, and red hover fill with white hover text while preserving existing sizing. The hero uses `Download for Mac` with `aria-label="Download Sidestream for Mac"` so it reads in parallel with the adjacent Windows CTA; the pricing and final CTAs retain `Free Download` with `aria-label="Free Download for Mac"`. The header intentionally has no download CTA. All visible Mac download CTAs should point at `https://sidestream.tv/api/download` unless the fulfillment host intentionally changes. Do not reintroduce the email modal as a blocking step without explicitly revisiting the unblocked installer strategy.
...
- `.feature-corner-demo` is a non-interactive decorative layer after `#hero` and before `.feature-glass`. Its clipping wrapper starts at `40vw` and spans `60vw`; the video sits another `10vw` into that wrapper, placing its source-space anchor at the horizontal viewport midpoint. The paired `translate(-4.75%, -13.7%)` and `transform-origin: 4.75% 13.7%` values compensate for the WebM's transparent padding before the `scale(0.7)` transform, locking the visible Premiere panel's top-left corner to `50vw 50vh`. Keep those percentages paired if the source crop changes, and do not add background fills or box-shadow mattes because the alpha WebM will reveal them.
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Pro card links to `/api/checkout/start`; server checkout price truth comes from async `getSidestreamProPriceId()` in `api/_lib/account.ts`, which validates `SIDESTREAM_PRO_PRICE_ID` when present, otherwise looks for the default Stripe Product `prod_UpwXh6oO1OmPyQ` and the `sidestream_pro_once_999` lookup key before creating the $9.99 one-time Price. If the amount changes, update the helper constants, then update visible copy, JSON-LD, `llms.txt`, and README together. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
...
- Local account/billing testing also requires Vercel dev plus env vars: a server-only Postgres connection string (`SIDESTREAM_POSTGRES_URL` preferred, `POSTGRES_URL` fallback), `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `SIDESTREAM_BASE_URL`. `SIDESTREAM_LICENSE_HASH_SECRET` must be stable and server-only. If it is absent, `api/_lib/account.ts` hashes devices with the first configured runtime URL selected from `SIDESTREAM_POSTGRES_URL`, `SIDESTREAM_POSTGRES_PRISMA_URL`, `POSTGRES_URL`, or `POSTGRES_PRISMA_URL`; `api/_lib/postgres.ts` trims/selects and URL-normalizes that value before returning the bytes used by the fallback. Before any Production URL/pool change, securely capture those exact resulting bytes and duplicate them into the dedicated variable without logging or further normalization, then prove the same existing real device/token succeeds before and after promotion. The repo has no secret-export/continuity-copy command, so cutover is blocked if the approved secret system cannot do this byte-for-byte without disclosure. The staged Production hostname may not satisfy the license host allowlist, so the post-promotion canary runs under maintenance on the real host; do not change host/env and thereby invalidate the artifact. If exact continuity cannot be proved, cutover is blocked until a tested dual-hash migration exists. `SIDESTREAM_PRO_PRODUCT_ID` defaults to `prod_UpwXh6oO1OmPyQ`; `SIDESTREAM_PRO_PRICE_ID` may override the active $9.99 one-time Price, while the default lookup key is `sidestream_pro_once_999`. `SIDESTREAM_UNLIMITED_PRICE_ID` is only a legacy migration fallback when it points at the Pro Product. `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` are only needed when testing the optional account/Customer Portal sign-in path. Use environment placeholders for Stripe keys and obtain real values from the Stripe Dashboard; rotate any secret key that has been pasted into chat before using live mode.
...
- The Postgres migration runner has a checksummed ledger and advisory lock. `npm run db:migrate -- --status` is authoritative for complete applied/pending filenames and rejects ledger/local checksum drift, but it prints no checksum values; `--validate` is local-only. Retain the runbook's separate local-plus-ledger checksum export before and after mutation. Use `--baseline` only after backup plus catalog verification of an existing non-empty pre-ledger schema. `scripts/verify-migration-baseline.mjs` recognizes only its narrower baseline/conditional-RLS profile, does not load `SIDESTREAM_DB_ENV_FILE`, and does not list all later hardening migrations. The runner, baseline verifier, and device tools currently permit remote TLS without certificate verification, so their production use is blocked until a code-owned change enforces provider CA trust plus verify-full-equivalent hostname/certificate validation. A URL fingerprint and `current_database()` alone are not authenticated database identity. Fallback keeps the migrated schema and promotes only a separately staged, Production-environment, runtime-distinct artifact qualified against the full chain; it never deletes the ledger or reverses destructive migrations.
- Migration and live-Stripe operator commands must start from an empty process environment, not a long `unset` list. The migration runner loads `SIDESTREAM_ENV_FILE` before `SIDESTREAM_DB_ENV_FILE`, and both its loader and Node `--env-file` preserve inherited values. The runbook uses command-specific mode-`0600` files, `env -i` allowlists, a direct-database target fingerprint/connection assertion, a live Stripe account assertion, and exact Product/Price allowlist assertions without printing credentials. Any inherited general env file, database URL, Stripe key, or allowlist blocks the gate.
...
- Vercel Preview/Test remains the only Stripe test-mode lifecycle proof. Before creating any Production-configured generated URL, record a project-level Deployment Protection scope that covers generated Production deployment URLs. Stage from a minimal allowlisted environment with explicit approved team/project identity, preserve distinct immutable `RELEASE_DEPLOYMENT` and `FALLBACK_DEPLOYMENT` IDs, and immediately prove each URL is protected before qualification. Immediately before promote, reassert the release ID, project, commit, build, Production environment, protection, and signed qualification evidence. Preview promotion rebuilds, and any source/config/environment change invalidates a staged artifact and requires full requalification.
- The current repository has no Production maintenance rule or operator bypass. Before every Production firewall mutation, the runbook requires a complete effective firewall/hostname/order export. The approved permanent fixed-window lead rule, temporary operator/webhook/public-read bypasses, and API deny belong to one exact reviewed order and test matrix; the permanent rule uses a rejecting action, never Log. A Vercel custom-rule bypass skips all later custom and managed WAF rules, so each tuple's actual bypass effect must be explicitly scoped, accepted, and separately tested. Tagged and untagged download GETs stay blocked because tagged GET can schedule a referral write.
- Keep the live Stripe event destination enabled during maintenance and fallback. Before enabling any webhook deny or recording a new boundary, reconcile every pre-existing Workbench Failed/Pending delivery through the compatible current artifact and require zero local due backlog/dead letters. Only then may the reviewed edge deny return non-`2xx` for the new bounded window. Live automatic retries last at most three days, Dashboard/Workbench resend at most 15 days, and Stripe CLI resend at most 30 days. Endpoint `enabled_events` and Workbench deliveries are endpoint-scoped, so reconciliation also paginates the account-level List Events API and requires a `2xx` attempt plus matching local ledger type for every unioned ID. Events created while a destination is disabled do not auto-resend.
- Vercel cron scheduling is a project-wide disable/enable control for the three routes in `vercel.json`; the repo has no one-job toggle or per-job kill switch. Keep scheduling globally disabled, invoke Stripe processing, lead replay, and maintenance sequentially through the protected operator bypass, then enable scheduling once after all pass.
...
- Checkout and lead rate limits are atomic Postgres controls, not a substitute for edge protection. The Blob fallback cannot consume the database lead limiter. Vercel Pro WAF uses per-region fixed windows, so a source can approach `2 * L * R` requests across a trailing boundary window, plus regional reconciliation risk. Before relying on fallback, require an explicit rejecting action and security approval based on measured reachable regions, or a durable shared fallback limiter; if that bound cannot be approved, cutover is blocked.
...
- Historical lifecycle reconciliation is a separate pre-cutover blocker. The repository has no tested idempotent tool that can find and repair old refund/dispute events already terminalized by the known-bad mapper while retaining exact event IDs and a watermark. Production remains blocked until that tool exists and proves its repair set, or evidence proves no historical row was affected. Manual ledger or entitlement edits are forbidden.
...
- 2026-07-14: Neutralized the obsolete single-device production procedure and hardened the sole API runbook with drain-before-deny Stripe ordering, protected immutable release/fallback identities, one reviewed WAF order and the fixed-window `2 * L * R` bound, exact legacy catalog proof, authenticated database-transport blockers, and a historical lifecycle reconciliation blocker. No production action was performed.
- 2026-07-14: Reopened the API-hardening docs after independent audit: moved the runbook's production database/live-Stripe CLI snippets to an empty allowlisted environment with target/account/allowlist assertions; added account-level paginated Stripe event enumeration unioned with endpoint deliveries; documented unresolved `refund.failed` and dispute-status blockers; corrected Vercel WAF limits to per-region scope; and made the still-executable alternate cutover document an explicit blocker. This docs-only worker made no production query, deploy, migration, Stripe/Vercel call, secret/WAF/cron/traffic change, or cutover.
...
- 2026-07-14: Consolidated the single-active-device schema, production/Test environment contract, API/page states, operator/support workflow, disposable-Postgres tests, and privacy boundary. Its original migration/cutover directions are now superseded by `docs/api-hardening-runbook.md`; no production migration, deployment, backfill, or enforcement was claimed.
