...
`config/upgrade-pricing-experiment.mjs` disables new annual assignments even with
stale rollout settings; public pricing and `/api/checkout/offer` match. Existing
...
- `account.html` - Minimal noindex account bridge on a plain near-black background. Signed-out visits immediately enter Google OAuth, while the server auth session shows returning users their plan status, coarse active-production-device status, explicit device deactivation, latest installer, sign out, a Stripe Customer Portal action, and a confirmed cancel-at-period-end action shown only for an active subscription.
- `credit-complete.html` - Minimal noindex/no-store credit Checkout return page. Success tells the customer to return to Sidestream while the panel refreshes the server wallet; cancellation states that no credits were purchased and never starts Google authentication.
...
- `api/credits/*`, `api/_lib/download-credits.ts`, `api/_lib/download-credit-response.ts`, `api/_lib/download-credit-pack.ts`, and `db/migrations/20260814120000_add_server_download_credits.sql` - Server-authoritative Free download credits. A channel-specific HMAC-hashed installation wallet receives one 1,000-credit starter grant, imports prior v1.0.19 usage only on first creation, and uses atomic 100-credit video/audio reservations with commit, release, and seven-day expiry. The append-only ledger makes retries idempotent; the client receives only balance/cost presentation data while purchases are disabled. The approved optional pack is exactly 1,000 additional credits for one-time USD $4.99, requires its own purchase flag, and rejects a configured Stripe Price with any other currency or amount. Unlimited remains the separate database-backed account/license entitlement and consumes no Free credits.
...
- `config/upgrade-pricing-experiment.mjs`, `api/_lib/upgrade-pricing-experiment.ts`, `db/migrations/20260812120000_add_upgrade_pricing_experiment.sql`, `db/migrations/20260825140000_add_annual_upgrade_pricing_experiment.sql`, and `docs/upgrade-pricing-experiment.md` - Authenticated Upgrade pricing contracts. Historical `upgrade-pricing-v1` remains closed and immutable. Production `upgrade-pricing-v2` concluded on 2026-09-04; the default was restored to `$19.99` one-time on 2026-09-16. Source disables new recurring assignments despite stale environment values; existing assignments, subscriptions, locked intents, and regional offers remain unchanged. Missing assignment/provider configuration still fails closed without creating an annual assignment.
- `api/_lib/annual-renewal-reminder.ts` and `api/internal/annual-renewal-reminders.ts` - Durable annual-renewal notice delivery for active v2 annual subscriptions. The shared cron bearer protects a bounded six-hour job that stays inert unless `SIDESTREAM_ANNUAL_RENEWAL_REMINDERS_ENABLED=true`, stages only renewals 7-30 days away, cancels stale/canceled rows, claims with short leases, retries provider failures, dead-letters terminal failures, and uses stable subscription/renewal idempotency. The email says `$19.99`, identifies the renewal date, links to Account cancellation, and confirms paid-through access.
- `api/_lib/upgrade-pricing-report.ts`, `api/internal/upgrade-pricing-report.ts`, `scripts/report-upgrade-pricing-experiment.mjs`, and `tests/upgrade-pricing-report*.test.mjs` - Protected read-only experiment measurement. The shared admin guard keeps the route POST-only, no-CORS, and no-store; repeatable-read SQL reports exact assignment/exposure, mature non-conversion, entitlement activation, lifecycle/retention, currency-isolated realized money and MRR, integrity defects, and exact activation client versions. Reports default to v2 and include only durable v2 assignments, so disabled/configuration/regional fallbacks cannot contaminate v2 conversion or revenue denominators; `--experiment upgrade-pricing-v1` preserves the ended monthly test. The local operator uses only `127.0.0.1`, privacy-safe IDs, bounded signed pagination, and explicitly separate modeled-LTV assumptions.
...
- `api/checkout/offer.ts` - Public read-only regional price-presentation endpoint used by the landing page. It selects from the same catalog and trusted Vercel country as Checkout, returns only the formatted price, currency, and `year`/`one_time` display cadence, varies by the trusted country header, and is private/no-store so regional responses cannot leak through a shared cache. It never accepts country, amount, currency, offer, Product, or Price input from the browser.
...
- `api/support/webhook.ts`, `api/internal/support/audit.ts`, `api/internal/support/process.ts`, `api/_lib/support-*.ts`, `db/migrations/20260825120000_add_support_safety_ledger.sql`, `db/migrations/20260825130000_add_support_reliability_queues.sql`, `docs/support-automation.md`, and `tests/support-automation.test.mjs` - Inert-by-default support-email intake with a signed Resend webhook, application-encrypted ticket content, deterministic prompt-injection/systematic-incident stops, tool-free strict-schema triage, and a separately prompted artifact auditor. Intake acknowledges only after the ticket and unique triage job commit together. Expiring lease-token jobs provide a durable triage backstop; flags/errors commit a unique privacy-minimal notification outbox row with their gate evidence, and bounded retry/backoff produces visible dead letters plus append-only delivery attempts. The protected processor claims at most 25 jobs/alerts per class, can requeue only one exact dead-letter UUID per request, and always returns `executed:false`. The audit schema admits bounded PR metadata or registered support-table operations, rejects raw SQL and core product scope, and this release contains no generic executor, customer-reply sender, merge, deploy, or database-write worker.
...
- Canonical acquisition integrity - `api/acquisition/*`, `api/_lib/acquisition-integrity.ts`, Checkout/auth/fulfillment helpers, `api/internal/customers/lookup.ts`, and `api/_lib/acquisition-funnel.ts` make one private acquisition UUID the durable root from first website or signed delivery entry through Google, Checkout, payment, refund/dispute, install claim, and Customer 360 reporting. `website`, `manychat_email`, and `facebook_lead_form` are server-owned immutable entry channels; a delayed signed-email click keeps the handoff's original timestamp and channel when the restored browser cookie observes the landing page. `website_direct_or_unknown` means the website entry is exact but the external origin was unavailable. It never hides missing internal linkage. Every new Checkout intent requires the UUID, Stripe Session/Invoice/PaymentIntent metadata must agree, and operator reads expose privacy-safe stage summaries, integrity alerts, signed journey pagination, and exact `cus_`/`cs_`/`pi_`/`ch_` lookup without returning those identifiers. Root and child-stage writes always acquire the acquisition root lock first so simultaneous landing and authenticated Upgrade requests converge instead of deadlocking.
...
- Support automation safety - `api/support/webhook.ts`, `api/internal/support/audit.ts`, `api/internal/support/process.ts`, and `api/_lib/support-*.ts` keep inbound email separate from the personal inbox and make two independent gates durable. Gate 1 holds back HTML/attachments, stops deterministic injection or systematic signals, and gives plain-text customer content only to a tool-free strict-schema model as untrusted user data. Every accepted ticket has one leased triage job; every Gate 1/Gate 2 flag or error has one transactionally enqueued alert with leased delivery, bounded retry/backoff, append-only attempts, and visible dead-letter state. Gate 2 accepts only exact PR or registered support-table artifacts, deterministically refuses core files/tables and dependencies, then asks a separate tool-free model to audit product impact. An audit pass is only ledger state; no action can execute in this release. The full processor, activation, canary, recovery, and rollback contract is `docs/support-automation.md`.
- Download credit ledger - `api/credits/sync.ts`, `api/credits/reserve.ts`, `api/credits/finalize.ts`, `api/credits/purchase.ts`, `api/_lib/download-credits.ts`, and the credit migration own server balance truth for Free installations. Wallet routes remain inert unless `SIDESTREAM_DOWNLOAD_CREDITS_ENABLED=1`; sync, reservation, and terminal transitions are rate-limited and fail closed; duplicate request keys converge without double spend or double grant. Raw installation identity and Stripe secrets are never stored in the ledger. Purchase Checkout and pack presentation additionally require `SIDESTREAM_CREDIT_PURCHASES_ENABLED=1`, so wallet rollout does not activate top-ups.
...
- Acquisition and retention report - `api/_lib/acquisition-funnel.ts` and `api/internal/customers/funnel.ts`; an authenticated non-browser caller selects a namespace, independent `first_install` or `first_purchase` cohort basis, bounded cohort, and later completed UTC-day observation boundary. The response contains canonical-root and deterministic legacy attribution groups, distinct counts for all ten stage grains, explicit integrity alerts, source totals capped at 100, and signed `(cohortAt, customerId)` journey pages capped at 100. Every first-open/account-activation/product-activation/return/one-and-done and attribution-coverage value exposes its exact numerator and denominator. Product activation means a successful download on the first-open UTC day divided by all cohort profiles; usage modes split opened profiles into zero, one, or two-plus accepted first-day attempts. Overall stickiness continues to use all install IDs rather than only attributable profiles.
- Meta creative ROAS - `middleware.ts`, `api/_lib/meta-roas-report.ts`, `api/internal/meta-roas-report.ts`, `scripts/import-meta-ad-spend.mjs`, and `20260831120000_add_meta_ad_spend.sql`; one safe `utm_content={{ad.id}}` creative key survives the deterministic Meta route while the default/paid experiment cohort remains separate. The protected observed report maps exact Checkout-linked live customer profiles to positive verified net money, joins operator-imported daily Meta spend, and calculates currency-isolated ROAS/CAC. Missing creative keys, missing spend, spend without acquisitions, and zero denominators stay explicit. See `docs/meta-creative-roas.md` before launching or judging a Meta creative.
...
- API operations - `api/_lib/postgres.ts` owns the shared bounded runtime pool; checksummed migrations own schema changes; `api/_lib/stripe-events.ts` owns durable claimed Stripe work; `api/_lib/maintenance.ts` owns bounded cleanup/redaction; `vercel.json` schedules all five `CRON_SECRET`-protected internal routes, including annual renewal reminders. The support webhook is provider-triggered, while the support audit and bounded queue processor use the separate support-admin bearer. The active Hetzner service must call the processor once per minute after activation; Vercel cron does not own that support timer. Customer reads never run migrations or drain event backlog.
...
| `/api/credits/purchase` | `POST` | `200` with an exact Stripe Checkout URL only when the requested server-advertised 1,000-credit/$4.99 USD pack is fully configured; successful/cancelled browser returns use `credit-complete.html` without Google authentication, otherwise `503 credit_purchases_unavailable` |
...
| `/api/billing/subscription/cancel` | `POST` | Schedules the signed-in account's exact active subscription to cancel at period end after Stripe customer ownership verification |
| `/api/stripe/webhook` | `POST` | `200 {"received":true}` after durable insert; duplicate adds `"duplicate":true` |
| `/api/support/webhook` | `POST` | Disabled unless the complete support configuration is valid; verifies the raw Resend signature, retrieves only the addressed inbound email, persists encrypted content before acknowledging, and schedules Gate 1. Duplicate delivery converges; HTML/attachments are withheld. |
...
| `/api/internal/annual-renewal-reminders` | `GET` | `CRON_SECRET`-protected bounded annual-renewal email job. It returns `{ok:true,outcome:"disabled"}` unless the literal `SIDESTREAM_ANNUAL_RENEWAL_REMINDERS_ENABLED=true` is set after migration/provider qualification; enabled runs return `{ok,outcome,staged,canceled,accepted,retryable,deadLetter}`. |
...
| `/api/internal/meta-roas-report` | `POST` | Same protected non-browser boundary; returns exact Checkout-linked Meta creative purchases, current verified net revenue, imported daily spend, ROAS, and CAC without cross-currency totals |
| `/api/internal/support/audit` | `POST` | `SIDESTREAM_SUPPORT_ADMIN_SECRET`-protected, non-browser Gate 2 for an exact PR or registered database artifact; returns verdict, human-approval state, and `executed:false` |
| `/api/internal/support/process` | `POST` | Same support-admin boundary; claims bounded due/expired-lease triage jobs and notification-outbox rows, reports retry/dead-letter counts, optionally recovers one exact dead-letter UUID, and always returns `executed:false` |
...
The five scheduled internal routes require `Authorization: Bearer
<CRON_SECRET>`. The protected Customer 360 list/detail/funnel and Meta ROAS routes instead
require `Authorization: Bearer <SIDESTREAM_CRM_ADMIN_SECRET>`. The support audit
and processor use the separate `SIDESTREAM_SUPPORT_ADMIN_SECRET`; they never accept the CRM or
cron credential. The shared cron
token must be 16-512 printable, non-space ASCII characters (`U+0021`-`U+007E`)
so all five scheduled route validators accept the same header; generate 32
...
ten minutes, maintenance daily at `04:13` UTC, Customer 360 usage sync daily at
`05:27` UTC, and the default-disabled annual reminder probe every six hours at
minute `17`.
...
```text
sidestream/1.0.21/Sidestream-1.0.21-Mac-Installer.dmg
```
...
The authenticated session exposes only `billing.hasSubscription` and `billing.canCancelSubscription` booleans; the Stripe Subscription ID stays server-side. `account.html` renders **Cancel subscription** only when the selected canonical license is an active subscription that is not already scheduled to end. After confirmation, `POST /api/billing/subscription/cancel` loads the server-owned Subscription ID, verifies that Stripe still reports the signed-in account's exact Customer, and sets `cancel_at_period_end=true`. It never accepts an account, Customer, or Subscription identifier from the browser. The response updates the account page immediately; the normal Stripe webhook remains the durable lifecycle path and later renders `Unlimited - cancels <date>`. One-time purchasers retain receipts/portal access but never receive the cancellation control.

The Unlimited branch of the in-panel Upgrade chooser is exactly:
...
`GET /api/checkout/offer` applies that same trusted-country selection without creating an intent or calling Stripe. It returns only `{ formattedPrice, currency, billingCadence }`, sets `Cache-Control: private, no-store`, and ignores query/body inputs. The canonical page renders `$19.99 one-time` as its resilient global/SEO fallback and hides annual-only terms. The public offer endpoint advertises `one_time`; a configured regional offer replaces only the amount. Historical account assignments can still carry their original recurring terms, disclosed in Checkout. The generated pay-first HTML remains a separate one-time surface. Checkout still resolves and snapshots its offer independently on the server, so changing or forging the displayed text cannot select a country, Price, currency, amount, or billing model.
...
Canonical paid access requires `entitlement_status=active` on `sidestream_pro` or compatible `sidestream_unlimited`. Partial refund remains active; full refund revokes; and an exact `refund.failed` event re-fetches the Refund, Charge, and PaymentIntent before restoring access only when current Stripe truth shows the refunded amount fell below the paid amount. Open inquiry/dispute statuses suspend, unknown future statuses fail closed, `won`, `warning_closed`, and `prevented` restore active paid truth, and `lost` remains deliberately irreversible. The Stripe-created-at plus event-ID watermark prevents stale Checkout, refund, dispute, or subscription events from resurrecting a later state. The current-event code gap is closed, but Production lifecycle cutover remains blocked on the separately documented historical reconciliation tool/evidence and live webhook selection. Legacy recurring access remains default-deny and requires an exact Product and Price in the two reviewed allowlists.
...
`scripts/apply-postgres-migrations.mjs` owns an advisory-locked SHA-256 ledger in `public.sidestream_schema_migrations`. Database-backed `--status` is authoritative for every applied/pending filename in the complete chain and fails on tracked ledger/local checksum mismatch. Connected modes enforce exact named selectors, authenticated remote TLS, one connection, database/port/namespace attestation, and operation-bound fingerprints; Production baseline/apply also require exact operation and target confirmations. `--validate` and `--dry-run` are strictly local file checks and are not Production-state evidence. A non-empty legacy schema requires a separately reviewed explicit `--baseline`; `scripts/verify-migration-baseline.mjs` is only the narrower known-catalog/conditional-RLS guard. Applying commits each pending SQL file and ledger row together. Runtime handlers never create or alter schema. The ordered tail includes `20260729120000_add_regional_checkout_offer_snapshots.sql`, `20260731120000_add_anonymous_acquisition_sessions.sql`, `20260803120000_add_acquisition_integrity.sql`, `20260812120000_add_upgrade_pricing_experiment.sql`, `20260814120000_add_server_download_credits.sql`, `20260825120000_add_support_safety_ledger.sql`, `20260825130000_add_support_automation_reliability.sql`, and `20260825140000_add_annual_upgrade_pricing_experiment.sql`. Credit, support, and annual-pricing code may deploy inert, but their enable flags must not be set before the matching final migrations are applied and checksummed. The private credit, support, and annual-reminder tables keep RLS enabled and revoke direct public/`anon`/`authenticated` access; support messages, gate runs, audit events, and reminder delivery state remain server-only.

Key hardened environment/configuration ownership:

| Area | Contract |
| --- | --- |
| Cron | One stable `CRON_SECRET`, 16-512 printable non-space ASCII characters (`U+0021`-`U+007E`), protects Stripe process, lead replay, maintenance, Customer 360 usage sync, and annual renewal reminders; use a secret-manager-generated 64-character hexadecimal token |
...
| Credit wallet | `SIDESTREAM_DOWNLOAD_CREDITS_ENABLED=1` activates wallet sync/reserve/finalize only after the migration gate. The starter grant and video/audio costs are fixed at 1,000 and 100/100. Top-up Checkout and pack presentation remain separately off unless `SIDESTREAM_CREDIT_PURCHASES_ENABLED=1`; `SIDESTREAM_CREDIT_PACK_PRICE_ID` must identify an active one-time USD $4.99 Price and `SIDESTREAM_CREDIT_PACK_CREDITS` must equal `1000`. The browser label is fixed to `1,000 more credits` and `$4.99 one-time`, so environment text cannot change the approved offer. Missing/malformed settings keep purchases unavailable. The live catalog entry is Product `prod_V8htjvzfhobt5j` with Price `price_1U8QTYDFKjeGlioXdrQNTHNf`; its configuration can remain staged without exposing purchases. Production wallet rollout must keep the purchase flag absent until the separately reviewed refund/dispute adjustment policy is qualified. |
| Support safety | `SIDESTREAM_SUPPORT_ENABLED=1` activates intake only when the exact inbound/alert/from mailboxes, Resend API/webhook secrets, OpenAI key and bounded models, stable 32+ character data-encryption secret, and separate 32+ character admin bearer are all valid. These values belong on the active Hetzner Website service. Public support links must not change until signed inbound delivery, encrypted storage, benign/injection/systematic canaries, both audit outcomes, idempotent replay, and exception-only alerts are live-proven under `docs/support-automation.md`. |
| Stripe lifecycle | Caught processing failures use fixed backoff and dead-letter at attempt 8, but process termination followed by lease reclaim has no claim-side attempt cap and can increment/reclaim indefinitely. A tested total-attempt terminal cap is a Production blocker. Legacy recurring access requires exact comma-separated `SIDESTREAM_LEGACY_SUBSCRIPTION_PRODUCT_IDS` and `SIDESTREAM_LEGACY_SUBSCRIPTION_PRICE_IDS`. Current `refund.failed` recovery and dispute-status mapping are implemented; historical lifecycle reconciliation and verified live webhook selection remain separate gates. |
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Free" and says "3 free downloads every day." The public Unlimited card renders `$19.99 per year` with automatic-renewal, 30-day reminder, and cancellation terms for the selected global annual offer. Trusted India, Brazil, and South Korea traffic with configured regional Prices instead renders its regional amount as `one-time` and hides annual-only terms. Its link still opens `/api/checkout/start`, authenticates the user, and opens server-selected Stripe Checkout. The separate pay-first acquisition landing keeps its `One-time` pill because that flow remains outside the authenticated Upgrade decision.
...
observed realized money. The default report is `upgrade-pricing-v2`; pass
`--experiment upgrade-pricing-v1` only for the ended one-time/monthly history.
...
`test:api` discovers every `tests/*.test.mjs` suite and fails if a Postgres suite is not explicitly classified. `test:postgres-integration` never silently skips: it requires `SIDESTREAM_TEST_POSTGRES_URL`, rejects a normalized host/port/database match with any runtime URL even when credentials/query options differ, runs serially in a random schema, rewrites the Stripe-event processor's credit-fulfillment dependency to the same schema-scoped test module, proves wallet bootstrap against real PostgreSQL parameter typing, asserts dormant pricing snapshots remain maintenance-protected without inventing an exposure, and drops that schema in `finally`. After a human runs `npx vercel@latest build`, run `npm run verify:vercel-build` to inspect `.vercel/output`; that verifier deliberately fails when no Vercel build artifact exists.
...
- Run `npm run test:support` after changing the support webhook, encryption, prompts, model payloads, schemas, ledger, queue/outbox processor, notifications, audit route, support migrations, or support runbook. It compiles the Hetzner API first, then proves deterministic injection/systematic/attachment stops, customer content in user role only, tool-free strict outputs, model-pass override, core-file/table/dependency refusals, raw-SQL rejection, authenticated encryption, signed-webhook ticket/job durability, duplicate convergence, retry/backoff, expired-lease recovery, dead-letter visibility, exact dead-letter requeue, transactionally enqueued privacy-minimal alerts, append-only delivery attempts, and POST-only non-browser audit/processor auth. Also run `npm run verify:vercel-contract` because both protected support routes are classified internal routes. Passing fixtures do not create or prove a mailbox, DNS, provider configuration, migration, timer, alert delivery, customer reply, PR, database write, Hetzner reload, or live automation.
...
- Run `npm run verify:checkout-contract` and `npm run test:entitlement` after checkout, authentication, activation-claim, account, or Stripe fulfillment changes. The source verifier proves the exact Upgrade, Google authentication, and Stripe sequence, the root-page allowlist, and both valid zero-total Stripe statuses. The entitlement command also runs the focused refund/dispute lifecycle suite, including exact failed-refund recovery, every current Stripe dispute status, event ordering, and fail-closed unknown status handling.
- Run `node --experimental-strip-types --test tests/subscription-cancellation.test.mjs tests/acquisition-route-coverage.test.mjs` after changing account subscription management. It proves authentication, one-time exclusion, exact Stripe Customer ownership, cancel-at-period-end behavior, idempotent already-scheduled responses, route inventory, and subscription-only page visibility without contacting Stripe.
...
- Run `node --experimental-strip-types --test tests/annual-renewal-reminder.test.mjs tests/upgrade-pricing-experiment.test.mjs tests/upgrade-pricing-checkout.test.mjs tests/upgrade-pricing-lifecycle.test.mjs tests/upgrade-pricing-report.test.mjs tests/upgrade-pricing-integration.test.mjs` after authenticated Upgrade experiment changes. Then run the disposable-Postgres report, Checkout abuse/concurrency, Customer 360 Upgrade, and aggregate Postgres suites. These prove permanent v1/v2 account assignment, exact monthly/annual Stripe terms, shared paid-eligibility locking, immutable snapshot/exposure lineage, first-class subscription truth and lifecycle, activation-over-exposure denominators, currency isolation, privacy, acquisition continuity, exact v1.0.11 activation lineage, and the annual reminder's bounded durable delivery contract. They do not prove a provider payment, delivered reminder email, deployed webhook, installed CEP root, or rendered dashboard.
...
- Run `npm run test:release-rollout` after changing the rollout policy, state contract, evaluator, operator flags, analytics mapping, or release-step workflow. It proves the no-data hold, one-step advance, per-step time/sample baseline, explicit cap, stale sidecar, artifact drift, analytics freshness, failure-stage, and concentration stops without contacting production or changing a manifest. Follow it with the installer-delivery checks and build for an actual manifest step.
...
- Confirm the public Unlimited card says `$19.99 per year` globally and shows automatic renewal, 30-day reminder, and account cancellation terms. From genuine India, Brazil, or South Korea egress with its configured regional Price, confirm the card instead says `₹499 one-time`, `R$25 one-time`, or `₩24,900 one-time` and hides the annual-only terms. It must always link to `/api/checkout/start`; `/api/checkout/offer` remains private/no-store and browser query parameters cannot select a region or billing model. Use a Free account without an already-open locked Checkout intent when comparing the landing presentation to a newly created Stripe Session.
...
- Support automation is deliberately inert until both support migrations, complete Hetzner-side configuration, and the one-minute protected processor timer are live-qualified. `audit_passed` never means executed, and this repository has no support executor. A `200` webhook response proves the encrypted ticket plus unique processing job committed; it does not prove triage or alert delivery. Watch the processor's current dead-letter counts and recover only an exact reviewed UUID, at most three times; never bulk-reset queue state. Never add raw email to system/developer prompts or alert mail, fetch attachment/HTML content automatically, accept free-form SQL/shell commands, reuse CRM/cron secrets, or let a model merge/deploy/write directly. Check existing MX ownership before assigning Resend to `sidestream.tv`; if it would displace working mail, use a dedicated receiving subdomain. Keep the current personal-address links until the new mailbox, queue recovery, and exception-only alerts are visibly proven.
...
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card links to `/api/checkout/start`; `$19.99 per year` plus annual terms is the static global/SEO fallback. The private/no-store `/api/checkout/offer` response may replace `[data-checkout-offer-price]`, change only the bounded display cadence, and hide annual-only lines for an approved regional one-time offer. Checkout truth still comes independently from `api/_lib/checkout-offers.ts` plus the stored intent snapshot, and no value returned to the browser is accepted back as a Checkout selector. The compatibility-named async `getSidestreamProPriceId()` resolves the global catalog's exact `1999` USD amount by validating configured/default Price IDs as non-authoritative hints, then checking the expanded Product `default_price`, exact `sidestream_pro_once_1999` lookup key, and any other active matching Product Price before its idempotent create fallback. India, Brazil, and South Korea use only their configured regional Price IDs and require exact Product/currency/amount/one-time validation; they never create Prices. A stale or deleted global configured Price may fall through; an explicitly configured invalid regional Price fails closed. The public tier name is Sidestream Unlimited, while existing `SIDESTREAM_PRO_*`, `sidestream_pro`, and historical lookup-key identifiers remain stable for purchase compatibility. Change the canonical contract, run `npm run pricing:sync`, provision new immutable Stripe Prices, update regional environment IDs, and run the pricing/checkout/entitlement gates together. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
- Authenticated Upgrade assignment is account-permanent, not browser-sticky. Historical `upgrade-pricing-v1` ended on 2026-08-21; its assignments, `$4.99/month` `monthly_half` terms, open Sessions, subscriptions, entitlements, and audit history remain immutable and supported. Production `upgrade-pricing-v2` ran 50/50 from 2026-08-27 through 2026-09-04, then concluded with `$19.99/year` selected for every future eligible global-USD account. Its source-level `10000` rollout overrides stale enabled/rollout environment values, but still requires the existing assignment secret and exact annual Price; failures remain one-time and outside the annual cohort. Existing v1/v2 assignments stay immutable, and regional offers remain on their existing one-time fallback. The annual Checkout says it renews automatically, promises an email 30 days before renewal, links cancellation to the Sidestream account, and confirms access through the paid year. Its enabled six-hour reminder job stages only active v2 annual subscriptions 7-30 days before renewal, skips scheduled cancellations, and uses a durable idempotent retry/dead-letter ledger. See `docs/upgrade-pricing-experiment.md` before changing assignment, Price, disclosure, reminder, webhook, lifecycle, or rollout behavior.
...
- In `DATABASE_CUTOVER_MODE="target"`, middleware rewrites the canonical `/api/download` and `/api/releases/latest` requests to the Hetzner Website origin along with every other `/api/*` request; a new Vercel `/version.json` SHA alone therefore does not publish a release manifest. Fast-forward, rebuild, restart, and health-attest the origin at the exact pushed SHA before checking the canonical manifest and full artifact bytes. The standard manifests remain static imports in the shared resolver and explicit `data/release-manifest*.json` Vercel `includeFiles` inputs so source-mode function builds carry the same Mac and Windows metadata.
- The Mac rollout operator does not make a live release by itself. `--apply` only changes the local manifest and checked-in step baseline, and the next evaluation must hold until that exact pair is committed, pushed, deployed to both Vercel and the target-mode Hetzner origin, and observed for at least 24 hours plus 20 additional closed intents. Never delete or reset the sidecar to bypass a later-step sample gate.
...
- Current Stripe lifecycle code handles `refund.failed` through exact provider re-verification and explicitly maps every current Dispute status, including terminal `warning_closed` and `prevented`; unknown future statuses suspend access. Do not call lifecycle cutover complete until the historical reconciliation blocker is closed and the live webhook destination is verified to select the newly supported event.
...
- 2026-08-31: Published the signed, notarized, and stapled standard Mac `1.0.21` installer to `sidestream/1.0.21/Sidestream-1.0.21-Mac-Installer.dmg` and advanced the shared public download/update manifest as a noncritical 25% canary. The release serializes physical pre-byte download resolution to one request while allowing a second physical transfer after the first begins receiving bytes; the independently qualified Windows and receipt-gated paid manifests remain unchanged.
- 2026-08-31: Added creative-level Meta attribution and observed ROAS: safe ad-ID `utm_content` capture without changing the fixed default/paid experience, a private daily spend ledger and guarded CSV importer, exact Checkout-linked purchased-customer/net-revenue reporting, currency-isolated ROAS/CAC, explicit integrity gates, static-before-dynamic Hetzner route ordering so protected funnel/report routes cannot be consumed by `[customerId]`, and target-mode paid landing rewrites that reach the origin-authenticated Hetzner database API.
- 2026-08-31: Added complete customer-attribution channel/path aggregates for first-open-day usage. Each source now exposes first-install cohort profiles, successful-download product activation, first-day attempts per open, and browse-only/single-download/multi-download counts and rates; the report derives these from the full cohort rather than paginated journey rows.
- 2026-08-29: Published the signed, notarized, and stapled standard Mac `1.0.20` installer to `sidestream/1.0.20/Sidestream-1.0.20-Mac-Installer.dmg` and advanced the shared public download/update manifest at a noncritical 25% rollout; deployed the matching SHA to both Vercel and the target-mode Hetzner API origin, statically traced both manifests into the shared resolver, and verified each release function's explicit Vercel file map while the independently qualified Windows manifest remained unchanged.
...
- 2026-08-27: Activated `upgrade-pricing-v2` in Production at rollout `5000` after applying the checksummed migration, creating and validating exact Test/live `$19.99/year` Prices, proving annual charge/renewal/cancel-at-period-end plus renewal failure/recovery and Billing Portal creation in Stripe Test, delivering the real 30-day reminder canary to the owned mailbox, and opening the exact live annual Checkout from the signed-in Free canary account without payment. The first protected report recorded one annual assignment/exposure with zero integrity defects; the one-time control, historical v1 assignments, regional offers, and fail-closed source defaults remain unchanged.
- 2026-08-27: Refreshed only the existing compatible dependency lockfile to remove the current npm advisories without changing application code or direct version ranges. The resolved tree now uses `@vercel/blob` 2.8.0 with `undici` 6.28.0 and Vite 7.3.6 with `esbuild` 0.28.2 and `nanoid` 3.3.18; `npm audit` reports zero vulnerabilities, and the API, checkout, entitlement, migration, and Production build gates remain the release proof.
- 2026-08-26: Hardened the full acquisition-to-Upgrade loop after real browser qualification exposed two timing failures. Delayed ManyChat and Facebook signed-email clicks now preserve their earlier server-owned first touch instead of quarantining it as a website rewrite; all acquisition stage writes follow root-before-stage lock ordering so simultaneous landing/authenticated Checkout starts converge; and the sandbox FREEDEV helper uses Stripe's current nested coupon-promotion contract. Disposable-Postgres regressions cover delayed browser observation and repeated concurrent authentication/landing writes.
- 2026-08-25: Staged dormant `upgrade-pricing-v2` for a future `$19.99 once` versus `$19.99/year` authenticated Upgrade test without reinterpreting the ended v1 cohort. Added the exact yearly Price contract, immutable v2 assignment/snapshot/exposure support, annual subscription fulfillment/lifecycle handling, explicit auto-renew/cancel/paid-through Checkout copy, and a durable six-hour reminder job that promises email 30 days before billing and retries with stable idempotency. The protected report now defaults to v2 and accepts an explicit v1 historical selector. Activation remains blocked on the new migration, live/Test Stripe Price and webhooks, delivered-email proof, end-to-end Test renewal/cancellation, rollout-0 control, and canary verification.
- 2026-08-25: Created the dedicated live Stripe **Sidestream Credits** Product and exact one-time 1,000-credit/USD $4.99 Price, then staged that Price in the protected Hetzner runtime without enabling credit sales. Updated the existing enabled webhook destination to `https://sidestream.tv/api/stripe/webhook` while preserving its signing secret, retained the 13 implemented checkout, invoice, subscription, refund, and dispute events, and removed unsupported `refund.failed`. Stripe Workbench showed 34 deliveries and zero failures for the week before the URL change; the Production queue had zero nonterminal or dead-letter events.
...
- 2026-08-25: Added an explicit account-page **Cancel subscription** action only for active subscription licenses. The authenticated endpoint accepts no browser billing identifiers, verifies the server-owned Subscription against the account's exact Stripe Customer, schedules cancellation at period end, preserves access until that date, updates the page immediately, and leaves one-time purchasers outside the flow.
...
- 2026-08-24: Made provider cutover and rollback health verification wait up to 20 seconds for the restarted Website service, avoiding a false failure during its normal two-second startup window while still failing closed on the requested provider or unhealthy state.
- 2026-08-24: Preserved public installer `If-None-Match`/`304` behavior across Vercel's Hetzner rewrite by copying only the bounded original validator into a middleware-owned internal header; forged copies are removed before the authenticated origin request.
