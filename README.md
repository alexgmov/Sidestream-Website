...
- `docs/single-device-entitlements.md` - Focused single-active-device contract, privacy boundary, API/page states, operator/support commands, disposable-Postgres proof, and human-gated production cutover/rollback runbook. It is a procedure, not evidence that any production rollout step occurred.
...
- `upgrade.html` - Minimal noindex checkout/cancel fallback page. Main upgrade entry points redirect directly to Stripe Checkout through `/api/checkout/start`; activation-bearing recovery enters the authenticated claim page so the account can safely choose same-device reconnect, confirmed transfer, or purchase without a duplicate charge.
...
- `api/download-lead.ts` - Vercel Node Function that accepts historical landing-page lead captures, including prior Windows waitlist and download email gate submissions, and stores each lead in server-side Postgres through the Sidestream Supabase/Vercel connector's `SIDESTREAM_POSTGRES_URL`, with generic `POSTGRES_URL` fallback if the connector env is unavailable.
...
- `api/checkout/start.ts`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - One-time Sidestream Pro Checkout entry points and server completion callback. Activation purchases use one stable idempotency key, persist the exact Session/Price/Product before redirect, and return through the callback with Stripe's unencoded `{CHECKOUT_SESSION_ID}` placeholder; no route pre-binds a browser activation to a signed-in account. A missing-activation Checkout request on the old Vercel hostname is stopped before Stripe because that is the unsafe fallback used by 1.0.12 after an activation-start failure; canonical website Checkout without an activation remains supported.
- `api/checkout/create.ts` - Backward-compatible authenticated JSON Checkout Session creator for account-page callers. It creates the same $9.99 one-time Checkout payment for an existing account customer.
...
- `api/stripe/webhook.ts` - Stripe webhook endpoint. Verifies signatures, stores idempotency rows, and updates Sidestream license state from completed one-time Checkout payments while retaining legacy subscription-event handling.
...
- Download lead capture - `api/download-lead.ts` accepts `POST /api/download-lead` JSON with an email, optional page, and optional source, captures the request IP from proxy headers, then writes to Postgres table `public.sidestream_download_leads` through a server-only Postgres connection string. Historical hero waitlist submissions use `cta_source = "windows-waitlist"` so their count can be queried separately from historical download emails. Accepted env names are `SIDESTREAM_POSTGRES_URL`, `SIDESTREAM_POSTGRES_PRISMA_URL`, `SIDESTREAM_POSTGRES_URL_NON_POOLING`, `POSTGRES_URL`, `POSTGRES_PRISMA_URL`, and `POSTGRES_URL_NON_POOLING`, in that order; if Postgres is not configured or the write fails, it falls back to the private Vercel Blob prefix `SIDESTREAM_DOWNLOAD_LEADS_BLOB_PREFIX` or `sidestream/download-leads`
- Account/auth/billing/device entitlement - `account.html`, `thank-you.html`, `upgrade.html`, `api/_lib/account.ts`, `api/_lib/entitlement.ts`, `api/_lib/device-policy.ts`, `api/_lib/license-environment.ts`, `api/auth/*`, `api/checkout/*`, `api/billing/*`, `api/stripe/webhook.ts`, `api/activation/*`, `api/account/device.ts`, and `api/license/*` own optional Google account management, the server-owned $9.99 one-time Sidestream Pro Price, persisted Checkout handoffs, one active production device per account, restricted Test isolation, confirmed transfers, download authorization, deactivation, and device-bound access/refresh credentials. The complete contract and cutover gates live in `docs/single-device-entitlements.md`
...
GET /api/download
HEAD /api/download
GET /api/download?platform=win32-x64
HEAD /api/download?platform=win32-x64
GET /api/releases/latest
HEAD /api/releases/latest
...
GET /api/activation/claim
POST /api/activation/claim
GET /api/account/device
POST /api/license/verify
POST /api/license/refresh
POST /api/license/authorize-download
...
The SaaS/account flow is server-owned. Google OAuth uses `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, optional `GOOGLE_REDIRECT_URI`, and `SIDESTREAM_BASE_URL`; successful sign-in stores an HTTP-only `sidestream_session` cookie backed by `public.sidestream_account_sessions`, but Google sign-in is not required before checkout. Stripe Checkout uses `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, the Sidestream Pro Product ID from `SIDESTREAM_PRO_PRODUCT_ID` or the default `prod_UpwXh6oO1OmPyQ`, and optional `SIDESTREAM_PRO_PRICE_ID`. When no explicit price is configured, the checkout resolver uses the active $9.99 one-time Price with lookup key `sidestream_pro_once_999`, creating it for the Sidestream Pro Product if needed. Direct upgrade links call `GET /api/checkout/start`, which creates a hosted `mode: "payment"` Checkout Session for a $9.99 one-time purchase, adds the Stripe-page custom text `One-time payment. No subscription.`, enables invoice creation, and returns through the server-owned `/api/checkout/complete` callback before `thank-you.html`. Activation Checkout is valid inside the 24-hour activation window: the exact Session, expected Price, Product, Stripe expiry, and 10-minute completion grace are persisted before redirect under a stable activation idempotency key. Webhook, completion callback, and activation-status fallback all converge on the same locked helper, which re-retrieves Stripe truth and requires the exact attached Session, `mode=payment`, paid/no-cost payment status, one expected line item, quantity one, expected Price/Product, and matching activation metadata before binding. The webhook remains primary; status reconciliation is rate-limited and only runs after exact device-hash validation. The old `SIDESTREAM_UNLIMITED_PRICE_ID` remains a migration fallback only when it points at the same Pro Product. Stripe-created customers are linked to existing Sidestream accounts by customer ID or normalized paid Checkout email; otherwise verified fulfillment creates a Stripe-first account row. New sessions use `sidestream_pro` while legacy `sidestream_unlimited` remains accepted for compatibility. Customer Portal and `/api/billing/receipt` remain the account-management surfaces.

Plugin activation rows are device-bound. `/api/activation/status` issues one deterministic, retry-safe credential family only after verified payment or an explicit restore. Current clients may recover that family for 10 minutes after completion; legacy clients through 1.0.13 receive the same `active` response throughout the activation's 24-hour lifetime because they do not understand the terminal `completed` state. Current-client access tokens last seven days. Tokens whose database-linked activation records are from legacy clients through 1.0.13 receive a 365-day access lifetime and `/api/license/verify` rolls that expiry forward, because those clients cannot retain or rotate the paired refresh credential; this decision never trusts a spoofable request user agent. The paired opaque refresh token is hashed at rest, bound to the same device, rotates atomically through `/api/license/refresh`, and has a rolling 365-day expiry. A two-minute predecessor-hash window returns the same derived rotated pair after one lost response or concurrent retry without accepting the old credential indefinitely. `/api/license/verify` and `/api/license/refresh` return 401 codes `invalid_token`, `revoked`, or `device_mismatch`, and 403 `license_inactive`; callers should retain credentials on transient failures. Restore uses `/api/activation/claim`: unauthenticated GET redirects through Google with an allowlisted next path, authenticated GET renders a no-store confirmation, and only an active-license, same-origin, CSRF-valid POST may CAS-bind a fresh activation whose account is still null or identical. Both Checkout entry routes send an already-signed-in active Pro account with an activation key to that confirmation instead of offering a second purchase. Do not store Stripe secrets, Google client secrets, raw payment data, activation keys, license tokens, refresh tokens, or permanent paid-state in browser code or logs.
...
The pricing headline intentionally sits halfway between the bottom of the `.feature-glass` band and the pricing cards: `#pricing` overrides the shared section top padding to `92px`, while `.pricing-head` uses a matching `92px` bottom margin so the cards stay in place. The mobile override uses a matching `74px` top padding and bottom margin. `.pricing-line` keeps "Unlock when you need more." on its own lighter-weight line. The two pricing cards use a larger `28px` corner radius and a pricing-only `IntersectionObserver` that adds `html.pricing-motion-ready` plus `.is-visible` so the cards glide up once before they fully enter the viewport; no global `.reveal` behavior is restored. The $0 card is labeled "Free" and says "5 free downloads every day." The Pro plan is a visible `$9.99 once` one-time upgrade that links straight to `/api/checkout/start` with no account requirement before Stripe Checkout.
...
The generic migration runner is idempotent SQL but has no migration ledger and replays every file. For production, use a verified server-only connection and apply only the intended new migration inside one transaction when unrelated migrations should not be replayed.
...
`devices:audit` is read-only unless `--apply` is explicit. `devices:manage view` is always read-only. Backfill, clear, and override mutations require an explicit target, non-pooling database URL variable, confirmation flags, and the human gates in `docs/single-device-entitlements.md`; do not treat a successful help/self-test invocation as production authorization.
...
- Confirm the Pro card says `$9.99 once`, links directly to `/api/checkout/start`, and does not route users through the old Google-first upgrade interstitial.
...
- With Vercel dev and account env configured, confirm `/api/checkout/start` redirects directly to one-time Stripe Checkout with invoice creation enabled, persists the activation's exact Session before redirect, successful Checkout passes through `/api/checkout/complete` and then reaches `/thank-you.html`, `/upgrade.html?checkout=cancelled` renders the retry fallback, `/account.html` can still sign in with Google, `/api/billing/receipt` returns the latest receipt, `/api/stripe/webhook` accepts signed Stripe CLI events, and `/api/billing/portal` opens Customer Portal.
...
- The Postgres migration runner has no migration ledger, so `--dry-run` lists files rather than proving live schema status. Follow the catalog checks and backup gate in `docs/single-device-entitlements.md`. Rollback changes `SIDESTREAM_DEVICE_POLICY_MODE` back to `observe`; it does not reverse the additive migration or resurrect credentials already marked `device_replaced` or `device_deactivated`.
...
- If a successful purchase still shows Free in the account page or plugin, check Vercel logs for `/api/stripe/webhook`, `/api/checkout/complete`, and `/api/activation/status`, then confirm every billing migration through `20260713180000_add_activation_checkout_and_refresh_rotation.sql` is applied. Runtime routes intentionally do not execute schema-altering DDL. The status fallback retrieves only the activation row's persisted exact Checkout Session and is cooldown-limited after device validation; it cannot repair a missing migration or an unattached Session.
...
- 2026-07-14: Consolidated the single-active-device schema, production/Test environment contract, API/page states, operator/support workflow, disposable-Postgres tests, privacy boundary, and ordered human-gated cutover/rollback runbook. Documentation does not claim that production migration, deployment, backfill, or enforcement occurred.
