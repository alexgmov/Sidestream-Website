...
- `config/upgrade-pricing-experiment.mjs`, `api/_lib/upgrade-pricing-experiment.ts`, `db/migrations/20260812120000_add_upgrade_pricing_experiment.sql`, and `docs/upgrade-pricing-experiment.md` - Authenticated `upgrade-pricing-v1` contract. A server-only HMAC bucket plus immutable account assignment selects the unchanged trusted-country one-time offer or an exact monthly subscription at the audited half-price ending. The client selects no commerce truth; active owners remain ineligible; assignment/exposure rows contain no direct identity or payment secrets; a shared account lock closes entitlement-versus-Checkout races; and assignment/config/Price failures use an observable one-time fallback.
...
- `api/checkout/start.ts` and `api/checkout/complete.ts` - Authenticated Sidestream Unlimited Checkout flow. Start sends signed-out users through Google authentication, rejects active paid owners under the same account eligibility lock used by entitlement grants, resolves an approved offer from the trusted edge country, persists the permanent experiment assignment when enabled, and stores the complete Product/Price/billing/assignment/acquisition/activation snapshot on the locked intent before creating or reusing Stripe Checkout. Control retains the exact one-time request contract and customer copy; `monthly_half` uses only subscription-valid parameters and copies the immutable metadata onto the Subscription. Checkout Session idempotency fingerprints every Stripe request parameter plus the logical attempt, so historical/open intents never reprice. Completion verifies exact Stripe and database truth before either variant grants the same entitlement. A complete zero-total one-time order may omit its PaymentIntent while reporting either `paid` or `no_payment_required`; that narrow path retains every existing snapshot and settlement check.
...
```text
sidestream/1.0.18/Sidestream-1.0.18-Mac-Installer.dmg
```
...
The current public Mac artifact, `Sidestream-1.0.18-Mac-Installer.dmg`, is 226,516,361 bytes, about 216 MiB. The current Windows artifact, `Sidestream-1.0.16-Windows-Installer.exe`, is 61,707,154 bytes, about 59 MiB. Use the live Vercel Usage view rather than stale Hobby math before adding artifacts or estimating a launch's transfer cost.
...
- Authenticated Upgrade assignment is account-permanent, not browser-sticky. `SIDESTREAM_UPGRADE_PRICING_EXPERIMENT_ENABLED` accepts the literal string `true`; `1` and other values fail closed to the observable one-time fallback. `SIDESTREAM_UPGRADE_PRICING_EXPERIMENT_ROLLOUT_BPS` is the monthly share of only future unassigned accounts; `5000` means 50/50. Disabling the experiment sends future unassigned users through an observable one-time fallback but does not rewrite assignments, expire open Sessions, cancel subscriptions, or revoke valid entitlements. Monthly lookup keys must include currency and amount (`sidestream_pro_monthly_<currency>_<minor>`); never reuse an amount-only key or a matching Product Price without that exact key. Regional monthly Prices are explicit env configuration and global USD alone may use guarded exact-key discovery/creation. See `docs/upgrade-pricing-experiment.md` before changing rollout, Price IDs, webhooks, or lifecycle.
