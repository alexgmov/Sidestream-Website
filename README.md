...
- `api/download-lead.ts` - Vercel Node Function that accepts email-gate submissions from the download modal and stores each lead in server-side Postgres, currently Neon through `POSTGRES_URL`, with private Vercel Blob fallback if the database env is unavailable.
...
Download CTAs are email-gated in the canonical HTML. Their anchors point at the canonical public Vercel download URL, `https://sidestream-xi.vercel.app/api/download`, so local static previews and adjacent static hosts do not 404 on a relative `/api/download` path. The first click opens `#download-email-gate`; after a valid email is entered, the script POSTs to `/api/download-lead`, stores the email locally under `sidestream.download.email` so repeat clicks do not keep asking in the same browser, closes the modal, and navigates directly to the installer URL with `window.location.assign(...)`. Lead capture writes to Neon/Postgres table `public.sidestream_download_leads` through server-only `POSTGRES_URL`. The table stores the normalized email, salted email hash, capture timestamp, page, CTA source, referrer, user agent, request IP address, Blob migration pathname when relevant, and JSON context. Keep it private by using only server-side credentials; do not expose direct database access to browser code. If Postgres is not configured or a write fails, the route falls back to writing one private JSON object per submission to Vercel Blob under `SIDESTREAM_DOWNLOAD_LEADS_BLOB_PREFIX` when configured, otherwise `sidestream/download-leads/YYYY-MM-DD/`. If lead capture times out or fails after a syntactically valid email, the browser still starts the installer download so the public download path does not break over lead-storage configuration.
...
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card uses a white border, no drop shadow, a `.beta-coming` minimum height, and a crisp overlay that says "Sidestream Unlimited" coming soon; the previous `$19` paid details remain commented out inside `.plan-beta-content` for later restore. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
...
- Download-lead capture uses the server-only `POSTGRES_URL` pooler connection. Do not expose a Postgres password or `POSTGRES_URL` to `Sidestream.html`, React browser code, or the CEP plugin.
