...
- The standard release manifests are runtime inputs to both `api/download.ts` and `api/releases/latest.ts`. Keep their `data/release-manifest*.json` `includeFiles` declarations in `vercel.json`; without that explicit dependency, Vercel can reuse a prior function bundle after a manifest-only commit even while `/version.json` reports the new SHA.
...
- 2026-08-29: Published the signed, notarized, and stapled standard Mac `1.0.20` installer to `sidestream/1.0.20/Sidestream-1.0.20-Mac-Installer.dmg` and advanced the shared public download/update manifest at a noncritical 25% rollout; both standard release functions now declare the manifest files as explicit Vercel bundle inputs, and the Vercel-build verifier checks each resulting file map, so a manifest-only deploy cannot reuse stale release metadata while the independently qualified Windows manifest remains unchanged.
