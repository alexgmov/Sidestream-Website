...
- 2026-07-20: Removed the internal purchase-confirmation control. `/api/checkout/start` now renders only `Continue with authentication with Google if you haven't already.` plus a hidden signed auto-submit form; signed-out buyers carry the opaque intent through state-verified Google OAuth, existing sessions skip Google, and both enter the same locked/rate-limited Stripe worker. Restore/transfer controls remain separate. Added focused no-button, OAuth-resume, account-binding, and abuse regressions. Only the isolated Preview/Test surface may be deployed; Production remains untouched.
