...
- Confirm the Unlimited card reads as a coming-soon paid plan: the underlying `$19` plan details are subtly blurred/dimmed, the overlay text says "Coming soon" and "Sidestream Unlimited", the overlay remains readable on mobile, and clicking/tapping it shows the Sidestream Unlimited coming-soon toast instead of checkout language.
...
- Confirm the decorative `.feature-corner-demo-video` starts from the right-side midpoint wrapper on desktop, plays opaque, renders at 70% scale from the visible Premiere top-left corner, uses real oversized WebM content to avoid right/bottom shader gaps, keeps `.feature-glass` unmoved, and stays hidden and paused on mobile.
...
- `.feature-corner-demo` is a non-interactive decorative layer after `#hero` and before `.feature-glass`. Keep the wrapper at `50vw` when the demo should begin from the page midpoint, but account for the WebM's transparent alpha padding before judging the visible Premiere corner. The current treatment uses a square full-plugin/timeline WebM crop, a responsive `left` offset, `bottom: calc(clamp(-330px, -18.4vw, -230px) - 10vh)`, `width: clamp(820px, 74vw, 1450px)`, and a `scale(0.7)` transform around the visible Premiere top-left corner on `.feature-corner-demo-video`. That deliberately uses more source video, not a larger zoom of the old crop, to fill right/bottom gaps while keeping `.feature-glass` in normal flow; do not add background fills or box-shadow mattes because the alpha WebM will reveal them.
- Pricing headline placement is tuned independently from shared `.sec-pad`: `#pricing { padding-top: 92px; }`, `.pricing-head { margin-bottom: 92px; }`, and the mobile override uses matching `74px` top and bottom spacing. `.pricing-line` is intentionally `font-weight: 300`. The Unlimited card uses a white border, no drop shadow, and a `.beta-coming` overlay so the blurred background still reads as the `$19` paid plan while the crisp overlay says "Sidestream Unlimited" coming soon. The pricing-card motion should stay scoped to `#pricing .plan.reveal`, use an early positive bottom `IntersectionObserver` margin, and avoid re-enabling global `.reveal` because it was previously disabled for environment fill-mode issues.
