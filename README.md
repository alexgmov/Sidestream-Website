...
- `api/checkout/start.ts` - Public Stripe Checkout redirect for the Sidestream Pro $9.99 one-time purchase. It resolves the server-owned Price from `SIDESTREAM_PRO_PRICE_ID` or the active `sidestream_pro_once_999` lookup-key Price, carries optional plugin activation metadata, and redirects directly to Stripe without requiring Google sign-in.
...
- Account/auth/billing - `account.html`, `thank-you.html`, `upgrade.html`, `api/_lib/account.ts`, `api/auth/*`, `api/checkout/start.ts`, `api/checkout/create.ts`, `api/billing/portal.ts`, `api/stripe/webhook.ts`, `api/activation/*`, and `api/license/verify.ts` own optional Google account management, the server-owned $9.99 one-time Sidestream Pro Price resolution, direct Checkout redirects, Customer Portal redirects, webhook-owned entitlements, plugin activation, and license-token verification
...
- Vercel's `/:path*` redirect pattern preserves non-root paths but does not match the bare `/` request. Keep the explicit host-conditioned `/` rules for `www.sidestream.tv` and `sidestream-xi.vercel.app` ahead of the path-preserving catchalls, or the duplicate host homepages will silently return `200` again.
