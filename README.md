...
selectors. Follow `docs/api-hardening-runbook.md`; local `--validate`/`--dry-run`
do not close that blocker or prove Production state.
...
is implemented and reviewed. The API hardening runbook is the sole Production
authority.
...
- The Postgres migration runner has a checksummed ledger and advisory lock. Database-backed `npm run db:migrate -- --status` is authoritative for complete applied/pending filenames and rejects ledger/local checksum drift, but it prints no checksum values. `--validate` and `--dry-run` are local-only and return before env-file loading or any connection; they cannot prove a Production target or state. Retain the runbook's separate local-plus-ledger checksum export before and after mutation. Use `--baseline` only after backup plus catalog verification of an existing non-empty pre-ledger schema. `scripts/verify-migration-baseline.mjs` recognizes only its narrower baseline/conditional-RLS profile, does not load `SIDESTREAM_DB_ENV_FILE`, and does not list all later hardening migrations. The runner, baseline verifier, legacy audit/apply, device tools, campaign report, and lead dump currently permit remote TLS without authenticated certificate/hostname proof, so their Production use is blocked until code-owned changes enforce clean selection, provider CA trust, verify-full-equivalent validation, and connected-target evidence. A preflight for one client cannot authenticate a second client. Fallback keeps the migrated schema and promotes only a separately staged, Production-environment, runtime-distinct artifact qualified against the full chain; it never deletes the ledger or reverses destructive migrations.
...
- Keep the live Stripe event destination enabled during maintenance and fallback. While the webhook deny is still disabled and the compatible current artifact serves it, reconcile every paginated pre-existing Workbench Failed/Pending delivery and require every retained exact event ID plus every local `terminal_at is null` row to finish as `processed` or `ignored`; zero merely due/expired work is insufficient. Record the boundary and timers, disable cron, and only then atomically activate the reviewed deny. After the deny is active and old in-flight writes drain, a final canonical historical lifecycle full/delta audit must close over every lifecycle ID through that boundary before migration or reopening; queue terminality is not canonical-state proof. Later delivery reconciliation carries the exact retained IDs and account-level List Events union. An already-active maintenance/fallback window preserves its earliest boundary and timers; it never resets them. Live automatic retries last at most three days, Dashboard/Workbench resend at most 15 days, and Stripe CLI resend at most 30 days. Events created while a destination is disabled do not auto-resend.
...
- If a successful purchase still shows Free in the account page or plugin, check `/api/stripe/webhook`, `/api/checkout/complete`, and activation logs, then inspect Stripe queue evidence; the current migration `--status` is loopback-disposable only, and Production diagnosis must use exclusively the future reviewed authenticated status procedure in `docs/api-hardening-runbook.md`, never the current remote database client. Runtime routes intentionally do not execute DDL, and account/session reads intentionally do not drain the queue. The status fallback retrieves only the persisted exact Checkout Session after device validation; it cannot repair a missing migration, unattached Session, refund, dispute, or poisoned event.
...
- The live one-time Pro catalog is a distinct pre-mutation gate: a clean-environment proof must mirror runtime selector precedence, retrieve the exact configured/default Product and selected Price without creating either, and retain exact-ID, live/active, linkage, one-time USD 999, and checksum evidence for pre-promotion reassertion. The recurring legacy proof is not a substitute.
...
- Historical lifecycle reconciliation is a separate pre-cutover blocker. The repository has no tested idempotent tool that can find and repair refund/dispute events terminalized by the known-bad mapper, so the runbook's explicit provisional action must remain closed before maintenance until that tool exists and actually runs. Its full scan must retain an inclusive high-watermark, exact-ID/type manifest/checksum, source and authenticated-target evidence, canonical outcomes, and entitlement watermarks; it cannot close the gate while old code still accepts events. After the maintenance deny is active and old in-flight writes drain, the tool must consume that proof in a final canonical full/delta reconciliation covering every lifecycle ID through the deny boundary, including pre-drain and transition IDs, then prove canonical entitlement state, resulting watermarks, and an idempotent no-op rerun before reopening. Manual ledger or entitlement edits are forbidden.
...
- 2026-07-14: Closed the final documentation gaps with a pre-mutation exact live Pro catalog proof, an explicit provisional historical-lifecycle producer before maintenance, mandatory post-deny consumption, and a Production-safe status troubleshooting route; follow-up audit aligned the four-file clean-environment inventory and complete runtime Price-selector contract. The required future authenticated database and historical tools remain blockers; no production action was performed.
- 2026-07-14: Closed the historical-lifecycle time-of-check gap in the documentation by making the pre-cutover scan provisional and requiring a retained high-watermark plus a final post-deny, post-drain full/delta canonical reconciliation through the deny boundary before migration or reopening. No production action was performed.
- 2026-07-14: Corrected the final documentation audit: staged the WAF deny disabled until a complete all-nonterminal Stripe drain, preserved fallback boundaries, made release qualification fail-closed, restricted Deployment Protection to Standard, blocked unsafe Production operator clients, hardened explicit database transport requirements, documented shared-rule `H` and unbounded crash/reclaim exposure, and corrected `leads:migrate` to its real HTTP replay contract. No production action was performed.
