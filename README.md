...
- Production, Preview, and Development `SIDESTREAM_INSTALLER_BLOB_PATHNAME` should resolve to the native/base `Mac-Installer.dmg` artifact. The `Mac-ZXP-Installer.dmg` path is the legacy ZXP-helper handoff and should not be used for the public website download.

## Recent Change Log

- Switched the documented Vercel download pathname from the ZXP-helper DMG to the native/base Mac installer DMG and clarified that website fulfillment should stay on the normal DMG flow.
