...
- `mobile-paid-prototype.html` - Local-only, `noindex` frontend review entry for the mobile paid-funnel prototype. It loads the current canonical page at runtime, preserves its styling, shader, media, motion, and feature content, then replaces the phone email handoff with a short post-purchase email note and `Buy Now $9.99` CTA, removes the Free plan and final download panel, labels the Pro-card action `Buy Now`, and routes both variant actions to the existing `/api/checkout/start` confirmation entry. It does not change `/`, and purchase-triggered installer email delivery is not implemented.
...
- `docs/paid-acquisition-contract.md` and `docs/paid-acquisition-runbook.md` - Normative `/mc` paid-acquisition boundary plus the default-off configuration, deterministic fixture commands, human-gated Test-only procedure, rollback, reporting/privacy limits, and remaining release gates. Neither document authorizes Production action.
- `middleware.ts`, `api/paid-acquisition/*`, `api/_lib/paid-acquisition.ts`, `api/_lib/paid-installer-email.ts`, `api/_lib/paid-release-manifest.ts`, `api/paid-download.ts`, and `api/releases/paid-latest.ts` - Additive paid-acquisition routing, landing, Checkout, email, receipt/claim, and platform-artifact surfaces. Only exact eligible mobile `GET /mc` navigation may receive a sticky 50/50 assignment; missing configuration falls back safely.
- `db/migrations/20260727010000_add_paid_acquisition_experiment.sql`, `tests/paid-*.test.mjs`, and `scripts/verify-paid-acquisition-e2e-fixtures.mjs` - Namespaced paid experiment schema and deterministic provider/database-free contract evidence. The fixture harness does not apply the migration or prove a deployed surface.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-resolve-waitlist-open]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers and the Resolve CTA opens its separate waitlist, while viewports at or below `900px` replace the hero CTA row with an email form headed "On your phone?" and explain, "We'll email you a link to your computer." That secondary sentence uses its original `14px` size; the submit action remains "Email me the download link," and lower-page download taps route back to that form. Public/legacy Sidestream Pro entry points retain the no-store `/api/checkout/start` confirmation page. The authenticated Free-account Upgrade form posts directly to `/api/checkout/create` and receives the Stripe redirect without visiting `upgrade.html` or the start confirmation.
...
- Paid acquisition - `middleware.ts` owns exact `/mc` eligibility and sticky `mc-control-v1`/`mc-paid-v1` 50/50 assignment. Only eligible mobile top-level `GET` navigation can be assigned; all uncertain/ineligible/configuration-failure traffic safely follows `/m`'s canonical ManyChat destination. `/m`, root/free, account, existing Checkout, activation, restore, installer, release, and legacy behavior remain preserved. See `docs/paid-acquisition-runbook.md`; the feature is default-off and no Production action is authorized.
- Download lead capture and replay - `api/download-lead.ts`, `api/_lib/download-leads.ts`, and `api/internal/download-leads/replay.ts` validate at most 8 KiB of JSON, converge repeated `(email, cta_source)` submissions, enforce 5/email and 20/IP per ten minutes, and fall back to deterministic private Blob records when Postgres fails. Scheduled replay processes 25 mapped records and deletes only after commit plus ETag match; manual replay is bounded to 100 and defaults to preserving records. Historical `windows-waitlist` rows remain queryable. Resolve waitlist submissions intentionally bypass this Postgres/replay path and remain in their separate durable private-Blob prefix.
...
`/mc` is the separate, default-off paid-acquisition experiment entry. Only an
eligible mobile top-level `GET /mc` navigation can receive a sticky 50/50
control/paid assignment. `HEAD`, desktop, tablet, bot/scanner, prefetch,
unknown/conflicting device evidence, malformed paths, and missing/invalid
configuration safely use the same canonical destination as `/m` without paid
state. `/m` itself remains unchanged. See `docs/paid-acquisition-runbook.md`.
...
| `/api/paid-acquisition/landing` | `GET` (internal rewrite only) | Private no-store paid landing only after a valid paid `/mc` assignment proof |
| `/api/paid-acquisition/checkout` | `POST` | Idempotent `200 {"url":"<https URL>","reused":boolean}` only for a valid paid entry and server-owned Test/Production binding |
| `/api/paid-acquisition/artifact` | `GET` | Current-payment verification then `302` to a short-lived platform artifact URL |
| `/api/paid-acquisition/claim` | `GET` | Receipt-cookie and Google OAuth claim/recovery flow; no client-selected payment or environment truth |
...
Run the deterministic paid-acquisition fixture proof without live Stripe,
Resend, Google OAuth, Postgres, Vercel, or Blob state:
...
These commands do not deploy, apply a migration, publish an artifact, make a
payment, send email, or prove macOS/Windows installation or loaded Premiere
behavior. The exact Test-only and rollback procedure is
`docs/paid-acquisition-runbook.md`.
...
- After any `/mc` routing, paid landing, paid Checkout/email/artifact/claim, entitlement bridge, or paid schema change, run `npm run test:paid-acquisition-e2e` and `node scripts/verify-paid-acquisition-e2e-fixtures.mjs`, then follow `docs/paid-acquisition-runbook.md`. Treat fixture success as deterministic contract evidence only, not deployed Test, payment, reporting, email-delivery, installation, or loaded-Premiere proof.
...
- 2026-07-27: Added the paid-acquisition runbook for exact `/mc` scope, default-off Test configuration, deterministic fixtures, safe rollback, reporting/privacy limits, preserved `/m` and legacy workflows, and explicit human release gates; no deployment, migration, artifact publication, payment, email, or loaded Premiere proof occurred.
- 2026-07-26: Added a local-only, `noindex` mobile paid-funnel prototype at `/mobile-paid-prototype.html`; it preserves the canonical presentation while replacing the phone handoff with a short post-purchase email note and `Buy Now $9.99` Checkout entry, retaining only Sidestream Pro pricing with a `Buy Now` action, and removing the final free-download panel. The canonical page and backend fulfillment remain unchanged, and purchase-triggered installer email delivery is not implemented.
