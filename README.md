...
- `api/auth/google/start.ts` and `api/auth/google/callback.ts` - Google OAuth redirect/callback handlers. They require the configured callback to share the browser-facing start origin before setting short-lived HTTP-only state and return-intent cookies, upsert `sidestream_accounts`, issue a server-side session cookie, and render a retryable noindex HTML error instead of raw JSON when sign-in state is stale. A current plugin activation may carry one server-signed `plugin_upgrade` capability through this state-verified callback; the callback then enters the same locked and rate-limited Checkout worker without rendering another confirmation page.
...
- `api/checkout/start.ts`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - Intentional one-time Sidestream Pro Checkout flow. GET start creates/resumes only a signed database intent and renders confirmation; confirmed same-origin POST create reuses one locked/idempotent Session and persists the exact Price/Product. The only direct alternative is the state-verified Google callback consuming a short-lived server-signed current-plugin Upgrade capability and calling that same worker. Generic GETs, link previews, arbitrary OAuth `next` values, legacy clients, and non-plugin activations cannot enter the direct boundary. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. The old Vercel host still fails closed when legacy activation context is missing.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers, while viewports at or below `900px` replace the hero platform choice with the email form and route lower-page download taps back to that form. Website Sidestream Pro entry points open the no-store `/api/checkout/start` confirmation page and reach Stripe only after the signed same-origin POST. Current plugin activations use the separately signed OAuth handoff described below.
...
| `/api/auth/google/start`, `/api/auth/google/callback` | `GET` | Existing-session account redirect or Google OAuth and server session creation; an exact state-verified current-plugin Upgrade callback redirects directly to the locked/reused Stripe Checkout Session |
...
- With Vercel dev and account env configured, confirm `GET /api/checkout/start` renders confirmation and makes no Stripe write; its signed same-origin `POST /api/checkout/create` creates/reuses one-time Checkout with invoice creation and persists the activation's exact Session before redirect. Separately start a current `source=plugin` activation, open its signed `plugin_upgrade` URL, complete Google OAuth, and confirm the state-verified callback goes directly to the same attached Stripe Test Checkout Session without displaying either internal confirmation page. Tamper with, omit, or expire that capability and confirm no Checkout worker call occurs; repeat with a non-plugin source and a 1.0.13 client and confirm they retain the confirmation path. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, cancelled Checkout resumes/rotates intentionally, `/account.html` can still sign in with Google, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
