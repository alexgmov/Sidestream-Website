...
- `api/_lib/customer-usage.ts`, `api/_lib/customer-query.ts`, `api/_lib/customer-admin.ts`, `api/internal/customer-usage/sync.ts`, and `api/internal/customers/*` - Once-daily privacy-limited telemetry aggregation plus private Customer 360 list/detail reads. `SIDESTREAM_TELEMETRY_POSTGRES_URL` is a separate read-only source, `SIDESTREAM_CRM_ADMIN_SECRET` protects POST-only non-browser reads and signs filter-bound cursors, and the read model excludes raw telemetry, identity values, `installIdHash`, Stripe IDs, search text, and merged tombstones.
...
- Customer 360 usage and private reads - `api/_lib/customer-usage.ts`, `api/_lib/customer-query.ts`, `api/internal/customer-usage/sync.ts`, `api/internal/customers/index.ts`, and `api/internal/customers/[customerId].ts`; schema-versioned telemetry becomes replaceable UTC daily aggregates, rolling 7/30-day activity, attempts-per-active-day frequency, first-attempt versus first-success timestamps, source freshness, compact currency partitions, signed keyset cursors, and explicit quality flags. The full cross-repo field/privacy/rollout contract is `docs/customer-360.md`.
...
checksummed migrations there, configure secrets/cron, deploy Preview/Test,
dry-run and verify backfill, separately approve any Test apply, verify protected
APIs and source freshness, then run the FlowState plan. That document also owns
