...
- `api/support/webhook.ts`, `api/internal/support/audit.ts`, `api/_lib/support-*.ts`, `db/migrations/20260825120000_add_support_safety_ledger.sql`, `docs/support-automation.md`, and `tests/support-automation.test.mjs` - Inert-by-default support-email intake with a signed Resend webhook, application-encrypted ticket content, deterministic prompt-injection/systematic-incident stops, tool-free strict-schema triage, and a separately prompted artifact auditor. Only flags/errors notify the private alert mailbox, and alerts omit customer content. The audit schema admits bounded PR metadata or registered support-table operations, rejects raw SQL and core product scope, and always returns `executed:false`; this release contains no generic executor, customer-reply sender, merge, deploy, or database-write worker.
...
- Support automation safety - `api/support/webhook.ts`, `api/internal/support/audit.ts`, and `api/_lib/support-*.ts` keep inbound email separate from the personal inbox and make two independent gates durable. Gate 1 holds back HTML/attachments, stops deterministic injection or systematic signals, and gives plain-text customer content only to a tool-free strict-schema model as untrusted user data. Gate 2 accepts only exact PR or registered support-table artifacts, deterministically refuses core files/tables and dependencies, then asks a separate tool-free model to audit product impact. Either flag/error sends a privacy-minimal alert. An audit pass is only ledger state; no action can execute in this release. The full activation/rollback contract is `docs/support-automation.md`.
...
uses the separate `SIDESTREAM_SUPPORT_ADMIN_SECRET`; it never accepts the CRM or
...
- Run `npm run test:support` after changing the support webhook, encryption, prompts, model payloads, schemas, ledger, notifications, audit route, support migration, or support runbook. It compiles the Hetzner API first, then proves deterministic injection/systematic/attachment stops, customer content in user role only, tool-free strict outputs, model-pass override, core-file/table/dependency refusals, raw-SQL rejection, authenticated encryption, signed-webhook persistence/idempotency boundaries, POST-only non-browser audit auth, privacy-minimal alerts, and private append-only migration shape. Also run `npm run verify:vercel-contract` because the safety audit is a classified internal route. Passing fixtures do not create or prove a mailbox, DNS, provider configuration, migration, alert delivery, customer reply, PR, database write, Hetzner reload, or live automation.
...
- Support automation is deliberately inert until its migration and complete Hetzner-side configuration are live-qualified. `audit_passed` never means executed, and this repository has no support executor. Never add raw email to system/developer prompts, fetch attachment/HTML content automatically, accept free-form SQL/shell commands, reuse CRM/cron secrets, or let a model merge/deploy/write directly. Check existing MX ownership before assigning Resend to `sidestream.tv`; if it would displace working mail, use a dedicated receiving subdomain. Keep the current personal-address links until the new mailbox and exception-only alerts are visibly proven.
