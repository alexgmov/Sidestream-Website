...
- `config/upgrade-pricing-experiment.mjs`, `api/_lib/upgrade-pricing-experiment.ts`, `db/migrations/20260812120000_add_upgrade_pricing_experiment.sql`, and `docs/upgrade-pricing-experiment.md` - Authenticated `upgrade-pricing-v1` contract. A server-only HMAC bucket plus immutable account assignment selects the unchanged trusted-country one-time offer or the exact recurring amount stored beside that offer in the canonical pricing contract. The historical `monthly_half` ID remains stable even when the approved recurring amount changes. The client selects no commerce truth; active owners remain ineligible; assignment/exposure rows contain no direct identity or payment secrets; a shared account lock closes entitlement-versus-Checkout races; and assignment/config/Price failures use an observable one-time fallback.
