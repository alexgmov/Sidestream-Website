...
- Landing referral attribution - Vercel redirects `/m` to the canonical landing page with `utm_source=manychat`, `/ig` with `utm_campaign=bio` for Sidestream's Instagram, and `/alex` with `utm_campaign=alex-bio` for Alex's personal Instagram; the same routes with a trailing slash are accepted so copied or normalized bio URLs do not fall through to a `404`. Both Instagram links also use `utm_source=instagram` and `utm_medium=social`. The browser maps those owned links to separate `manychat`, `instagram-bio`, and `instagram-alex` sources and sends one non-blocking POST to `/api/referral-visit`; the server stores a private, scanner-aware, daily-deduplicated Blob record using the existing analytics HMAC secret. This measures landing visits, not installer downloads, purchases, or activations.
