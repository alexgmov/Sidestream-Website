...
The current hosted database provider is Neon Postgres. All database access is server-side through the shared `pg` pool and server-only Postgres connection variables; the browser and CEP extension do not use a database client or provider Data API.
...
- `index.html` - Canonical page implementation served at `/`. Contains the shader mount root, header, hero, desktop Mac/Windows download CTAs plus independent DaVinci Resolve and After Effects waitlist CTAs/modals, mobile email handoff, feature sections, pricing, final CTA, footer, styles, rotating-word script, toast behavior, crawler metadata, and structured data.
- `mobile-paid-prototype.html` - Local-only, `noindex` frontend review entry for the mobile paid-funnel prototype. It loads the current canonical page at runtime, preserves its styling, shader, media, motion, and feature content, then replaces the phone email handoff with a `$9.99` purchase CTA, removes the Free plan and final download panel, and routes variant purchase/download links to the existing `/api/checkout/start` confirmation entry. It does not change `/`, and purchase-triggered installer email delivery is not implemented.
...
- `upgrade.html` - Minimal noindex public/legacy checkout fallback page. Anonymous and activation-bearing entry points can still open the server-owned `/api/checkout/start` confirmation boundary, while activation recovery enters the authenticated claim page so the account can safely choose same-device reconnect, confirmed transfer, or a confirmed purchase without a duplicate charge. The signed-in account Upgrade path does not render this page.
...
- `api/referral-visit.ts` and `api/_lib/referral-visits.ts` - First-party landing-referral attribution for the owned ManyChat, Sidestream Instagram, and Alex Instagram links. A real page load posts the allowlisted `manychat`, `instagram-bio`, or `instagram-alex` source, receives `204` without waiting for storage, and writes at most one private Blob record per anonymous request fingerprint/day/source/classification. Daily HMACs reuse the installer-analytics secret; raw IP and user-agent values are never stored.
...
- `api/checkout/start.ts`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - Intentional one-time Sidestream Pro Checkout flow. GET start creates/resumes only a signed database intent and renders confirmation for anonymous/activation flows; same-origin POST create is the sole Session-creation boundary. Its signed-in account form branch creates the account-bound intent internally and returns straight to Stripe, while confirmed public forms reuse one locked/idempotent Session. Both persist the exact Price/Product. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. The old Vercel host still fails closed when legacy activation context is missing.
...
- `db/migrations/20260707120000_enable_sidestream_server_table_rls.sql` - Postgres access-hardening migration that enables RLS on server-owned Sidestream public tables and revokes direct `anon` / `authenticated` Data API access left by the former hosted-database integration. The Vercel API routes use only the server-side Neon Postgres connection.
...
- `db/migrations/20260714120000_add_installer_request_tracking.sql` - Adds the server-owned `public.sidestream_installer_requests` attribution table, reporting indexes, RLS, and explicit direct-access revocations for legacy Data API roles.
...
- `scripts/run-api-tests.mjs`, `scripts/run-postgres-integration.mjs`, `scripts/validate-vercel-contract.mjs`, `scripts/verify-production-source.mjs`, and `scripts/verify-vercel-build.mjs` - Aggregate handler/state-machine test discovery, disposable-Postgres concurrency proof with runtime-target rejection, static Vercel route/cron contract validation, the clean canonical-release/Vercel-project deployment gate, and the post-build function/landing-surface verifier.
...
- `scripts/reset-alex-upgrade-state.mjs` and `tests/reset-alex-upgrade-state.test.mjs` - Fixed-identity account/upgrade reset for Alex Garrett across the permanent Production and isolated-Test Neon projects plus the exact live and Customer 360 sandbox Stripe accounts. It is dry-run by default, obtains direct verify-full database connections from the authenticated Neon CLI, requires a literal apply confirmation, deletes matching Stripe customers and app-owned account/payment authority, preserves historical Stripe objects and Customer 360 audit/projection history, then fails unless post-reset readback is clean.
...
- `scripts/report-referral-visits.mjs` and `tests/referral-visits.test.mjs` - Read-only private-Blob aggregate reporting plus focused route/privacy tests for first-party landing referrals. The report returns daily human/scanner counts without returning anonymous hashes; its totals are sums of daily uniques, not identified people across multiple days.
...
- `demos/sidestream-panel-corner.webm` - Square VP9-alpha WebM generated from the ProRes source `sidestream demo Linked Comp 01_2.mov` using the full-plugin/timeline top-left crop. Mounted as a decorative Premiere/Sidestream corner inside the desktop hero, visually scaled to 70% from the Premiere panel's top-left corner and anchored at `45vw 25vh`. The wrapper is hidden at `900px` and below so mobile visitors go directly from the wordmark to the hero copy and computer-handoff form.
- `pryt.png` - Lightweight 500×500 transparent Premiere Pro/YouTube mark artwork shown only in the mobile hero. The visible marks occupy the center of the padded square canvas so the responsive wrapper can center them without a background matte; the displayed image is currently enlarged another 80% to `min(79.488vw, 311.04px)`.
...
- Header/nav - `header`, `.nav`, `.brand`, `.nav-links`; the desktop header exposes Features, Pricing, and Account as compact glass pill links without a download CTA
...
- DaVinci Resolve waitlist - `[data-resolve-waitlist-open]` is the third desktop hero pill. It carries the inline three-color Resolve mark, opens `#resolve-waitlist-gate`, and records successful email submissions through the dedicated Blob-only `/api/resolve-waitlist` route. The server fixes `cta_source = "davinci-resolve-waitlist"`; the browser cannot choose another bucket. It is hidden with the rest of `.desktop-download-ctas` at `900px` and below, where the existing computer download handoff remains the only hero form.
- After Effects waitlist - `[data-after-effects-waitlist-open]` is the fourth desktop hero pill. It carries the compact `Ae` mark, opens `#after-effects-waitlist-gate`, and records successful email submissions through `/api/after-effects-waitlist`. The server fixes `cta_source = "after-effects-waitlist"` and stores it independently from Resolve. It follows the same desktop-only breakpoint, leaving the existing mobile computer handoff unchanged.
...
- Download and upgrade actions - `[data-download]`, `[data-windows-download]`, `[data-resolve-waitlist-open]`, `[data-after-effects-waitlist-open]`, `[data-purchase]`, `#mobile-download-handoff`, and `#toast`; desktop Mac/Windows CTAs retain their direct installers and each product waitlist CTA opens its own modal, while viewports at or below `900px` replace the hero CTA row with an email form headed "On your phone?" and explain, "We'll email you a link to your computer." That secondary sentence uses its original `14px` size; the submit action remains "Email me the download link," and lower-page download taps route back to that form. Public/legacy Sidestream Pro entry points retain the no-store `/api/checkout/start` confirmation page. The authenticated Free-account Upgrade form posts directly to `/api/checkout/create` and receives the Stripe redirect without visiting `upgrade.html` or the start confirmation.
...
- Landing referral attribution - `/m` maps to `manychat`, `/ig` and the full Sidestream bio UTM URL map to `instagram-bio`, and `/alex` plus the full personal bio UTM URL map to `instagram-alex`. The loaded page POSTs the allowlisted source to `/api/referral-visit`; private Blob pathnames dedupe repeated visits from the same anonymous request fingerprint on the same UTC day and separate likely-human from likely-scanner traffic. These links measure landings, not downloads, installs, activations, purchases, or durable identities.
- Download lead capture and replay - `api/download-lead.ts`, `api/_lib/download-leads.ts`, and `api/internal/download-leads/replay.ts` validate at most 8 KiB of JSON, converge repeated `(email, cta_source)` submissions, enforce 5/email and 20/IP per ten minutes, and fall back to deterministic private Blob records when Postgres fails. Scheduled replay processes 25 mapped records and deletes only after commit plus ETag match; manual replay is bounded to 100 and defaults to preserving records. Historical `windows-waitlist` rows remain queryable. Resolve and After Effects waitlist submissions intentionally bypass this Postgres/replay path and remain in their separate durable private-Blob prefixes.
...
The short first-party ManyChat referral link is:

```text
https://sidestream.tv/m
```

It returns a temporary redirect to `https://sidestream.tv/?utm_source=manychat` so the canonical page retains normal UTM context while Sidestream records its own privacy-limited visit. The destination document keeps the canonical root URL in its metadata.

The Sidestream Instagram bio should use the full URL `https://sidestream.tv/?utm_source=instagram&utm_medium=social&utm_campaign=bio`; `/ig` and `/ig/` remain temporary shortcuts to that destination. The personal Alex Instagram bio should use `https://sidestream.tv/?utm_source=instagram&utm_medium=social&utm_campaign=alex-bio`; `/alex` and `/alex/` remain temporary shortcuts. Both full URLs trigger distinct first-party server-side referral sources after the landing page loads.
...
| `/api/referral-visit` | `POST` | `204` after accepting allowlisted `manychat`, `instagram-bio`, or `instagram-alex` and scheduling a private, daily-deduped anonymous Blob write |
...
| `/api/resolve-waitlist` | `POST` | `200 {"ok":true}` only after durable hashed rate-limit consumption and a deterministic private-Blob write in the Resolve prefix; fails closed if Blob is unavailable |
| `/api/after-effects-waitlist` | `POST` | `200 {"ok":true}` only after durable hashed rate-limit consumption and a deterministic private-Blob write in the After Effects prefix; fails closed if Blob is unavailable |
...
| `/api/checkout/create` | `POST` | Signed-in account form or confirmed public form `303` to Stripe; confirmed JSON request returns `200 {"url":"...","reused":boolean}` |
...
The fixed shader mount uses its inset box instead of an explicit `100vh`, and `DemoOne` fills that mount with `h-full` instead of creating a second `h-screen` height. This avoids a mobile Safari viewport seam when browser chrome expands or collapses; the matching `#1b1b1b` theme color keeps Safari's chrome from introducing a hard black band.

The header is a fixed transparent overlay with no scroll divider so the shader remains uninterrupted behind the nav. The `.hero-pad` section fills the first viewport and aligns the hero headline, subline, and primary `Download for Mac` CTA to the lower-left first-fold gutter. The Sidestream wordmark and hero copy share the viewport-left `24px` first-fold gutter, and the Features/Pricing/Account navigation cluster is absolutely anchored to the viewport's top-right corner with a `15px` top offset and matching `24px` right gutter. Each desktop nav link uses a compact rounded glass frame with a white-fill hover state so it stays legible as the shader moves without competing with the larger download pills. The header intentionally has no download CTA; pricing is linked from the header and remains available in the page body, while downloads remain available in the hero, pricing, and final CTA sections.
...
Review the isolated mobile paid-funnel prototype at `http://localhost:5173/mobile-paid-prototype.html` with a `390x844` viewport. The canonical page remains at `/`; the prototype does not implement purchase-triggered installer email delivery and must not be treated as Production behavior.
...
curl -i -X POST http://127.0.0.1:3000/api/after-effects-waitlist \
  -H 'Content-Type: application/json' \
  --data '{"email":"after-effects@example.com","page":"/"}'
```

If Vercel Blob OIDC is disabled for the Development environment, local `/api/download`, `/api/resolve-waitlist`, and `/api/after-effects-waitlist` return Blob auth/config errors even though Preview and Production have Blob env attached. Fix that in the Vercel Blob store settings, or add a valid `BLOB_READ_WRITE_TOKEN` for local development. `/api/download-lead` prefers Postgres when `POSTGRES_URL` or a supported `SIDESTREAM_POSTGRES_*` connection string is available and only needs Blob auth for the fallback path; both product waitlist routes are intentionally Blob-only.

### Production deployment

Until the divergent `main` history is deliberately reconciled, the only canonical Production source is `origin/codex/release-1.0.14`. Do not create a Production worktree from `origin/main`, another feature branch, an earlier deployment, or an existing `.vercel/output` directory. A Vercel deployment replaces the complete site and function bundle; a correct one-line change on a stale base can therefore remove unrelated newer features.

Link the clean canonical checkout to `alex-3685s-projects/sidestream`, pull its Production environment, and use the guarded command:

```bash
npx vercel@latest link --yes --project sidestream --scope alex-3685s-projects
npx vercel@latest pull --yes --environment=production --scope alex-3685s-projects
npm run deploy:production
```

`deploy:production` fails unless the worktree is clean, `HEAD` exactly matches the live remote `origin/codex/release-1.0.14` head, and `.vercel/project.json` identifies the pinned Sidestream project/team. It then creates a fresh Production build, rejects a bundle without both product waitlist surfaces or the exact `/ig` and `/m` redirect routes, and only then runs the prebuilt Production deployment. Afterward, still inspect `sidestream.tv`, confirm the active alias/deployment ID, verify both short links at the canonical domain, open each rendered waitlist modal, verify both waitlist `GET` routes return `405 Allow: POST`, and use a clearly synthetic POST plus masked private-Blob readback for any capture path that changed.
...
Read first-party landing-referral totals directly from the configured private
Blob store with an approved read/write token or Vercel-provided Blob credentials:
...
This report is read-only. `uniqueDailyLikelyHumanVisitors` is the useful ManyChat
landing count after the scanner heuristic; totals sum anonymous daily uniques, so
one person visiting on two UTC days contributes two. The record is landing-visit
evidence only and must not be labeled a download, install, activation, or purchase.

The `20260707120000_enable_sidestream_server_table_rls.sql` migration preserves access hardening for database copies that still define the legacy `anon` and `authenticated` Data API roles. It locks down direct Data API access to leads, accounts, sessions, activation rows, license rows, license-token hashes, Stripe event payloads, and billing resource rows. Current Sidestream runtime uses only server-side Neon Postgres and has no browser database client or Data API path. After applying the migration to a compatible database, inspect RLS and role grants and smoke-test the Vercel API routes.
...
Reset Alex Garrett's account and upgrade authority for a completely fresh
Production/Test upgrade-flow run with the fixed operator command. First
authenticate the pinned Neon CLI with `npx --yes neonctl@2.37.1 auth`, then
create an ignored env file containing only:

```dotenv
SIDESTREAM_RESET_PRODUCTION_STRIPE_SECRET_KEY=sk_live_...
SIDESTREAM_RESET_TEST_STRIPE_SECRET_KEY=sk_test_...
```

On Alex's workstation, the executable
`/Users/alexgarrett/.local/bin/resetalex` is already on `PATH` and uses
`/Users/alexgarrett/.config/sidestream/reset-alex.env` by default. After the
one-time credential file is created with mode `600`, run `resetalex --dry-run`
for the non-mutating inventory or run `resetalex` by itself to apply the exact
fixed reset immediately. The short command embeds the required literal
confirmation but does not broaden the script's fixed identity or provider
scope. `SIDESTREAM_RESET_ENV_FILE` can override the default credential-file
path.

Inventory both exact Neon projects and Stripe accounts without changing them:

```bash
SIDESTREAM_RESET_ENV_FILE=/absolute/path/to/sidestream-reset.env \
  npm run account:reset:alex
```

Review the provider IDs, customer IDs, and table counts in that dry-run output.
Apply the same fixed scope only when those are correct:

```bash
SIDESTREAM_RESET_ENV_FILE=/absolute/path/to/sidestream-reset.env \
  npm run account:reset:alex -- \
  --apply --confirm DELETE-ALEX-GARRETT-UPGRADE-STATE
```

The script cannot accept arbitrary people, projects, database URLs, or Stripe
accounts. It resolves direct connections for Neon projects
`dark-butterfly-59697025` and `ancient-breeze-53489732`, forces
`sslmode=verify-full`, proves the connected database/role and Stripe account IDs,
deletes exact email/name matches plus Stripe customers linked by those account
rows, runs serializable database cleanup with bounded webhook-settle passes, and
requires zero-state readback. It intentionally keeps Stripe invoice/payment/
charge/refund/dispute history and Customer 360 profiles, installs, commerce,
merges, and immutable identity reviews; only raw Customer 360 account/payment
links are removed. Download leads and installer/referral analytics are preserved.
It also does not remove the local CEP plugin, caches, receipts, or app state.
...
- Run `node --test tests/reset-alex-upgrade-state.test.mjs` after changing the fixed Alex Garrett reset. The focused suite covers dry-run/apply gating, exact identity matching, Neon direct-target validation, Stripe mode isolation and customer pagination, cumulative verification hints, zero-count checks, and secret-free reporting. Do not use `--apply` as a test command.
- After changing the hero CTA system or mobile handoff, run the focused `tests/download-leads.test.mjs` suite, then verify at a realistic phone width that the inline form replaces the desktop CTA row, its "On your phone?" copy and 20%-off explanation fit without hiding the submit button, invalid email stays local, success is announced, and lower download CTAs scroll back to the form. Confirm the generated HTML and plain-text emails both include `STREAM20`. At desktop width, confirm the form is hidden, both direct platform downloads remain unchanged, and the Resolve and After Effects waitlists each open, validate, submit to their dedicated route, and report success without starting a download. Route coverage must prove each server-fixed source and deterministic private-Blob prefix plus the shared hashed rate limit, `429`, and fail-closed `503` behavior.
- Run `node scripts/assert-no-runtime-ddl.mjs` and `node scripts/validate-vercel-contract.mjs` after API/migration/routing work. For a human Vercel build, follow `npx vercel@latest build` with `npm run verify:vercel-build`; the verifier requires both product waitlist functions and each CTA/modal/browser-route marker set plus the exact built `/ig` and `/m` redirects. For Production, use only `npm run deploy:production`, which adds clean canonical-source and pinned-project checks before building.
...
- For `mobile-paid-prototype.html`, run `npm run build`, serve the repository with `npm run dev`, then verify the prototype at `390x844`: no horizontal overflow or clipped purchase copy, a minimum `48px` hero purchase target with visible keyboard focus, no visible Free-plan/discount/email-before-purchase messaging, no large gap where the final CTA was removed, and every visible purchase/download action resolving to `/api/checkout/start`. Recheck `/` separately to confirm the canonical hero handoff, pricing, final CTA, and direct desktop downloads remain unchanged. This prototype has no purchase-to-installer-email backend connection.
...
- Run `npm run test:referral-visits` after changing `/m`, `/api/referral-visit`, the ManyChat browser hook, private-Blob referral storage, or its report. It verifies the short-route contract, allowlisted source, bounded request body, response-before-storage behavior, daily anonymous dedupe inputs, scanner separation, and hash-free aggregate output.
...
- Confirm the Sidestream wordmark and desktop hero copy share the viewport-left `24px` first-fold gutter, and the Features/Pricing/Account header cluster sits at the viewport's top-right with a `15px` top offset and `24px` right gutter. Confirm each link has its own compact rounded glass frame, turns white with black text on hover, shows a visible keyboard focus ring, and does not introduce a download header CTA.
...
- Confirm the pricing MacBook Pro mockup video stays unloaded during the initial hero/features load, prewarms shortly before it approaches, plays and loops only while visible, stays muted, pauses offscreen or while the page is hidden, sits centered below the two pricing panels plus final CTA, and does not create horizontal overflow.
...
- Confirm the decorative `.feature-corner-demo-video` keeps the visible Premiere panel's compensated top-left anchor within `1px` of `45vw 25vh` at multiple desktop window sizes, reaches the hero-to-feature boundary without an empty strip at tall ratios such as `1892x1990`, is clipped at that section boundary, and keeps `.feature-glass` unmoved. At `900px` and below, confirm `.feature-corner-demo` is hidden and contributes no layout height, `pryt.png` is centered in the open space above the headline without a visible background matte, and the full mobile handoff remains legible without horizontal overflow.
...
- With Vercel dev and account env configured, confirm `GET /api/checkout/start` renders confirmation and makes no Stripe write; signed public forms and the authenticated account's direct `account_purchase` form are the only `/api/checkout/create` branches that create/reuse one-time Checkout with invoice creation. Confirm the account form returns one `303` straight to hosted Stripe and cancellation returns to `/account.html` without either confirmation page. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, anonymous/activation cancellations resume/rotate intentionally, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
- On the signed-in desktop account page, confirm a Free account's Upgrade button posts directly to Checkout, disables as `Opening Stripe...`, and reaches hosted Stripe without rendering `upgrade.html` or `/api/checkout/start`; confirm the refund action opens a new Gmail compose tab with the support address, subject, message, and signed-in account email prefilled; and confirm every row action shares the panel's right edge, including wrapped receipt and refund controls. Below `680px`, confirm the controls remain full-width.
...
- The decorative hero recording is intentionally desktop-only. Do not reintroduce it below `901px` without retesting alpha decoding in iOS in-app browsers; `background: transparent` alone cannot remove a decoder/compositor matte when VP9 alpha falls back to the asset's black RGB plane.
...
- ManyChat and both Instagram referral sources deliberately measure real landing-page API calls, not upstream sends, link clicks that never load the page, downloads, installs, activations, or purchases. `/m`, `/ig`, and `/alex` preserve their UTM destinations, but the full UTM URLs work without the shortcuts and the authoritative owned count is the private-Blob report, not Vercel's plan-gated UTM panel. Same-day reloads from the same anonymous request fingerprint and source are deduped; cross-day totals are not durable person counts.
...
- A successful narrow visual assertion does not qualify a Production artifact. The July 26 header-only deployment started from divergent `origin/main`, passed its Features/Account/Pricing assertion, and replaced the active Resolve CTA and API with the older Windows-waitlist snapshot. Keep `origin/codex/release-1.0.14` as the canonical Production source until `main` is deliberately reconciled, and use `npm run deploy:production` so stale branch ancestry, wrong Vercel linkage, reused build output, missing Resolve UI, and a missing Resolve function all fail before promotion.
...
- The Windows waitlist is historical: `cta_source = "windows-waitlist"` rows remain queryable, but its retired modal/capture code was replaced by active product waitlists after the Windows hero pill became a direct platform download. Resolve and After Effects submissions use the dedicated Blob-only `/api/resolve-waitlist` and `/api/after-effects-waitlist` routes, not `/api/download-lead`; records live under their respective `sidestream/<product>-waitlist/v1/fallback-v2/<shard>/<lead-hmac>.json` prefixes, pathnames contain no plaintext email, and repeated `(email, source)` submissions merge into the same private record. Do not point the generic fallback replay job at either durable waitlist prefix.
- Resolve and After Effects waitlist capture require `BLOB_READ_WRITE_TOKEN` or Vercel Blob OIDC plus `BLOB_STORE_ID`, `SIDESTREAM_LEAD_HASH_SECRET`, and `SIDESTREAM_RATE_LIMIT_HASH_SECRET` in Preview and Production. Both routes fail closed with `503` when rate limiting or storage is unavailable and log only aggregate outcome/error codes, never the submitted email or raw request IP. Keep their fixed sources, rate-limit scopes, and private prefixes distinct.
...
- Server-owned Sidestream SaaS tables must stay private and server-only. On databases that define the legacy Data API roles, keep RLS enabled with no direct `anon` or `authenticated` table access. If a future feature needs browser-side database reads or writes, add the narrow policy for that feature intentionally and document the public data shape; do not make the private account, session, activation, license, license-token, Stripe event, or lead tables broadly API-readable.
...
- 2026-07-27: Restored the Pricing link in the top-right desktop header, preserving the existing Features and Account glass pills, the in-page pricing section, and the mobile-hidden navigation behavior.
- 2026-07-27: Restored server-side landing attribution for both full Instagram bio URLs and their `/ig` and `/alex` shortcuts, with separate `instagram-bio` and `instagram-alex` private-Blob sources, trailing-slash routes, focused tests, and deployable-artifact guards for both mappings and redirects.
- 2026-07-27: Restored the missing Production `/ig` Instagram bio redirect and made the prebuilt deployment verifier require the exact `/ig` and `/m` redirect routes before promotion, closing the gap where source configuration could pass while the canonical short link returned Vercel `404`.
- 2026-07-27: Added a desktop After Effects waitlist pill and dedicated modal beside the existing Resolve CTA, plus fail-closed `/api/after-effects-waitlist` ingestion with a server-fixed source, isolated rate-limit scope, deterministic private-Blob prefix, focused route coverage, and Vercel bundle assertions. The mobile computer handoff remains unchanged.
- 2026-07-26: Restored the DaVinci Resolve waitlist CTA and `/api/resolve-waitlist` after a header-only Production deployment was built from divergent `origin/main`; added a clean canonical-release/project gate plus fresh-build assertions for the Resolve CTA, modal, browser route, and server function.
- 2026-07-26: Added a local-only, `noindex` mobile paid-funnel prototype at `/mobile-paid-prototype.html`; it preserves the canonical presentation while replacing the phone handoff with a `$9.99` Checkout entry, retaining only Sidestream Pro pricing, and removing the final free-download panel. The canonical page and backend fulfillment remain unchanged, and purchase-triggered installer email delivery is not implemented.
- 2026-07-26: Added a dry-run-first, literally confirmed Alex Garrett upgrade-state reset that verifies the permanent Production/Test Neon project IDs and exact live/sandbox Stripe accounts, removes matching Stripe customers plus app-owned account/payment authority, preserves historical Stripe and Customer 360 audit/projection records, and requires clean post-reset readback. Installed the machine-local `resetalex` command for the same fixed reset, with `--dry-run` available for inventory-only checks.
...
- 2026-07-26: Enlarged the mobile Premiere Pro/YouTube logo artwork another 80%, from `172.8px`/`153.6px` to `311.04px`/`276.48px` maximums.
- 2026-07-26: Doubled the mobile Premiere Pro/YouTube logo artwork size, from `86.4px`/`76.8px` to `172.8px`/`153.6px` maximums, while leaving the handoff text at `14px`.
- 2026-07-26: Applied the intended cumulative logo reductions to the mobile Premiere Pro/YouTube artwork, shrinking it first by 60% and then another 40% to 24% of its original displayed size while keeping the handoff text at `14px`.
- 2026-07-26: Restored the mobile computer-handoff sentence to its original `14px` after clarifying that the requested size reductions were intended for the logos.
- 2026-07-26: Reduced the mobile computer-handoff sentence another 40%, from `5.6px` to `3.36px`, without changing the rest of the form.
- 2026-07-26: Reduced the mobile computer-handoff sentence from `14px` to `5.6px`, making it 60% smaller while leaving the rest of the form unchanged.
- 2026-07-26: Added the transparent `pryt.png` Premiere Pro/YouTube artwork to the center of the mobile hero's open space while keeping the panel recording desktop-only.
- 2026-07-26: Removed the decorative Premiere panel recording and its reserved spacer from the mobile hero while preserving the desktop recording.
- 2026-07-26: Clipped the phone hero video to the recording's real alpha bounds and added a tiny transparent lead-in to its existing wrapper mask, removing the black VP9 video-plane seam in iOS in-app browsers while leaving desktop geometry and the lower fade unchanged.
- 2026-07-26: Simplified the mobile computer-handoff subtext to "We'll email you a link to your computer."
- 2026-07-24: Lifted the phone hero recording roughly another 5% while preserving its scale and the headline/email-card positions.
- 2026-07-24: Raised the phone hero headline and mobile download handoff, tightened the phone-only demo footprint, and removed the mobile Safari shader/chrome seam without changing the desktop composition.
- 2026-07-24: Moved the phone hero recording another `24px` higher and reduced it by roughly 7% so the headline stays legible over the lower fade.
...
- 2026-07-24: Reduced the phone-width hero recording by 10%, adjusted its horizontal anchor to `70%`, raised its whole clipping wrapper without cropping the top edge, then extended the mobile/tablet wrapper to reveal the Video/Audio controls and a larger section of the Premiere timeline before the full-length lower-edge fade, without changing the desktop composition.
...
- 2026-07-22: Fixed the Account refund action to open a prefilled Gmail compose draft in a new tab; the old generic `mailto:` handoff could land in the inbox without composing the request.
- 2026-07-22: Removed both Sidestream confirmation pages from the authenticated Free-account Upgrade path. The account button now submits one same-origin POST that creates its account-bound intent server-side and returns a `303` directly to Stripe; account Checkout cancellation returns to the account page, while anonymous and legacy activation safeguards remain intact.
- 2026-07-21: Added the short `https://sidestream.tv/m` ManyChat link and first-party landing-referral attribution through a privacy-limited API, private Blob daily dedupe, likely-scanner separation, and a read-only aggregate report, avoiding Vercel's paid UTM breakdown and any Production database migration.
...
- 2026-07-20: The Account page now keeps its one-time receipt and refund row hidden unless the authenticated session finds a completed one-time Sidestream purchase, separating purchase evidence from the earlier and broader Stripe-customer flag.
- 2026-07-19: Declared Neon Postgres as the current server-only database provider and replaced every obsolete provider-specific README reference with provider-neutral migration and RLS history so agents do not mistake legacy infrastructure for the active runtime.
...
- Added an RLS hardening migration for server-owned Sidestream public tables, revoking direct `anon` and `authenticated` Data API table access while preserving the server-only Postgres route contract.
...
- Added support for Vercel-managed Postgres connection env names across account/billing APIs, download-lead capture, and Postgres maintainer scripts, with `SIDESTREAM_POSTGRES_URL` preferred over older generic `POSTGRES_URL` fallbacks.
