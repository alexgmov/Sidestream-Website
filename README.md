...
- `api/send-download-links.ts` and `api/_lib/download-link-email.ts` - Mobile-only computer handoff. The public POST route requires an idempotency key, stores the `mobile-download-handoff` lead plus bounded UTM context in the existing private replay queue, enforces a durable hashed 3/email and 10/IP per-hour Blob limit, and sends one transactional Resend message from `downloads@alexg.mov` with direct Mac and Windows installer links. The email presents both installers as matching white platform-marked capsules on a dark panel, mirroring the website CTAs while retaining explicit platform labels; its Windows mark reuses the landing page's exact SVG silhouette instead of font glyphs. Provider errors and logs never return or print the recipient address.
