...
- `api/referral-visit.ts` and `api/_lib/referral-visits.ts` - First-party landing-page referral attribution. The POST-only route accepts the allowlisted `manychat` and `instagram-bio` sources, returns `204` before its bounded background write, hashes request identity per source/day without storing raw IP or user agent, flags likely scanners, and writes one deterministic private Vercel Blob record per daily visitor.
...
- `api/checkout/start.ts`, `api/checkout/create.ts`, and `api/checkout/complete.ts` - Intentional one-time Sidestream Pro Checkout flow. Anonymous GET start creates/resumes only a signed database intent and renders confirmation; confirmed same-origin POST create is the website Session-creation boundary. Activation-bearing GET start is the shipped-panel compatibility boundary and resumes or creates its one locked, idempotent, attached Stripe Session directly. Both paths persist the exact Price/Product. Completion re-fetches Stripe truth before fulfillment and returns through the literal `{CHECKOUT_SESSION_ID}` placeholder. The old Vercel host still fails closed when legacy activation context is missing.
...
- `tests/referral-visits.test.mjs` - Focused tests for ManyChat and Instagram-bio source validation, privacy-limited daily hashing, deterministic private-Blob pathnames, short routes, and scanner classification.
...
- Landing referral attribution - Vercel redirects `/m` to the canonical landing page with `utm_source=manychat` and `/ig` with the standard Instagram bio tags `utm_source=instagram`, `utm_medium=social`, and `utm_campaign=bio`. The browser maps those owned links to separate `manychat` and `instagram-bio` sources and sends one non-blocking POST to `/api/referral-visit`; the server stores a private, scanner-aware, daily-deduplicated Blob record using the existing analytics HMAC secret. This measures landing visits, not installer downloads, purchases, or activations.
...
It temporarily redirects to the canonical root with `utm_source=instagram&utm_medium=social&utm_campaign=bio` so Instagram-bio visits remain separate from ManyChat visits.
...
The exact additive device schema is the only exception recorded here. Its
Production application completed on 2026-07-21; rerun only the read-only live
verifier to confirm the pinned migration contract and connected target:

```bash
node scripts/apply-production-device-schema.mjs --verify
```

A passing result proves the presence and exact shape of
`sidestream_account_devices` and `sidestream_device_transfers`; the 2026-07-21
live verification passed against target fingerprint `1c9e04dba0f2439c`. It
does not authorize another apply, the general migration runner, a deployment,
or device-policy enforcement.
...
- Run `npm run test:referral-visits` after changing `/m`, `/ig`, `/api/referral-visit`, or the private-Blob attribution helper. It verifies the source allowlist, short routes, daily deduplication, privacy boundary, storage path, and scanner heuristic.
...
- With Vercel dev and account env configured, confirm anonymous `GET /api/checkout/start` renders confirmation and makes no Stripe write; only its signed same-origin `POST /api/checkout/create` creates/reuses website Checkout. Separately confirm an activation-bearing GET from a shipped panel directly creates or resumes exactly one idempotent attached Stripe Session. Confirm successful Checkout passes through `/api/checkout/complete` to `/thank-you.html`, activation status returns active credentials on both the pre-hardening and fully migrated disposable schemas, cancelled Checkout resumes/rotates intentionally, `/account.html` can still sign in with Google, receipt and Customer Portal work, the webhook only records/queues signed Stripe events, and account/session reads never process queue backlog.
...
- Production still lacks `sidestream_checkout_intents` and `sidestream_licenses.entitlement_status`; the checkout-intent and entitlement-lifecycle migrations remain not migrated and outside this run. In contrast, the exact additive `20260714190000_add_single_active_account_devices.sql` migration was applied and verified against Production on 2026-07-21, restoring `sidestream_account_devices` and `sidestream_device_transfers` for account claim, credential verification, refresh, download authorization, and transfer/deactivation paths. This is schema availability only: no deployment or device-policy enforcement changed, and broader hardening plus Customer 360 migrations remain outside this run. Customer-facing lifecycle reads must still use `LICENSE_ENTITLEMENT_STATUS_SQL`; a direct `l.entitlement_status` reference fails at PostgreSQL parse time before fallback logic can run. Do not replace the remaining gaps with runtime DDL, a manual table or column addition, or an unreviewed migration. The compatibility expression recognizes only exact attached one-time paid rows and the legacy credential path remains device-hash bound.
...
- 2026-07-21: Repaired a Production v1.0.14 Upgrade regression caused by deploying the ManyChat change from a branch that omitted the route-scoped activation-claim middleware and legacy unmarked-URL recovery. The Production branch now combines both features; future deploys must preserve the shipped-panel compatibility chain documented above.
