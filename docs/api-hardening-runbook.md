...
| `/api/auth/session` | `GET` | Always `200`: `{"authenticated":false}` or authenticated `user`, `license`, and billing booleans for Customer/subscription/cancellation capability | Dependency failure is a server `500`; this read never processes Stripe events or exposes Stripe identifiers |
...
| `/api/billing/subscription/cancel` | `POST` | Authenticated `200` after the server-owned active Subscription is verified against the account Customer and set to cancel at period end | `400` no cancellable active subscription, `401` unauthenticated, `403` Customer mismatch, `409` missing/mismatched provider identity, `502` Stripe did not confirm cancellation |
