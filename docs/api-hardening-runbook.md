...
- All runtime database users share the pool in `api/_lib/postgres.ts` and use a
  ...
  URL.
- Anonymous `GET /api/checkout/start` is a read/confirmation boundary. Only a
  same-origin `POST /api/checkout/create` may create or reuse a website Checkout
  Session: anonymous requests require signed confirmation fields, while the
...
1. Anonymous entry points use `GET /api/checkout/start`, which creates or resumes
   a database Checkout intent and renders a no-store confirmation page. It must
   not initialize Stripe, create a Customer, resolve/create a Price, or create a
   Checkout Session. Activation-bearing shipped-panel GETs retain their separate
   direct, capability-bound compatibility path.
2. Anonymous forms submit signed intent fields to same-origin
...
3. The POST atomically consumes limits of 8 requests per anonymous intent or 8
   per direct account, plus 20 per IP, in a 15-minute window. It locks the
...
returns to `account.html`; anonymous cancellations remain GET reads whose next
signed confirmation POST may explicitly request bounded Session rotation. A
caller cannot supply its own Stripe or activation tuple.
...
| Checkout volume | Confirmation GETs, direct-account and confirmed anonymous POSTs, created/reused Sessions, `csrf_rejected`, dependency errors | Warning when create failures exceed 1% or 5 in 15 minutes. A GET without a matching confirmation POST is abandonment, not a Stripe failure. |
