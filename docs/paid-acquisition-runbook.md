...
| Stripe | `STRIPE_SECRET_KEY` in test mode plus reviewed Test `SIDESTREAM_PRO_PRODUCT_ID` and `SIDESTREAM_PRO_PRICE_ID` for USD 999 |
...
9. After separate payment approval, complete a real Stripe **test-mode**
   payment and verify server-retrieved USD 999 Product/Price/quantity/currency
